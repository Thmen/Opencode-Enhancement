[Home](README.md) / Orchestrate teams of Claude Code sessions
...
* [When to use agent teams](Getting started/02-ClaudeCodeoverview.md#when-to-use-agent-teams), including best use cases and how they compare with subagents
* [Starting a team](Getting started/02-ClaudeCodeoverview.md#start-your-first-agent-team)
* [Controlling teammates](Getting started/02-ClaudeCodeoverview.md#control-your-agent-team), including display modes, task assignment, and delegation
* [Best practices for parallel work](Getting started/02-ClaudeCodeoverview.md#best-practices)

## When to use agent teams

Agent teams are most effective for tasks where parallel exploration adds real value. See [use case examples](Getting started/02-ClaudeCodeoverview.md#use-case-examples) for full scenarios. The strongest use cases are:
...
Agent teams add coordination overhead and use significantly more tokens than a single session. They work best when teammates can operate independently. For sequential tasks, same-file edits, or work with many dependencies, a single session or [subagents](48-Createcustomsubagents.md) are more effective.
...
From there, Claude creates a team with a [shared task list](26-Interactivemode.md#task-list), spawns teammates for each perspective, has them explore the problem, synthesizes findings, and attempts to [clean up the team](Getting started/02-ClaudeCodeoverview.md#clean-up-the-team) when finished.
...
If you want each teammate in its own split pane, see [Choose a display mode](Getting started/02-ClaudeCodeoverview.md#choose-a-display-mode).
...
Use [hooks](24-Hooksreference.md) to enforce rules when teammates finish work or tasks complete:

* [`TeammateIdle`](24-Hooksreference.md#teammateidle): runs when a teammate is about to go idle. Exit with code 2 to send feedback and keep the teammate working.
* [`TaskCompleted`](24-Hooksreference.md#taskcompleted): runs when a task is being marked complete. Exit with code 2 to prevent completion and send feedback.

## How agent teams work

This section covers the architecture and mechanics behind agent teams. If you want to start using them, see [Control your agent team](Getting started/02-ClaudeCodeoverview.md#control-your-agent-team) above.
...
See [Choose a display mode](Getting started/02-ClaudeCodeoverview.md#choose-a-display-mode) for display configuration options. Teammate messages arrive at the lead automatically.
...
* **Token costs scale linearly**: each teammate has its own context window and consumes tokens independently. See [agent team token costs](12-Managecostseffectively.md#agent-team-token-costs) for details.
...
Having 5-6 [tasks](01-OrchestrateteamsofClaudeCodesessions.md#architecture) per teammate keeps everyone productive without excessive context switching. If you have 15 independent tasks, 3 teammates is a good starting point.
...
Teammate permission requests bubble up to the lead, which can create friction. Pre-approve common operations in your [permission settings](36-Configurepermissions.md) before spawning teammates to reduce interruptions.
...
* **Lightweight delegation**: [subagents](48-Createcustomsubagents.md) spawn helper agents for research or verification within your session, better for tasks that don't need inter-agent coordination
* **Manual parallel sessions**: [Git worktrees](Use Claude Code/02-Commonworkflows.md#run-parallel-claude-code-sessions-with-git-worktrees) let you run multiple Claude Code sessions yourself without automated team coordination
* **Compare approaches**: see the [subagent vs agent team](Core concepts/01-ExtendClaudeCode.md#compare-similar-features) comparison for a side-by-side breakdown
