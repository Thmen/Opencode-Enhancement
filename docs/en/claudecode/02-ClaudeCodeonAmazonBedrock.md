[Home](README.md) / Claude Code on Amazon Bedrock
...
<Note>
  If you are deploying Claude Code to multiple users, [pin your model versions](Getting started/02-ClaudeCodeoverview.md#4-pin-model-versions) to prevent breakage when Anthropic releases new models.
</Note>
...
Claude Code supports automatic credential refresh for AWS SSO and corporate identity providers. Add these settings to your Claude Code settings file (see [Settings](44-ClaudeCodesettings.md) for file locations).
...
* You can use settings files for environment variables like `AWS_PROFILE` that you don't want to leak to other processes. See [Settings](44-ClaudeCodesettings.md) for more information.
...
When a user selects one of these versions in `/model`, Claude Code calls Bedrock with the mapped ARN. Versions without an override fall back to the built-in Bedrock model ID or any matching inference profile discovered at startup. See [Override model IDs per version](32-Modelconfiguration.md#override-model-ids-per-version) for details on how overrides interact with `availableModels` and other model settings.
...
[Amazon Bedrock Guardrails](https://docs.aws.amazon.com/bedrock/latest/userguide/guardrails.html) let you implement content filtering for Claude Code. Create a Guardrail in the [Amazon Bedrock console](https://console.aws.amazon.com/bedrock/), publish a version, then add the Guardrail headers to your [settings file](44-ClaudeCodesettings.md). Enable Cross-Region inference on your Guardrail if you're using cross-region inference profiles.
