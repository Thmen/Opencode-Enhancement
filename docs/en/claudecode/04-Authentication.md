...
After [installing Claude Code](45-Advancedsetup.md#install-claude-code), run `claude` in your terminal. On first launch, Claude Code opens a browser window for you to log in.
...
* **Claude Console**: log in with your Console credentials. Your admin must have [invited you](Getting started/02-ClaudeCodeoverview.md#claude-console-authentication) first.
* **Cloud providers**: if your organization uses [Amazon Bedrock](02-ClaudeCodeonAmazonBedrock.md), [Google Vertex AI](22-ClaudeCodeonGoogleVertexAI.md), or [Microsoft Foundry](31-ClaudeCodeonMicrosoftFoundry.md), set the required environment variables before running `claude`. No browser login is needed.

To log out and re-authenticate, type `/logout` at the Claude Code prompt.

If you're having trouble logging in, see [authentication troubleshooting](52-Troubleshooting.md#authentication-issues).

## Set up team authentication

For teams and organizations, you can configure Claude Code access in one of these ways:

* [Claude for Teams or Enterprise](Getting started/02-ClaudeCodeoverview.md#claude-for-teams-or-enterprise), recommended for most teams
* [Claude Console](Getting started/02-ClaudeCodeoverview.md#claude-console-authentication)
...
* **Custom credential scripts**: the [`apiKeyHelper`](44-ClaudeCodesettings.md#available-settings) setting can be configured to run a shell script that returns an API key.
...
1. Cloud provider credentials, when `CLAUDE_CODE_USE_BEDROCK`, `CLAUDE_CODE_USE_VERTEX`, or `CLAUDE_CODE_USE_FOUNDRY` is set. See [third-party integrations](50-Enterprisedeploymentoverview.md) for setup.
2. `ANTHROPIC_AUTH_TOKEN` environment variable. Sent as the `Authorization: Bearer` header. Use this when routing through an [LLM gateway or proxy](29-LLMgatewayconfiguration.md) that authenticates with bearer tokens rather than Anthropic API keys.
...
4. [`apiKeyHelper`](44-ClaudeCodesettings.md#available-settings) script output. Use this for dynamic or rotating credentials, such as short-lived tokens fetched from a vault.
...
[Claude Code on the Web](08-ClaudeCodeontheweb.md) always uses your subscription credentials. `ANTHROPIC_API_KEY` and `ANTHROPIC_AUTH_TOKEN` in the sandbox environment do not override them.
