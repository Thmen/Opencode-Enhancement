[Home](README.md) / Push events into a running session with channels
...
Unlike integrations that spawn a fresh cloud session or wait to be polled, the event arrives in the session you already have open: see [how channels compare](Getting started/02-ClaudeCodeoverview.md#how-channels-compare).
...
* [Supported channels](Getting started/02-ClaudeCodeoverview.md#supported-channels): Telegram, Discord, and iMessage setup
* [Install and run a channel](Getting started/02-ClaudeCodeoverview.md#quickstart) with fakechat, a localhost demo
* [Who can push messages](Getting started/02-ClaudeCodeoverview.md#security): sender allowlists and how you pair
* [Enable channels for your organization](Getting started/02-ClaudeCodeoverview.md#enterprise-controls) on Team and Enterprise
* [How channels compare](Getting started/02-ClaudeCodeoverview.md#how-channels-compare) to web sessions, Slack, MCP, and Remote Control

To build your own channel, see the [Channels reference](06-Channelsreference.md).
...
* Claude Code [installed and authenticated](Getting started/03-Quickstart.md#step-1-install-claude-code) with a claude.ai account
...
On top of that, you control which servers are enabled each session with `--channels`, and on Team and Enterprise plans your organization controls availability with [`channelsEnabled`](Getting started/02-ClaudeCodeoverview.md#enterprise-controls).

Being in `.mcp.json` isn't enough to push messages: a server also has to be named in `--channels`.

The allowlist also gates [permission relay](06-Channelsreference.md#relay-permission-prompts) if the channel declares it. Anyone who can reply through the channel can approve or deny tool use in your session, so only allowlist senders you trust with that authority.
...
| ---------------------------------------------------- | --------------------------------------------------------------------- | --------------------------------------------------------- |
| [Claude Code on the web](08-ClaudeCodeontheweb.md) | Runs tasks in a fresh cloud sandbox, cloned from GitHub               | Delegating self-contained async work you check on later   |
| [Claude in Slack](Platforms and integrations/05-ClaudeCodeinSlack.md)                         | Spawns a web session from an `@Claude` mention in a channel or thread | Starting tasks directly from team conversation context    |
| Standard [MCP server](30-ConnectClaudeCodetotoolsviaMCP.md)                       | Claude queries it during a task; nothing is pushed to the session     | Giving Claude on-demand access to read or query a system  |
| [Remote Control](Platforms and integrations/04-ContinuelocalsessionsfromanydevicewithRemoteControl.md)                 | You drive your local session from claude.ai or the Claude mobile app  | Steering an in-progress session while away from your desk |
...
* **[Webhook receiver](06-Channelsreference.md#example-build-a-webhook-receiver)**: a webhook from CI, your error tracker, a deploy pipeline, or other external service arrives where Claude already has your files open and remembers what you were debugging.

## Next steps

Once you have a channel running, explore these related features:

* [Build your own channel](06-Channelsreference.md) for systems that don't have plugins yet
* [Remote Control](Platforms and integrations/04-ContinuelocalsessionsfromanydevicewithRemoteControl.md) to drive a local session from your phone instead of forwarding events into it
* [Scheduled tasks](41-Runpromptsonaschedule.md) to poll on a timer instead of reacting to pushed events
