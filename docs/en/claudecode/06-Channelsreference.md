[Home](README.md) / Channels reference
...
**Note:**
Channels are in [research preview](05-Pusheventsintoarunningsessionwithchannels.md#research-preview) and require Claude Code v2.1.80 or later. They require claude.ai login. Console and API key authentication is not supported. Team and Enterprise organizations must [explicitly enable them](05-Pusheventsintoarunningsessionwithchannels.md#enterprise-controls).
...
You can build a one-way or two-way channel. One-way channels forward alerts, webhooks, or monitoring events for Claude to act on. Two-way channels like chat bridges also [expose a reply tool](Getting started/02-ClaudeCodeoverview.md#expose-a-reply-tool) so Claude can send messages back. A channel with a trusted sender path can also opt in to [relay permission prompts](Getting started/02-ClaudeCodeoverview.md#relay-permission-prompts) so you can approve or deny tool use remotely.

This page covers:

* [Overview](Getting started/02-ClaudeCodeoverview.md#overview): how channels work
* [What you need](Getting started/02-ClaudeCodeoverview.md#what-you-need): requirements and general steps
* [Example: build a webhook receiver](Getting started/02-ClaudeCodeoverview.md#example-build-a-webhook-receiver): a minimal one-way walkthrough
* [Server options](Getting started/02-ClaudeCodeoverview.md#server-options): the constructor fields
* [Notification format](Getting started/02-ClaudeCodeoverview.md#notification-format): the event payload
* [Expose a reply tool](Getting started/02-ClaudeCodeoverview.md#expose-a-reply-tool): let Claude send messages back
* [Gate inbound messages](Getting started/02-ClaudeCodeoverview.md#gate-inbound-messages): sender checks to prevent prompt injection
* [Relay permission prompts](Getting started/02-ClaudeCodeoverview.md#relay-permission-prompts): forward tool approval prompts to remote channels

To use an existing channel instead of building one, see [Channels](05-Pusheventsintoarunningsessionwithchannels.md). Telegram, Discord, iMessage, and fakechat are included in the research preview.
...
The [Server options](Getting started/02-ClaudeCodeoverview.md#server-options) and [Notification format](Getting started/02-ClaudeCodeoverview.md#notification-format) sections cover each of these in detail. See [Example: build a webhook receiver](Getting started/02-ClaudeCodeoverview.md#example-build-a-webhook-receiver) for a full walkthrough.

During the research preview, custom channels aren't on the [approved allowlist](05-Pusheventsintoarunningsessionwithchannels.md#supported-channels). Use `--dangerously-load-development-channels` to test locally. See [Test during the research preview](Getting started/02-ClaudeCodeoverview.md#test-during-the-research-preview) for details.
...
If you see "blocked by org policy," your Team or Enterprise admin needs to [enable channels](05-Pusheventsintoarunningsessionwithchannels.md#enterprise-controls) first.
...
In your Claude Code terminal, you'll see Claude receive the message and start responding: reading files, running commands, or whatever the message calls for. This is a one-way channel, so Claude acts in your session but doesn't send anything back through the webhook. To add replies, see [Expose a reply tool](Getting started/02-ClaudeCodeoverview.md#expose-a-reply-tool).
...
During the research preview, every channel must be on the [approved allowlist](05-Pusheventsintoarunningsessionwithchannels.md#research-preview) to register. The development flag bypasses the allowlist for specific entries after a confirmation prompt. This example shows both entry types:
...
| `capabilities.experimental['claude/channel/permission']` | `object` | Optional. Always `{}`. Declares that this channel can receive permission relay requests. When declared, Claude Code forwards tool approval prompts to your channel so you can approve or deny them remotely. See [Relay permission prompts](Getting started/02-ClaudeCodeoverview.md#relay-permission-prompts). |
| `capabilities.tools`                                     | `object` | Two-way only. Always `{}`. Standard MCP tool capability. See [Expose a reply tool](Getting started/02-ClaudeCodeoverview.md#expose-a-reply-tool).                                                                                                                                                               |
...
To add these to the [webhook receiver above](Getting started/02-ClaudeCodeoverview.md#example-build-a-webhook-receiver):
...
Only declare the capability if your channel [authenticates the sender](Getting started/02-ClaudeCodeoverview.md#gate-inbound-messages), because anyone who can reply through your channel can approve or deny tool use in your session.

To add these to a two-way chat bridge like the one assembled in [Expose a reply tool](Getting started/02-ClaudeCodeoverview.md#expose-a-reply-tool):
...
Register a notification handler between your `Server` constructor and `mcp.connect()`. Claude Code calls it with the [four request fields](Getting started/02-ClaudeCodeoverview.md#permission-request-fields) when a permission dialog opens. Your handler formats the prompt for your platform and includes instructions for replying with the ID:
...
The assembled `webhook.ts` below combines all three extensions from this page: the reply tool, sender gating, and permission relay. If you're starting here, you'll also need the [project setup and `.mcp.json` entry](Getting started/02-ClaudeCodeoverview.md#example-build-a-webhook-receiver) from the initial walkthrough.
...
Test the verdict path in three terminals. The first is your Claude Code session, started with the [development flag](Getting started/02-ClaudeCodeoverview.md#test-during-the-research-preview) so it spawns `webhook.ts`:
...
To make your channel installable and shareable, wrap it in a [plugin](38-Createplugins.md) and publish it to a [marketplace](37-Createanddistributeapluginmarketplace.md). Users install it with `/plugin install`, then enable it per session with `--channels plugin:<name>@<marketplace>`.

A channel published to your own marketplace still needs `--dangerously-load-development-channels` to run, since it isn't on the [approved allowlist](05-Pusheventsintoarunningsessionwithchannels.md#supported-channels). To get it added, [submit it to the official marketplace](38-Createplugins.md#submit-your-plugin-to-the-official-marketplace). Channel plugins go through security review before being approved. On Team and Enterprise plans, an admin can instead include your plugin in the organization's own [`allowedChannelPlugins`](05-Pusheventsintoarunningsessionwithchannels.md#restrict-which-channel-plugins-can-run) list, which replaces the default Anthropic allowlist.

## See also

* [Channels](05-Pusheventsintoarunningsessionwithchannels.md) to install and use Telegram, Discord, iMessage, or the fakechat demo, and to enable channels for a Team or Enterprise org
...
* [MCP](30-ConnectClaudeCodetotoolsviaMCP.md) for the underlying protocol that channel servers implement
* [Plugins](38-Createplugins.md) to package your channel so users can install it with `/plugin install`
