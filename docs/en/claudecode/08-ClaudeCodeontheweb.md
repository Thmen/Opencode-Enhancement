[Home](README.md) / Claude Code on the web
...
You can [kick off new tasks on the web from your terminal](Getting started/02-ClaudeCodeoverview.md#from-terminal-to-web) with `--remote`, or [teleport web sessions back to your terminal](Getting started/02-ClaudeCodeoverview.md#from-web-to-terminal) to continue locally. To use the web interface while running Claude Code on your own machine instead of cloud infrastructure, see [Remote Control](Platforms and integrations/04-ContinuelocalsessionsfromanydevicewithRemoteControl.md).
...
2. **Environment setup**: Claude prepares a secure cloud environment with your code, then runs your [setup script](Getting started/02-ClaudeCodeoverview.md#setup-scripts) if configured
...
Monitor all sessions with `/tasks`. When a session completes, you can create a PR from the web interface or [teleport](Getting started/02-ClaudeCodeoverview.md#from-web-to-terminal) the session to your terminal to continue working.
...
display name is visible to all recipients with access. [Claude in Slack](Platforms and integrations/05-ClaudeCodeinSlack.md)
...
Run Claude on a recurring schedule to automate work like daily PR reviews, dependency audits, and CI failure analysis. See [Schedule tasks on the web](54-Scheduletasksontheweb.md) for the full guide.
...
1. **Environment preparation**: We clone your repository and run any configured [setup script](Getting started/02-ClaudeCodeoverview.md#setup-scripts). The repo will be cloned with the default branch on your GitHub repo. If you would like to check out a specific branch, you can specify that in the prompt.
...
A setup script is a Bash script that runs when a new cloud session starts, before Claude Code launches. Use setup scripts to install dependencies, configure tools, or prepare anything the cloud environment needs that isn't in the [default image](Getting started/02-ClaudeCodeoverview.md#default-image).
...
Use a setup script to install things the cloud needs but your laptop already has, like a language runtime or CLI tool. Use a [SessionStart hook](24-Hooksreference.md#sessionstart) for project setup that should run everywhere, cloud and local, like `npm install`.
...
Custom environment images and snapshots are not yet supported. Use [setup scripts](Getting started/02-ClaudeCodeoverview.md#setup-scripts) to install packages when a session starts, or [SessionStart hooks](24-Hooksreference.md#sessionstart) for dependency installation that should also run in local environments. SessionStart hooks have [known limitations](Getting started/02-ClaudeCodeoverview.md#dependency-management-limitations).
...
SessionStart hooks can persist environment variables for subsequent Bash commands by writing to the file specified in the `CLAUDE_ENV_FILE` environment variable. For details, see [SessionStart hooks](24-Hooksreference.md#sessionstart) in the hooks reference.
...
* **Requires network access**: Install commands need network access to reach package registries. If your environment is configured with "No internet" access, these hooks will fail. Use "Limited" (the default) or "Full" network access. The [default allowlist](Getting started/02-ClaudeCodeoverview.md#default-allowed-domains) includes common registries like npm, PyPI, RubyGems, and crates.io.
* **Proxy compatibility**: All outbound traffic in remote environments passes through a [security proxy](Getting started/02-ClaudeCodeoverview.md#security-proxy). Some package managers do not work correctly with this proxy. Bun is a known example.
...
By default, network access is limited to [allowlisted domains](Getting started/02-ClaudeCodeoverview.md#default-allowed-domains).
...
1. **Automate environment setup**: Use [setup scripts](Getting started/02-ClaudeCodeoverview.md#setup-scripts) to install dependencies and configure tools before Claude Code launches. For more advanced scenarios, configure [SessionStart hooks](24-Hooksreference.md#sessionstart).
...
* [Hooks configuration](24-Hooksreference.md)
* [Settings reference](44-ClaudeCodesettings.md)
* [Security](42-Security.md)
* [Data usage](13-Datausage.md)
