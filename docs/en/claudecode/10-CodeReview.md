...
To run Claude in your own CI infrastructure instead of this managed service, see [GitHub Actions](20-ClaudeCodeGitHubActions.md) or [GitLab CI/CD](21-ClaudeCodeGitLabCICD.md).

This page covers:

* [How reviews work](Getting started/02-ClaudeCodeoverview.md#how-reviews-work)
* [Setup](Getting started/02-ClaudeCodeoverview.md#set-up-code-review)
* [Customizing reviews](Getting started/02-ClaudeCodeoverview.md#customize-reviews) with `CLAUDE.md` and `REVIEW.md`
* [Pricing](Getting started/02-ClaudeCodeoverview.md#pricing)

## How reviews work

Once an admin [enables Code Review](Getting started/02-ClaudeCodeoverview.md#set-up-code-review) for your organization, reviews trigger when a PR opens, on every push, or when manually requested, depending on the repository's configured behavior. Commenting `@claude review` [starts reviews on a PR](Getting started/02-ClaudeCodeoverview.md#manually-trigger-reviews) in any mode.
...
Reviews scale in cost with PR size and complexity, completing in 20 minutes on average. Admins can monitor review activity and spend via the [analytics dashboard](Getting started/02-ClaudeCodeoverview.md#view-usage).
...
By default, Code Review focuses on correctness: bugs that would break production, not formatting preferences or missing test coverage. You can expand what it checks by [adding guidance files](Getting started/02-ClaudeCodeoverview.md#customize-reviews) to your repository.
...
Claude reads `CLAUDE.md` files at every level of your directory hierarchy, so rules in a subdirectory's `CLAUDE.md` apply only to files under that path. See the [memory documentation](Use Claude Code/03-HowClauderemembersyourproject.md) for more on how `CLAUDE.md` works.

For review-specific guidance that you don't want applied to general Claude Code sessions, use [`REVIEW.md`](Getting started/02-ClaudeCodeoverview.md#review-md) instead.
...
In any mode, commenting `@claude review` [opts the PR into push-triggered reviews](Getting started/02-ClaudeCodeoverview.md#manually-trigger-reviews), so additional cost accrues per push after that comment.
...
Monitor spend via the weekly cost chart in [analytics](Getting started/02-ClaudeCodeoverview.md#view-usage) or the per-repo average cost column in admin settings.
...
* [Plugins](17-Discoverandinstallprebuiltpluginsthroughmarketplaces.md): browse the plugin marketplace, including a `code-review` plugin for running on-demand reviews locally before pushing
* [GitHub Actions](20-ClaudeCodeGitHubActions.md): run Claude in your own GitHub Actions workflows for custom automation beyond code review
* [GitLab CI/CD](21-ClaudeCodeGitLabCICD.md): self-hosted Claude integration for GitLab pipelines
* [Memory](Use Claude Code/03-HowClauderemembersyourproject.md): how `CLAUDE.md` files work across Claude Code
* [Analytics](03-Trackteamusagewithanalytics.md): track Claude Code usage beyond code review
