[Home](README.md) / Manage costs effectively
...
This page covers how to [track your costs](Getting started/02-ClaudeCodeoverview.md#track-your-costs), [manage costs for teams](Getting started/02-ClaudeCodeoverview.md#managing-costs-for-teams), and [reduce token usage](Getting started/02-ClaudeCodeoverview.md#reduce-token-usage).
...
[Agent teams](01-OrchestrateteamsofClaudeCodesessions.md) spawn multiple Claude Code instances, each with its own context window. Token usage scales with the number of active teammates and how long each one runs.
...
* Agent teams are disabled by default. Set `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` in your [settings.json](44-ClaudeCodesettings.md) or environment to enable them. See [enable agent teams](01-OrchestrateteamsofClaudeCodesessions.md#enable-agent-teams).
...
Use `/cost` to check your current token usage, or [configure your status line](47-Customizeyourstatusline.md#context-window-usage) to display it continuously.
...
Your [CLAUDE.md](Use Claude Code/03-HowClauderemembersyourproject.md) file is loaded into context at session start. If it contains detailed instructions for specific workflows (like PR reviews or database migrations), those tokens are present even when you're doing unrelated work. [Skills](46-ExtendClaudewithskills.md) load on-demand only when invoked, so moving specialized instructions into skills keeps your base context smaller. Aim to keep CLAUDE.md under \~500 lines by including only essentials.

### Adjust extended thinking

Extended thinking is enabled by default because it significantly improves performance on complex planning and reasoning tasks. Thinking tokens are billed as output tokens, and the default budget can be tens of thousands of tokens per request depending on the model. For simpler tasks where deep reasoning isn't needed, you can reduce costs by lowering the [effort level](32-Modelconfiguration.md#adjust-effort-level) with `/effort` or in `/model`, disabling thinking in `/config`, or lowering the budget with `MAX_THINKING_TOKENS=8000`.

### Delegate verbose operations to subagents

Running tests, fetching documentation, or processing log files can consume significant context. Delegate these to [subagents](48-Createcustomsubagents.md#isolate-high-volume-operations) so the verbose output stays in the subagent's context while only a summary returns to your main conversation.

### Manage agent team costs

Agent teams use approximately 7x more tokens than standard sessions when teammates run in plan mode, because each teammate maintains its own context window and runs as a separate Claude instance. Keep team tasks small and self-contained to limit per-teammate token usage. See [agent teams](01-OrchestrateteamsofClaudeCodesessions.md) for details.
...
* **Use plan mode for complex tasks**: Press Shift+Tab to enter [plan mode](Use Claude Code/02-Commonworkflows.md#use-plan-mode-for-safe-code-analysis) before implementation. Claude explores the codebase and proposes an approach for your approval, preventing expensive re-work when the initial direction is wrong.
