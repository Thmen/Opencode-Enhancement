[Home](README.md) / Discover and install prebuilt plugins through marketplaces
...
Looking to create and distribute your own marketplace? See [Create and distribute a plugin marketplace](37-Createanddistributeapluginmarketplace.md).
...
If you run into issues, see [Code intelligence troubleshooting](Getting started/02-ClaudeCodeoverview.md#code-intelligence-issues).

### External integrations

These plugins bundle pre-configured [MCP servers](30-ConnectClaudeCodetotoolsviaMCP.md) so you can connect Claude to external services without manual setup:
...
To choose a different [installation scope](44-ClaudeCodesettings.md#configuration-scopes), use the interactive UI: run `/plugin`, go to the **Discover** tab, and press **Enter** on a plugin. You'll see options for:
...
You may also see plugins with **managed** scope—these are installed by administrators via [managed settings](44-ClaudeCodesettings.md#settings-files) and cannot be modified.
...
To disable all automatic updates entirely for both Claude Code and all plugins, set the `DISABLE_AUTOUPDATER` environment variable. See [Auto updates](45-Advancedsetup.md#auto-updates) for details.
...
For full configuration options including `extraKnownMarketplaces` and `enabledPlugins`, see [Plugin settings](44-ClaudeCodesettings.md#plugin-settings).

## Security

Plugins and marketplaces are highly trusted components that can execute arbitrary code on your machine with your user privileges. Only install plugins and add marketplaces from sources you trust. Organizations can restrict which marketplaces users are allowed to add using [managed marketplace restrictions](37-Createanddistributeapluginmarketplace.md#managed-marketplace-restrictions).
...
2. **Update Claude Code**:
   ...
   * **Native installer**: Re-run the install command from [Setup](45-Advancedsetup.md)
...
For detailed troubleshooting with solutions, see [Troubleshooting](37-Createanddistributeapluginmarketplace.md#troubleshooting) in the marketplace guide. For debugging tools, see [Debugging and development tools](39-Pluginsreference.md#debugging-and-development-tools).
...
* **Build your own plugins**: See [Plugins](38-Createplugins.md) to create skills, agents, and hooks
* **Create a marketplace**: See [Create a plugin marketplace](37-Createanddistributeapluginmarketplace.md) to distribute plugins to your team or community
* **Technical reference**: See [Plugins reference](39-Pluginsreference.md) for complete specifications
