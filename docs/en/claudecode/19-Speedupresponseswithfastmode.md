[Home](README.md) / Speed up responses with fast mode
...
This page covers how to [toggle fast mode](Getting started/02-ClaudeCodeoverview.md#toggle-fast-mode), its [cost tradeoff](Getting started/02-ClaudeCodeoverview.md#understand-the-cost-tradeoff), [when to use it](Getting started/02-ClaudeCodeoverview.md#decide-when-to-use-fast-mode), [requirements](Getting started/02-ClaudeCodeoverview.md#requirements), [per-session opt-in](Getting started/02-ClaudeCodeoverview.md#require-per-session-opt-in), and [rate limit behavior](Getting started/02-ClaudeCodeoverview.md#handle-rate-limits).
...
* Set `"fastMode": true` in your [user settings file](44-ClaudeCodesettings.md)

By default, fast mode persists across sessions. Administrators can configure fast mode to reset each session. See [require per-session opt-in](Getting started/02-ClaudeCodeoverview.md#require-per-session-opt-in) for details.

For the best cost efficiency, enable fast mode at the start of a session rather than switching mid-conversation. See [understand the cost tradeoff](Getting started/02-ClaudeCodeoverview.md#understand-the-cost-tradeoff) for details.
...
You can combine both: use fast mode with a lower [effort level](32-Modelconfiguration.md#adjust-effort-level) for maximum speed on straightforward tasks.
...
Another option to disable fast mode entirely is to set `CLAUDE_CODE_DISABLE_FAST_MODE=1`. See [Environment variables](18-Environmentvariables.md).
...
* [Model configuration](32-Modelconfiguration.md): switch models and adjust effort levels
* [Manage costs effectively](12-Managecostseffectively.md): track token usage and reduce costs
* [Status line configuration](47-Customizeyourstatusline.md): display model and context information
