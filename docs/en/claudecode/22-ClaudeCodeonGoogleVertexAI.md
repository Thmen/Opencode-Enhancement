[Home](README.md) / Claude Code on Google Vertex AI
...
**Note:**
If you are deploying Claude Code to multiple users, [pin your model versions](Getting started/02-ClaudeCodeoverview.md#5-pin-model-versions) to prevent breakage when Anthropic releases new models.
...
For current and legacy model IDs, see [Models overview](https://platform.claude.com/docs/en/about-claude/models/overview). See [Model configuration](32-Modelconfiguration.md#pin-models-for-third-party-deployments) for the full list of environment variables.
...
To enable the 1M context window for your pinned model, append `[1m]` to the model ID. See [Pin models for third-party deployments](32-Modelconfiguration.md#pin-models-for-third-party-deployments) for details.
