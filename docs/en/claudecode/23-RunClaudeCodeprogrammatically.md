[Home](README.md) / Run Claude Code programmatically
...
To run Claude Code programmatically from the CLI, pass `-p` with your prompt and any [CLI options](09-CLIreference.md):
...
Add the `-p` (or `--print`) flag to any `claude` command to run it non-interactively. All [CLI options](09-CLIreference.md) work with `-p`, including:

* `--continue` for [continuing conversations](Getting started/02-ClaudeCodeoverview.md#continue-conversations)
* `--allowedTools` for [auto-approving tools](Getting started/02-ClaudeCodeoverview.md#auto-approve-tools)
* `--output-format` for [structured output](Getting started/02-ClaudeCodeoverview.md#get-structured-output)
...
Add `--bare` to reduce startup time by skipping auto-discovery of hooks, skills, plugins, MCP servers, auto memory, and CLAUDE.md. Without it, `claude -p` loads the same [context](Core concepts/02-HowClaudeCodeworks.md#the-context-window) an interactive session would, including anything configured in the working directory or `~/.claude`.
...
These examples highlight common CLI patterns. For CI and other scripted calls, add [`--bare`](Getting started/02-ClaudeCodeoverview.md#start-faster-with-bare-mode) so they don't pick up whatever happens to be configured locally.
...
The `--allowedTools` flag uses [permission rule syntax](44-ClaudeCodesettings.md#permission-rule-syntax). The trailing ` *` enables prefix matching, so `Bash(git diff *)` allows any command starting with `git diff`. The space before `*` is important: without it, `Bash(git diff*)` would also match `git diff-index`.

**Note:**
User-invoked [skills](46-ExtendClaudewithskills.md) like `/commit` and [built-in commands](11-Built-incommands.md) are only available in interactive mode. In `-p` mode, describe the task you want to accomplish instead.
...
See [system prompt flags](09-CLIreference.md#system-prompt-flags) for more options including `--system-prompt` to fully replace the default prompt.
...
* [CLI reference](09-CLIreference.md): all CLI flags and options
* [GitHub Actions](20-ClaudeCodeGitHubActions.md): use the Agent SDK in GitHub workflows
* [GitLab CI/CD](21-ClaudeCodeGitLabCICD.md): use the Agent SDK in GitLab pipelines
