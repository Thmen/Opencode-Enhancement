[Home](README.md) / Hooks reference
...
The table below summarizes when each event fires. The [Hook events](Getting started/02-ClaudeCodeoverview.md#hook-events) section documents the full input schema and decision control options for each one.
...
The [Configuration](Getting started/02-ClaudeCodeoverview.md#configuration) section below documents the full schema, and each [hook event](Getting started/02-ClaudeCodeoverview.md#hook-events) section documents what input your command receives and what output it can return.

## Configuration

Hooks are defined in JSON settings files. The configuration has three levels of nesting:

1. Choose a [hook event](Getting started/02-ClaudeCodeoverview.md#hook-events) to respond to, like `PreToolUse` or `Stop`
2. Add a [matcher group](Getting started/02-ClaudeCodeoverview.md#matcher-patterns) to filter when it fires, like "only for the Bash tool"
3. Define one or more [hook handlers](Getting started/02-ClaudeCodeoverview.md#hook-handler-fields) to run when matched

See [How a hook resolves](Getting started/02-ClaudeCodeoverview.md#how-a-hook-resolves) above for a complete walkthrough with an annotated example.
...
| [Plugin](38-Createplugins.md) `hooks/hooks.json`                   | When plugin is enabled        | Yes, bundled with the plugin       |
| [Skill](46-ExtendClaudewithskills.md) or [agent](48-Createcustomsubagents.md) frontmatter | While the component is active | Yes, defined in the component file |

For details on settings file resolution, see [settings](44-ClaudeCodesettings.md). Enterprise administrators can use `allowManagedHooksOnly` to block user, project, and plugin hooks. See [Hook configuration](44-ClaudeCodesettings.md#hook-configuration).
...
The matcher is a regex, so `Edit|Write` matches either tool and `Notebook.*` matches any tool starting with Notebook. The matcher runs against a field from the [JSON input](Getting started/02-ClaudeCodeoverview.md#hook-input-and-output) that Claude Code sends to your hook on stdin. For tool events, that field is `tool_name`. Each [hook event](Getting started/02-ClaudeCodeoverview.md#hook-events) section lists the full set of matcher values and the input schema for that event.
...
[MCP](30-ConnectClaudeCodetotoolsviaMCP.md) server tools appear as regular tools in tool events (`PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `PermissionRequest`), so you can match them the same way you match any other tool name.
...
* **[Command hooks](Getting started/02-ClaudeCodeoverview.md#command-hook-fields)** (`type: "command"`): run a shell command. Your script receives the event's [JSON input](Getting started/02-ClaudeCodeoverview.md#hook-input-and-output) on stdin and communicates results back through exit codes and stdout.
* **[HTTP hooks](Getting started/02-ClaudeCodeoverview.md#http-hook-fields)** (`type: "http"`): send the event's JSON input as an HTTP POST request to a URL. The endpoint communicates results back through the response body using the same [JSON output format](Getting started/02-ClaudeCodeoverview.md#json-output) as command hooks.
* **[Prompt hooks](Getting started/02-ClaudeCodeoverview.md#prompt-and-agent-hook-fields)** (`type: "prompt"`): send a prompt to a Claude model for single-turn evaluation. The model returns a yes/no decision as JSON. See [Prompt-based hooks](Getting started/02-ClaudeCodeoverview.md#prompt-based-hooks).
* **[Agent hooks](Getting started/02-ClaudeCodeoverview.md#prompt-and-agent-hook-fields)** (`type: "agent"`): spawn a subagent that can use tools like Read, Grep, and Glob to verify conditions before returning a decision. See [Agent-based hooks](Getting started/02-ClaudeCodeoverview.md#agent-based-hooks).
...
| `once`          | no       | If `true`, runs only once per session then is removed. Skills only, not agents. See [Hooks in skills and agents](Getting started/02-ClaudeCodeoverview.md#hooks-in-skills-and-agents) |

#### Command hook fields

In addition to the [common fields](Getting started/02-ClaudeCodeoverview.md#common-fields), command hooks accept these fields:

| Field     | Required | Description                                                                                                                                                                                                                           |
| :-------- | :------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `command` | yes      | Shell command to execute                                                                                                                                                                                                              |
| `async`   | no       | If `true`, runs in the background without blocking. See [Run hooks in the background](Getting started/02-ClaudeCodeoverview.md#run-hooks-in-the-background)                                                                                                                   |
...
In addition to the [common fields](Getting started/02-ClaudeCodeoverview.md#common-fields), HTTP hooks accept these fields:
...
Claude Code sends the hook's [JSON input](Getting started/02-ClaudeCodeoverview.md#hook-input-and-output) as the POST request body with `Content-Type: application/json`. The response body uses the same [JSON output format](Getting started/02-ClaudeCodeoverview.md#json-output) as command hooks.
...
In addition to the [common fields](Getting started/02-ClaudeCodeoverview.md#common-fields), prompt and agent hooks accept these fields:
...
* `${CLAUDE_PLUGIN_ROOT}`: the plugin's installation directory, for scripts bundled with a [plugin](38-Createplugins.md). Changes on each plugin update.
* `${CLAUDE_PLUGIN_DATA}`: the plugin's [persistent data directory](39-Pluginsreference.md#persistent-data-directory), for dependencies and state that should survive plugin updates.
...
See the [plugin components reference](39-Pluginsreference.md#hooks) for details on creating plugin hooks.

### Hooks in skills and agents

In addition to settings files and plugins, hooks can be defined directly in [skills](46-ExtendClaudewithskills.md) and [subagents](48-Createcustomsubagents.md) using frontmatter. These hooks are scoped to the component's lifecycle and only run when that component is active.
...
Command hooks receive JSON data via stdin and communicate results through exit codes, stdout, and stderr. HTTP hooks receive the same JSON as the POST request body and communicate results through the HTTP response body. This section covers fields and behavior common to all events. Each event's section under [Hook events](Getting started/02-ClaudeCodeoverview.md#hook-events) includes its specific input schema and decision control options.

### Common input fields

Hook events receive these fields as JSON, in addition to event-specific fields documented in each [hook event](Getting started/02-ClaudeCodeoverview.md#hook-events) section. For command hooks, this JSON arrives via stdin. For HTTP hooks, it arrives as the POST request body.
...
| `permission_mode` | Current [permission mode](36-Configurepermissions.md#permission-modes): `"default"`, `"plan"`, `"acceptEdits"`, `"auto"`, `"dontAsk"`, or `"bypassPermissions"`. Not all events receive this field: see each event's JSON example below to check |
...
The `tool_name` and `tool_input` fields are event-specific. Each [hook event](Getting started/02-ClaudeCodeoverview.md#hook-events) section documents the additional fields for that event.

### Exit code output

The exit code from your hook command tells Claude Code whether the action should proceed, be blocked, or be ignored.

**Exit 0** means success. Claude Code parses stdout for [JSON output fields](Getting started/02-ClaudeCodeoverview.md#json-output). JSON output is only processed on exit 0. For most events, stdout is only shown in verbose mode (`Ctrl+O`). The exceptions are `UserPromptSubmit` and `SessionStart`, where stdout is added as context that Claude can see and act on.

**Exit 2** means a blocking error. Claude Code ignores stdout and any JSON in it. Instead, stderr text is fed back to Claude as an error message. The effect depends on the event: `PreToolUse` blocks the tool call, `UserPromptSubmit` rejects the prompt, and so on. See [exit code 2 behavior](Getting started/02-ClaudeCodeoverview.md#exit-code-2-behavior-per-event) for the full list.
...
* **2xx with a JSON body**: success, parsed using the same [JSON output](Getting started/02-ClaudeCodeoverview.md#json-output) schema as command hooks
...
Exit codes let you allow or block, but JSON output gives you finer-grained control. Instead of exiting with code 2 to block, exit 0 and print a JSON object to stdout. Claude Code reads specific fields from that JSON to control behavior, including [decision control](Getting started/02-ClaudeCodeoverview.md#decision-control) for blocking, allowing, or escalating to the user.

**Note:**
You must choose one approach per hook, not both: either use exit codes alone for signaling, or exit 0 and print JSON for structured control. Claude Code only processes JSON on exit 0. If you exit 2, any JSON is ignored.

Your hook's stdout must contain only the JSON object. If your shell profile prints text on startup, it can interfere with JSON parsing. See [JSON validation failed](25-Automateworkflowswithhooks.md#json-validation-failed) in the troubleshooting guide.
...
#### PreToolUse
Uses `hookSpecificOutput` for richer control: allow, deny, or escalate to the user. You can also modify tool input before it runs or inject additional context for Claude. See [PreToolUse decision control](Getting started/02-ClaudeCodeoverview.md#pretooluse-decision-control) for the full set of options.
...
#### PermissionRequest
Uses `hookSpecificOutput` to allow or deny a permission request on behalf of the user. When allowing, you can also modify the tool's input or apply permission rules so the user isn't prompted again. See [PermissionRequest decision control](Getting started/02-ClaudeCodeoverview.md#permissionrequest-decision-control) for the full set of options.
...
Runs when Claude Code starts a new session or resumes an existing session. Useful for loading development context like existing issues or recent changes to your codebase, or setting up environment variables. For static context that does not require a script, use [CLAUDE.md](Use Claude Code/03-HowClauderemembersyourproject.md) instead.
...
In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), SessionStart hooks receive `source`, `model`, and optionally `agent_type`. The `source` field indicates how the session started: `"startup"` for new sessions, `"resume"` for resumed sessions, `"clear"` after `/clear`, or `"compact"` after compaction. The `model` field contains the model identifier. If you start Claude Code with `claude --agent <name>`, an `agent_type` field contains the agent name.
...
Any text your hook script prints to stdout is added as context for Claude. In addition to the [JSON output fields](Getting started/02-ClaudeCodeoverview.md#json-output) available to all hooks, you can return these event-specific fields:
...
**Note:**
`CLAUDE_ENV_FILE` is available for SessionStart, [CwdChanged](Getting started/02-ClaudeCodeoverview.md#cwdchanged), and [FileChanged](Getting started/02-ClaudeCodeoverview.md#filechanged) hooks. Other hook types do not have access to this variable.
...
In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), InstructionsLoaded hooks receive these fields:
...
In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), UserPromptSubmit hooks receive the `prompt` field containing the text the user submitted.
...
`UserPromptSubmit` hooks can control whether a user prompt is processed and add context. All [JSON output fields](Getting started/02-ClaudeCodeoverview.md#json-output) are available.
...
Runs after Claude creates tool parameters and before processing the tool call. Matches on tool name: `Bash`, `Edit`, `Write`, `Read`, `Glob`, `Grep`, `Agent`, `WebFetch`, `WebSearch`, and any [MCP tool names](Getting started/02-ClaudeCodeoverview.md#match-mcp-tools).

Use [PreToolUse decision control](Getting started/02-ClaudeCodeoverview.md#pretooluse-decision-control) to allow, deny, or ask for permission to use the tool.

#### PreToolUse input

In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), PreToolUse hooks receive `tool_name`, `tool_input`, and `tool_use_id`. The `tool_input` fields depend on the tool:
...
Spawns a [subagent](48-Createcustomsubagents.md).
...
| Field                      | Description                                                                                                                                                                                                     |
| :------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `permissionDecision`       | `"allow"` skips the permission prompt. `"deny"` prevents the tool call. `"ask"` prompts the user to confirm. [Deny and ask rules](36-Configurepermissions.md#manage-permissions) still apply when a hook returns `"allow"` |
...
Use [PermissionRequest decision control](Getting started/02-ClaudeCodeoverview.md#permissionrequest-decision-control) to allow or deny on behalf of the user.
...
`PermissionRequest` hooks can allow or deny permission requests. In addition to the [JSON output fields](Getting started/02-ClaudeCodeoverview.md#json-output) available to all hooks, your hook script can return a `decision` object with these event-specific fields:
...
| `updatedPermissions` | For `"allow"` only: array of [permission update entries](Getting started/02-ClaudeCodeoverview.md#permission-update-entries) to apply, such as adding an allow rule or changing the session permission mode |
...
The `updatedPermissions` output field and the [`permission_suggestions` input field](Getting started/02-ClaudeCodeoverview.md#permissionrequest-input) both use the same array of entry objects. Each entry has a `type` that determines its other fields, and a `destination` that controls where the change is written.
...
`PostToolUse` hooks can provide feedback to Claude after tool execution. In addition to the [JSON output fields](Getting started/02-ClaudeCodeoverview.md#json-output) available to all hooks, your hook script can return these event-specific fields:
...
| `updatedMCPToolOutput` | For [MCP tools](Getting started/02-ClaudeCodeoverview.md#match-mcp-tools) only: replaces the tool's output with the provided value |
...
`PostToolUseFailure` hooks can provide context to Claude after a tool failure. In addition to the [JSON output fields](Getting started/02-ClaudeCodeoverview.md#json-output) available to all hooks, your hook script can return these event-specific fields:
...
In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), Notification hooks receive `message` with the notification text, an optional `title`, and `notification_type` indicating which type fired.
...
Notification hooks cannot block or modify notifications. In addition to the [JSON output fields](Getting started/02-ClaudeCodeoverview.md#json-output) available to all hooks, you can return `additionalContext` to add context to the conversation:
...
In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), SubagentStart hooks receive `agent_id` with the unique identifier for the subagent and `agent_type` with the agent name (built-in agents like `"Bash"`, `"Explore"`, `"Plan"`, or custom agent names).
...
SubagentStart hooks cannot block subagent creation, but they can inject context into the subagent. In addition to the [JSON output fields](Getting started/02-ClaudeCodeoverview.md#json-output) available to all hooks, you can return:
...
In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), SubagentStop hooks receive `stop_hook_active`, `agent_id`, `agent_type`, `agent_transcript_path`, and `last_assistant_message`. The `agent_type` field is the value used for matcher filtering. The `transcript_path` is the main session's transcript, while `agent_transcript_path` is the subagent's own transcript stored in a nested `subagents/` folder. The `last_assistant_message` field contains the text content of the subagent's final response, so hooks can access it without parsing the transcript file.
...
SubagentStop hooks use the same decision control format as [Stop hooks](Getting started/02-ClaudeCodeoverview.md#stop-decision-control).

### Stop

Runs when the main Claude Code agent has finished responding. Does not run if
the stoppage occurred due to a user interrupt. API errors fire
[StopFailure](Getting started/02-ClaudeCodeoverview.md#stopfailure) instead.

#### Stop input

In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), Stop hooks receive `stop_hook_active` and `last_assistant_message`. The `stop_hook_active` field is `true` when Claude Code is already continuing as a result of a stop hook. Check this value or process the transcript to prevent Claude Code from running indefinitely. The `last_assistant_message` field contains the text content of Claude's final response, so hooks can access it without parsing the transcript file.
...
`Stop` and `SubagentStop` hooks can control whether Claude continues. In addition to the [JSON output fields](Getting started/02-ClaudeCodeoverview.md#json-output) available to all hooks, your hook script can return these event-specific fields:
...
Runs instead of [Stop](Getting started/02-ClaudeCodeoverview.md#stop) when the turn ends due to an API error. Output and exit code are ignored. Use this to log failures, send alerts, or take recovery actions when Claude cannot complete a response due to rate limits, authentication problems, or other API errors.

#### StopFailure input

In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), StopFailure hooks receive `error`, optional `error_details`, and optional `last_assistant_message`. The `error` field identifies the error type and is used for matcher filtering.
...
Runs when an [agent team](01-OrchestrateteamsofClaudeCodesessions.md) teammate is about to go idle after finishing its turn. Use this to enforce quality gates before a teammate stops working, such as requiring passing lint checks or verifying that output files exist.
...
In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), TeammateIdle hooks receive `teammate_name` and `team_name`.
...
In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), TaskCompleted hooks receive `task_id`, `task_subject`, and optionally `task_description`, `teammate_name`, and `team_name`.
...
In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), ConfigChange hooks receive `source` and optionally `file_path`. The `source` field indicates which configuration type changed, and `file_path` provides the path to the specific file that was modified.
...
Runs when the working directory changes during a session, for example when Claude executes a `cd` command. Use this to react to directory changes: reload environment variables, activate project-specific toolchains, or run setup scripts automatically. Pairs with [FileChanged](Getting started/02-ClaudeCodeoverview.md#filechanged) for tools like [direnv](https://direnv.net/) that manage per-directory environment.

CwdChanged hooks have access to `CLAUDE_ENV_FILE`. Variables written to that file persist into subsequent Bash commands for the session, just as in [SessionStart hooks](Getting started/02-ClaudeCodeoverview.md#persist-environment-variables). Only `type: "command"` hooks are supported.

CwdChanged does not support matchers and fires on every directory change.

#### CwdChanged input

In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), CwdChanged hooks receive `old_cwd` and `new_cwd`.
...
In addition to the [JSON output fields](Getting started/02-ClaudeCodeoverview.md#json-output) available to all hooks, CwdChanged hooks can return `watchPaths` to dynamically set which file paths [FileChanged](Getting started/02-ClaudeCodeoverview.md#filechanged) watches:
...
FileChanged hooks have access to `CLAUDE_ENV_FILE`. Variables written to that file persist into subsequent Bash commands for the session, just as in [SessionStart hooks](Getting started/02-ClaudeCodeoverview.md#persist-environment-variables). Only `type: "command"` hooks are supported.

#### FileChanged input

In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), FileChanged hooks receive `file_path` and `event`.
...
In addition to the [JSON output fields](Getting started/02-ClaudeCodeoverview.md#json-output) available to all hooks, FileChanged hooks can return `watchPaths` to dynamically update which file paths are watched:
...
When you run `claude --worktree` or a [subagent uses `isolation: "worktree"`](48-Createcustomsubagents.md#choose-the-subagent-scope), Claude Code creates an isolated working copy using `git worktree`. If you configure a WorktreeCreate hook, it replaces the default git behavior, letting you use a different version control system like SVN, Perforce, or Mercurial.
...
In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), WorktreeCreate hooks receive the `name` field. This is a slug identifier for the new worktree, either specified by the user or auto-generated (for example, `bold-oak-a3f2`).
...
The cleanup counterpart to [WorktreeCreate](Getting started/02-ClaudeCodeoverview.md#worktreecreate). This hook fires when a worktree is being removed, either when you exit a `--worktree` session and choose to remove it, or when a subagent with `isolation: "worktree"` finishes. For git-based worktrees, Claude handles cleanup automatically with `git worktree remove`. If you configured a WorktreeCreate hook for a non-git version control system, pair it with a WorktreeRemove hook to handle cleanup. Without one, the worktree directory is left on disk.
...
In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), WorktreeRemove hooks receive the `worktree_path` field, which is the absolute path to the worktree being removed.
...
In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), PreCompact hooks receive `trigger` and `custom_instructions`. For `manual`, `custom_instructions` contains what the user passes into `/compact`. For `auto`, `custom_instructions` is empty.
...
In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), PostCompact hooks receive `trigger` and `compact_summary`. The `compact_summary` field contains the conversation summary generated by the compact operation.
...
In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), SessionEnd hooks receive a `reason` field indicating why the session ended. See the [reason table](Getting started/02-ClaudeCodeoverview.md#sessionend) above for all values.
...
In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), Elicitation hooks receive `mcp_server_name`, `message`, and optional `mode`, `url`, `elicitation_id`, and `requested_schema` fields.
...
In addition to the [common input fields](Getting started/02-ClaudeCodeoverview.md#common-input-fields), ElicitationResult hooks receive `mcp_server_name`, `action`, and optional `mode`, `elicitation_id`, and `content` fields.
...
This `Stop` hook uses a detailed prompt to check three conditions before allowing Claude to stop. If `"ok"` is `false`, Claude continues working with the provided reason as its next instruction. `SubagentStop` hooks use the same format to evaluate whether a [subagent](48-Createcustomsubagents.md) should stop:
...
Set `type` to `"agent"` and provide a `prompt` string. The configuration fields are the same as [prompt hooks](Getting started/02-ClaudeCodeoverview.md#prompt-hook-configuration), with a longer default timeout:
...
For troubleshooting common issues like hooks not firing, infinite Stop hook loops, or configuration errors, see [Limitations and troubleshooting](25-Automateworkflowswithhooks.md#limitations-and-troubleshooting) in the guide.
