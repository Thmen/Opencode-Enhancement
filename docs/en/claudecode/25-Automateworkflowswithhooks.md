[Home](README.md) / Automate workflows with hooks
...
For decisions that require judgment rather than deterministic rules, you can also use [prompt-based hooks](Getting started/02-ClaudeCodeoverview.md#prompt-based-hooks) or [agent-based hooks](Getting started/02-ClaudeCodeoverview.md#agent-based-hooks) that use a Claude model to evaluate conditions.
...
Hooks let you run code at key points in Claude Code's lifecycle: format files after edits, block commands before they execute, send notifications when Claude needs input, inject context at session start, and more. For the full list of hook events, see the [Hooks reference](24-Hooksreference.md#hook-lifecycle).

Each example includes a ready-to-use configuration block that you add to a [settings file](Getting started/02-ClaudeCodeoverview.md#configure-hook-location). The most common patterns:

* [Get notified when Claude needs input](Getting started/02-ClaudeCodeoverview.md#get-notified-when-claude-needs-input)
* [Auto-format code after edits](Getting started/02-ClaudeCodeoverview.md#auto-format-code-after-edits)
...
You can replace the `echo` with any command that produces dynamic output, like `git log --oneline -5` to show recent commits. For injecting context on every session start, consider using [CLAUDE.md](Use Claude Code/03-HowClauderemembersyourproject.md) instead. For environment variables, see [`CLAUDE_ENV_FILE`](24-Hooksreference.md#persist-environment-variables) in the reference.
...
Keep the matcher as narrow as possible. Matching on `.*` or leaving the matcher empty would auto-approve every permission prompt, including file writes and shell commands. See the [PermissionRequest reference](24-Hooksreference.md#permissionrequest-decision-control) for the full set of decision fields.
...
* `"type": "http"`: POST event data to a URL. See [HTTP hooks](Getting started/02-ClaudeCodeoverview.md#http-hooks).
* `"type": "prompt"`: single-turn LLM evaluation. See [Prompt-based hooks](Getting started/02-ClaudeCodeoverview.md#prompt-based-hooks).
* `"type": "agent"`: multi-turn verification with tool access. See [Agent-based hooks](Getting started/02-ClaudeCodeoverview.md#agent-based-hooks).
...
Your script can parse that JSON and act on any of those fields. `UserPromptSubmit` hooks get the `prompt` text instead, `SessionStart` hooks get the `source` (startup, resume, clear, compact), and so on. See [Common input fields](24-Hooksreference.md#common-input-fields) in the reference for shared fields, and each event's section for event-specific schemas.
...
Returning `"allow"` skips the interactive prompt but does not override [permission rules](36-Configurepermissions.md#manage-permissions). If a deny rule matches the tool call, the call is blocked even when your hook returns `"allow"`. If an ask rule matches, the user is still prompted. This means deny rules from any settings scope, including [managed settings](44-ClaudeCodesettings.md#settings-files), always take precedence over hook approvals.

Other events use different decision patterns. For example, `PostToolUse` and `Stop` hooks use a top-level `decision: "block"` field, while `PermissionRequest` uses `hookSpecificOutput.decision.behavior`. See the [summary table](24-Hooksreference.md#decision-control) in the reference for a full breakdown by event.

For `UserPromptSubmit` hooks, use `additionalContext` instead to inject text into Claude's context. Prompt-based hooks (`type: "prompt"`) handle output differently: see [Prompt-based hooks](Getting started/02-ClaudeCodeoverview.md#prompt-based-hooks).
...
| [Skill](46-ExtendClaudewithskills.md) or [agent](48-Createcustomsubagents.md) frontmatter | While the skill or agent is active | Yes, defined in the component file |

Run [`/hooks`](24-Hooksreference.md#the-hooks-menu) in Claude Code to browse all configured hooks grouped by event. To disable all hooks at once, set `"disableAllHooks": true` in your settings file.
...
For full configuration options, see [Prompt-based hooks](24-Hooksreference.md#prompt-based-hooks) in the reference.
...
For full configuration options, see [Agent-based hooks](24-Hooksreference.md#agent-based-hooks) in the reference.
...
The endpoint should return a JSON response body using the same [output format](24-Hooksreference.md#json-output) as command hooks. To block a tool call, return a 2xx response with the appropriate `hookSpecificOutput` fields. HTTP status codes alone cannot block actions.
...
For full configuration options and response handling, see [HTTP hooks](24-Hooksreference.md#http-hook-fields) in the reference.
...
* `PermissionRequest` hooks do not fire in [non-interactive mode](23-RunClaudeCodeprogrammatically.md) (`-p`). Use `PreToolUse` hooks for automated permission decisions.
* `Stop` hooks fire whenever Claude finishes responding, not only at task completion. They do not fire on user interrupts. API errors fire [StopFailure](24-Hooksreference.md#stopfailure) instead.
...
* [Hooks reference](24-Hooksreference.md): full event schemas, JSON output format, async hooks, and MCP tool hooks
* [Security considerations](24-Hooksreference.md#security-considerations): review before deploying hooks in shared or production environments
