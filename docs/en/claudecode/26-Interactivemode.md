[Home](README.md) / Interactive mode
...
| `Alt+Y` (after `Ctrl+Y`) | Cycle paste history          | After pasting, cycle through previously deleted text. Requires [Option as Meta](Getting started/02-ClaudeCodeoverview.md#keyboard-shortcuts) on macOS |
| `Alt+B`                  | Move cursor back one word    | Word navigation. Requires [Option as Meta](Getting started/02-ClaudeCodeoverview.md#keyboard-shortcuts) on macOS                                      |
| `Alt+F`                  | Move cursor forward one word | Word navigation. Requires [Option as Meta](Getting started/02-ClaudeCodeoverview.md#keyboard-shortcuts) on macOS                                      |
...
| Shortcut     | Description            | Notes                                                                                                                                                      |
| :----------- | :--------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Hold `Space` | Push-to-talk dictation | Requires [voice dictation](53-Voicedictation.md) to be enabled. Transcript inserts at cursor. [Rebindable](53-Voicedictation.md#rebind-the-push-to-talk-key) |

## Built-in commands

Type `/` in Claude Code to see all available commands, or type `/` followed by any letters to filter. The `/` menu shows both built-in commands and [bundled skills](46-ExtendClaudewithskills.md#bundled-skills) like `/simplify`. Not all commands are visible to every user since some depend on your platform or plan.

See the [commands reference](11-Built-incommands.md) for the full list of built-in commands. To create your own commands, see [skills](46-ExtendClaudewithskills.md).
...
To disable all background task functionality, set the `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` environment variable to `1`. See [Environment variables](18-Environmentvariables.md) for details.
...
`/btw` is the inverse of a [subagent](48-Createcustomsubagents.md): it sees your full conversation but has no tools, while a subagent has full tools but starts with an empty context. Use `/btw` to ask about what Claude already knows from this session; use a subagent to go find out something new.
...
* [Skills](46-ExtendClaudewithskills.md) - Custom prompts and workflows
* [Checkpointing](07-Checkpointing.md) - Rewind Claude's edits and restore previous states
* [CLI reference](09-CLIreference.md) - Command-line flags and options
* [Settings](44-ClaudeCodesettings.md) - Configuration options
* [Memory management](Use Claude Code/03-HowClauderemembersyourproject.md) - Managing CLAUDE.md files
