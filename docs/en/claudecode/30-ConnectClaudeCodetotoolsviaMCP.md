[Home](README.md) / Connect Claude Code to tools via MCP
...
* **React to external events**: An MCP server can also act as a [channel](05-Pusheventsintoarunningsessionwithchannels.md) that pushes messages into your session, so Claude reacts to Telegram messages, Discord chats, or webhook events while you're away.
...
[Plugins](38-Createplugins.md) can bundle MCP servers, automatically providing tools and integrations when the plugin is enabled. Plugin MCP servers work identically to user-configured servers.
...
* **Environment variables**: use `${CLAUDE_PLUGIN_ROOT}` for bundled plugin files and `${CLAUDE_PLUGIN_DATA}` for [persistent state](39-Pluginsreference.md#persistent-data-directory) that survives plugin updates
...
See the [plugin components reference](39-Pluginsreference.md#mcp-servers) for details on bundling MCP servers with plugins.
...
<Note>
  The term "local scope" for MCP servers differs from general local settings. MCP local-scoped servers are stored in `~/.claude.json` (your home directory), while general local settings use `.claude/settings.local.json` (in the project directory). See [Settings](44-ClaudeCodesettings.md#settings-files) for details on settings file locations.
</Note>
...
<Note>
  ...
  * **Managed**: `managed-mcp.json` in system directories (see [Managed MCP configuration](Getting started/02-ClaudeCodeoverview.md#managed-mcp-configuration))
</Note>
...
To auto-respond to elicitation requests without showing a dialog, use the [`Elicitation` hook](24-Hooksreference.md#elicitation).
...
Claude Code automatically enables Tool Search when your MCP tool descriptions would consume more than 10% of the context window. You can [adjust this threshold](Getting started/02-ClaudeCodeoverview.md#configure-tool-search) or disable tool search entirely. When triggered:
...
If you're building an MCP server, the server instructions field becomes more useful with Tool Search enabled. Server instructions help Claude understand when to search for your tools, similar to how [skills](46-ExtendClaudewithskills.md) work.
...
Or set the value in your [settings.json `env` field](44-ClaudeCodesettings.md#available-settings).
...
Instead of taking exclusive control, administrators can allow users to configure their own MCP servers while enforcing restrictions on which servers are permitted. This approach uses `allowedMcpServers` and `deniedMcpServers` in the [managed settings file](44-ClaudeCodesettings.md#settings-files).
