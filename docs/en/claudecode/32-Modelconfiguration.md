[Home](README.md) / Model configuration
...
Enterprise administrators can use `availableModels` in [managed or policy settings](44-ClaudeCodesettings.md#settings-files) to restrict which models users can select.
...
The Default option in the model picker is not affected by `availableModels`. It always remains available and represents the system's runtime default [based on the user's subscription tier](Getting started/02-ClaudeCodeoverview.md#default-model-setting).
...
* **Skill and subagent frontmatter**: set `effort` in a [skill](46-ExtendClaudewithskills.md#frontmatter-reference) or [subagent](48-Createcustomsubagents.md#supported-frontmatter-fields) markdown file to override the effort level when that skill or subagent runs
...
To disable adaptive reasoning on Opus 4.6 and Sonnet 4.6 and revert to the previous fixed thinking budget, set `CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING=1`. When disabled, these models use the fixed budget controlled by `MAX_THINKING_TOKENS`. See [environment variables](18-Environmentvariables.md).
...
To disable 1M context entirely, set `CLAUDE_CODE_DISABLE_1M_CONTEXT=1`. This removes 1M model variants from the model picker. See [environment variables](18-Environmentvariables.md).
...
1. In [status line](47-Customizeyourstatusline.md) (if configured)
...
| `ANTHROPIC_DEFAULT_HAIKU_MODEL`  | The model to use for `haiku`, or [background functionality](12-Managecostseffectively.md#background-token-usage) |
| `CLAUDE_CODE_SUBAGENT_MODEL`     | The model to use for [subagents](48-Createcustomsubagents.md)                                              |
...
When deploying Claude Code through [Bedrock](02-ClaudeCodeonAmazonBedrock.md), [Vertex AI](22-ClaudeCodeonGoogleVertexAI.md), or [Foundry](31-ClaudeCodeonMicrosoftFoundry.md), pin model versions before rolling out to users.
...
To enable [extended context](Getting started/02-ClaudeCodeoverview.md#extended-context) for a pinned model, append `[1m]` to the model ID in `ANTHROPIC_DEFAULT_OPUS_MODEL` or `ANTHROPIC_DEFAULT_SONNET_MODEL`:
...
Claude Code enables features like [effort levels](Getting started/02-ClaudeCodeoverview.md#adjust-effort-level) and [extended thinking](Use Claude Code/02-Commonworkflows.md#use-extended-thinking-thinking-mode) by matching the model ID against known patterns. Provider-specific IDs such as Bedrock ARNs or custom deployment names often don't match these patterns, leaving supported features disabled. Set `_SUPPORTED_CAPABILITIES` to tell Claude Code which features the model actually supports:

| Capability value       | Enables                                                                         |
| ---------------------- | ------------------------------------------------------------------------------- |
| `effort`               | [Effort levels](Getting started/02-ClaudeCodeoverview.md#adjust-effort-level) and the `/effort` command                 |
| `max_effort`           | The `max` effort level                                                          |
| `thinking`             | [Extended thinking](Use Claude Code/02-Commonworkflows.md#use-extended-thinking-thinking-mode)   |
...
Set `modelOverrides` in your [settings file](44-ClaudeCodesettings.md#settings-files):
