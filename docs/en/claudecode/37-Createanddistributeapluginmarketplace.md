[Home](README.md) / Create and distribute a plugin marketplace
...
Looking to install plugins from an existing marketplace? See [Discover and install prebuilt plugins](17-Discoverandinstallprebuiltpluginsthroughmarketplaces.md).

## Overview

Creating and distributing a marketplace involves:

1. **Creating plugins**: build one or more plugins with commands, agents, hooks, MCP servers, or LSP servers. This guide assumes you already have plugins to distribute; see [Create plugins](38-Createplugins.md) for details on how to create them.
2. **Creating a marketplace file**: define a `marketplace.json` that lists your plugins and where to find them (see [Create the marketplace file](Getting started/02-ClaudeCodeoverview.md#create-the-marketplace-file)).
3. **Host the marketplace**: push to GitHub, GitLab, or another git host (see [Host and distribute marketplaces](Getting started/02-ClaudeCodeoverview.md#host-and-distribute-marketplaces)).
4. **Share with users**: users add your marketplace with `/plugin marketplace add` and install individual plugins (see [Discover and install plugins](17-Discoverandinstallprebuiltpluginsthroughmarketplaces.md)).
...
To learn more about what plugins can do, including hooks, agents, MCP servers, and LSP servers, see [Plugins](38-Createplugins.md).
...
If you need to share files across plugins, use symlinks (which are followed during copying). See [Plugin caching and file resolution](39-Pluginsreference.md#plugin-caching-and-file-resolution) for details.
...
Each plugin entry needs at minimum a `name` and `source` (where to fetch it from). See the [full schema](Getting started/02-ClaudeCodeoverview.md#marketplace-schema) below for all available fields.
...
| `owner`   | object | Marketplace maintainer information ([see fields below](Getting started/02-ClaudeCodeoverview.md#owner-fields))                                                                                                 |                |
...
Each plugin entry in the `plugins` array describes a plugin and where to find it. You can include any field from the [plugin manifest schema](39-Pluginsreference.md#plugin-manifest-schema) (like `description`, `version`, `author`, `commands`, `hooks`, etc.), plus these marketplace-specific fields: `source`, `category`, `tags`, and `strict`.
...
| `source` | string\|object | Where to fetch the plugin from (see [Plugin sources](Getting started/02-ClaudeCodeoverview.md#plugin-sources) below)                                                                           |
...
| `strict`      | boolean | Controls whether `plugin.json` is the authority for component definitions (default: true). See [Strict mode](Getting started/02-ClaudeCodeoverview.md#strict-mode) below. |
...
For the full range of add commands (GitHub, Git URLs, local paths, remote URLs), see [Add marketplaces](17-Discoverandinstallprebuiltpluginsthroughmarketplaces.md#add-marketplaces).
...
For full configuration options, see [Plugin settings](44-ClaudeCodesettings.md#plugin-settings).
...
For organizations requiring strict control over plugin sources, administrators can restrict which plugin marketplaces users are allowed to add using the [`strictKnownMarketplaces`](44-ClaudeCodesettings.md#strictknownmarketplaces) setting in managed settings.
...
**Note:**
`strictKnownMarketplaces` restricts what users can add, but does not register marketplaces on its own. To make allowed marketplaces available automatically without users running `/plugin marketplace add`, pair it with [`extraKnownMarketplaces`](44-ClaudeCodesettings.md#extraknownmarketplaces) in the same `managed-settings.json`. See [Using both together](44-ClaudeCodesettings.md#strictknownmarketplaces).
...
Because `strictKnownMarketplaces` is set in [managed settings](44-ClaudeCodesettings.md#settings-files), individual users and project configurations cannot override these restrictions.

For complete configuration details including all supported source types and comparison with `extraKnownMarketplaces`, see the [strictKnownMarketplaces reference](44-ClaudeCodesettings.md#strictknownmarketplaces).
...
To support "stable" and "latest" release channels for your plugins, you can set up two marketplaces that point to different refs or SHAs of the same repo. You can then assign the two marketplaces to different user groups through [managed settings](44-ClaudeCodesettings.md#settings-files).
...
For complete plugin testing workflows, see [Test your plugins locally](38-Createplugins.md#test-your-plugins-locally). For technical troubleshooting, see [Plugins reference](39-Pluginsreference.md).
...
| `plugins[0].source: Path contains ".."`           | Source path contains `..`                       | Use paths relative to the marketplace root without `..`. See [Relative paths](Getting started/02-ClaudeCodeoverview.md#relative-paths) |
...
**Solutions**: See [Plugin caching and file resolution](39-Pluginsreference.md#plugin-caching-and-file-resolution) for workarounds including symlinks and directory restructuring.

For additional debugging tools and common issues, see [Debugging and development tools](39-Pluginsreference.md#debugging-and-development-tools).

## See also

* [Discover and install prebuilt plugins](17-Discoverandinstallprebuiltpluginsthroughmarketplaces.md) - Installing plugins from existing marketplaces
* [Plugins](38-Createplugins.md) - Creating your own plugins
* [Plugins reference](39-Pluginsreference.md) - Complete technical specifications and schemas
* [Plugin settings](44-ClaudeCodesettings.md#plugin-settings) - Plugin configuration options
* [strictKnownMarketplaces reference](44-ClaudeCodesettings.md#strictknownmarketplaces) - Managed marketplace restrictions
