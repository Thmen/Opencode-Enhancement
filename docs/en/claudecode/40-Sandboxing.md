...
* **User confirmation**: New domain requests trigger permission prompts (unless [`allowManagedDomainsOnly`](44-ClaudeCodesettings.md#sandbox-settings) is enabled, which blocks non-allowed domains automatically)
...
By default, if the sandbox cannot start (missing dependencies, unsupported platform, or platform restrictions), Claude Code shows a warning and runs commands without sandboxing. To make this a hard failure instead, set [`sandbox.failIfUnavailable`](44-ClaudeCodesettings.md#sandbox-settings) to `true`. This is intended for managed deployments that require sandboxing as a security gate.
...
Customize sandbox behavior through your `settings.json` file. See [Settings](44-ClaudeCodesettings.md#sandbox-settings) for complete configuration reference.
...
You can disable this escape hatch by setting `"allowUnsandboxedCommands": false` in your [sandbox settings](44-ClaudeCodesettings.md#sandbox-settings). When disabled, the `dangerouslyDisableSandbox` parameter is completely ignored and all commands must run sandboxed or be explicitly listed in `excludedCommands`.
...
* Cannot read files that are denied in your [Claude permission settings](36-Configurepermissions.md#manage-permissions)
...
Sandboxing and [permissions](36-Configurepermissions.md) are complementary security layers that work together:
...
* **Permission rules**: Combine with [permission settings](36-Configurepermissions.md) for defense-in-depth
* **Development containers**: Use with [devcontainers](16-Developmentcontainers.md) for additional isolation
* **Enterprise policies**: Enforce sandbox configurations through [managed settings](44-ClaudeCodesettings.md#settings-precedence)
...
* **Built-in file tools**: Read, Edit, and Write use the permission system directly rather than running through the sandbox. See [permissions](36-Configurepermissions.md).
* **Computer use on Desktop**: when Claude opens apps and controls your screen on macOS, it runs on your actual desktop rather than in an isolated environment. Per-app permission prompts gate each application. See [computer use](14-UseClaudeCodeDesktop.md#let-claude-use-your-computer).

## See also

* [Security](42-Security.md) - Comprehensive security features and best practices
* [Permissions](36-Configurepermissions.md) - Permission configuration and access control
* [Settings](44-ClaudeCodesettings.md) - Complete configuration reference
* [CLI reference](09-CLIreference.md) - Command-line options
