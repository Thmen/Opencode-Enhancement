[Home](README.md) / Configure server-managed settings (public beta)
...
Claude Code supports two approaches for centralized configuration. Server-managed settings deliver configuration from Anthropic's servers. [Endpoint-managed settings](44-ClaudeCodesettings.md#settings-files) are deployed directly to devices through native OS policies (macOS managed preferences, Windows registry) or managed settings files.
...
| **[Endpoint-managed settings](44-ClaudeCodesettings.md#settings-files)** | Organizations with MDM or endpoint management            | Settings deployed to devices via MDM configuration profiles, registry policies, or managed settings files |
...
To confirm that settings are being applied, ask a user to restart Claude Code. If the configuration includes settings that trigger the [security approval dialog](Getting started/02-ClaudeCodeoverview.md#security-approval-dialogs), the user sees a prompt describing the managed settings on startup. You can also verify that managed permission rules are active by having a user run `/permissions` to view their effective permission rules.
...
* [MCP server configurations](30-ConnectClaudeCodetotoolsviaMCP.md#managed-mcp-configuration) cannot be distributed through server-managed settings.

## Settings delivery

### Settings precedence

Server-managed settings and [endpoint-managed settings](44-ClaudeCodesettings.md#settings-files) both occupy the highest tier in the Claude Code [settings hierarchy](44-ClaudeCodesettings.md#settings-precedence). No other settings level can override them, including command line arguments. When both are present, server-managed settings take precedence and endpoint-managed settings are not used.
...
* Google Vertex AI
* Microsoft Foundry
* Custom API endpoints via `ANTHROPIC_BASE_URL` or [LLM gateways](29-LLMgatewayconfiguration.md)
...
To detect runtime configuration changes, use [`ConfigChange` hooks](24-Hooksreference.md#configchange) to log modifications or block unauthorized changes before they take effect.

For stronger enforcement guarantees, use [endpoint-managed settings](44-ClaudeCodesettings.md#settings-files) on devices enrolled in an MDM solution.

## See also

Related pages for managing Claude Code configuration:

* [Settings](44-ClaudeCodesettings.md): complete configuration reference including all available settings
* [Endpoint-managed settings](44-ClaudeCodesettings.md#settings-files): managed settings deployed to devices by IT
