[Home](README.md) / Advanced setup

# Advanced setup

> System requirements, platform-specific installation, version management, and uninstallation for Claude Code.

This page covers system requirements, platform-specific installation details, updates, and uninstallation. For a guided walkthrough of your first session, see the [quickstart](Getting started/03-Quickstart.md). If you've never used a terminal before, see the [terminal guide](/en/terminal-guide).
...
* **Operating system**:
  ...
  * Alpine Linux 3.19+
* **Hardware**: 4 GB+ RAM
* **Network**: internet connection required. See [network configuration](34-Enterprisenetworkconfiguration.md#network-access-requirements).
...
* **ripgrep**: usually included with Claude Code. If search fails, see [search troubleshooting](52-Troubleshooting.md#search-and-discovery-issues).

## Install Claude Code

**Tip:**
Prefer a graphical interface? The [Desktop app](15-Getstartedwiththedesktopapp.md) lets you use Claude Code without the terminal. Download it for [macOS](https://claude.ai/api/desktop/darwin/universal/dmg/latest/redirect?utm_source=claude_code\&utm_medium=docs) or [Windows](https://claude.ai/api/desktop/win32/x64/exe/latest/redirect?utm_source=claude_code\&utm_medium=docs).
...
If you encounter any issues during installation, see the [troubleshooting guide](52-Troubleshooting.md).
...
If Claude Code can't find your Git Bash installation, set the path in your [settings.json file](44-ClaudeCodesettings.md):
...
Claude Code can also run PowerShell natively on Windows as an opt-in preview. See [PowerShell tool](51-Toolsreference.md#powershell-tool) for setup and limitations.

**Option 2: WSL**

Both WSL 1 and WSL 2 are supported. WSL 2 supports [sandboxing](40-Sandboxing.md) for enhanced security. WSL 1 does not support sandboxing.
...
Then set `USE_BUILTIN_RIPGREP` to `0` in your [`settings.json`](44-ClaudeCodesettings.md#available-settings) file:
...
For a more detailed check of your installation and configuration, run [`claude doctor`](52-Troubleshooting.md#get-more-help):
...
Claude Code requires a Pro, Max, Teams, Enterprise, or Console account. The free Claude.ai plan does not include Claude Code access. You can also use Claude Code with a third-party API provider like [Amazon Bedrock](02-ClaudeCodeonAmazonBedrock.md), [Google Vertex AI](22-ClaudeCodeonGoogleVertexAI.md), or [Microsoft Foundry](31-ClaudeCodeonMicrosoftFoundry.md).

After installing, log in by running `claude` and following the browser prompts. See [Authentication](04-Authentication.md) for all account types and team setup options.

## Update Claude Code

Native installations automatically update in the background. You can [configure the release channel](Getting started/02-ClaudeCodeoverview.md#configure-release-channel) to control whether you receive updates immediately or on a delayed stable schedule, or [disable auto-updates](Getting started/02-ClaudeCodeoverview.md#disable-auto-updates) entirely. Homebrew and WinGet installations require manual updates.
...
Configure this via `/config` → **Auto-update channel**, or add it to your [settings.json file](44-ClaudeCodesettings.md):
...
For enterprise deployments, you can enforce a consistent release channel across your organization using [managed settings](36-Configurepermissions.md#managed-settings).

### Disable auto-updates

Set `DISABLE_AUTOUPDATER` to `"1"` in the `env` key of your [`settings.json`](44-ClaudeCodesettings.md#available-settings) file:
...
The native installer accepts either a specific version number or a release channel (`latest` or `stable`). The channel you choose at install time becomes your default for auto-updates. See [configure release channel](Getting started/02-ClaudeCodeoverview.md#configure-release-channel) for more information.
...
npm installation is deprecated. The native installer is faster, requires no dependencies, and auto-updates in the background. Use the [native installation](Getting started/02-ClaudeCodeoverview.md#install-claude-code) method when possible.
...
**Warning:**
Do NOT use `sudo npm install -g` as this can lead to permission issues and security risks. If you encounter permission errors, see [troubleshooting permission errors](52-Troubleshooting.md#permission-errors-during-installation).
