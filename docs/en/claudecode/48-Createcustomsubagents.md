[Home](README.md) / Create custom subagents
...
**Note:**
If you need multiple agents working in parallel and communicating with each other, see [agent teams](01-OrchestrateteamsofClaudeCodesessions.md) instead. Subagents work within a single session; agent teams coordinate across separate sessions.
...
Claude Code includes several built-in subagents like **Explore**, **Plan**, and **general-purpose**. You can also create custom subagents to handle specific tasks. This page covers the [built-in subagents](Getting started/02-ClaudeCodeoverview.md#built-in-subagents), [how to create your own](Getting started/02-ClaudeCodeoverview.md#quickstart-create-your-first-subagent), [full configuration options](Getting started/02-ClaudeCodeoverview.md#configure-subagents), [patterns for working with subagents](Getting started/02-ClaudeCodeoverview.md#work-with-subagents), and [example subagents](Getting started/02-ClaudeCodeoverview.md#example-subagents).
...
#### Plan
A research agent used during [plan mode](Use Claude Code/02-Commonworkflows.md#use-plan-mode-for-safe-code-analysis) to gather context before presenting a plan.
...
Subagents are defined in Markdown files with YAML frontmatter. You can [create them manually](Getting started/02-ClaudeCodeoverview.md#write-subagent-files) or use the `/agents` command.
...
| Plugin's `agents/` directory | Where plugin is enabled | 4 (lowest)  | Installed with [plugins](38-Createplugins.md) |
...
The `--agents` flag accepts JSON with the same [frontmatter](Getting started/02-ClaudeCodeoverview.md#supported-frontmatter-fields) fields as file-based subagents: `description`, `prompt`, `tools`, `disallowedTools`, `model`, `permissionMode`, `mcpServers`, `hooks`, `maxTurns`, `skills`, `initialPrompt`, `memory`, `effort`, `background`, and `isolation`. Use `prompt` for the system prompt, equivalent to the markdown body in file-based subagents.

**Plugin subagents** come from [plugins](38-Createplugins.md) you've installed. They appear in `/agents` alongside your custom subagents. See the [plugin components reference](39-Pluginsreference.md#agents) for details on creating plugin subagents.

**Note:**
For security reasons, plugin subagents do not support the `hooks`, `mcpServers`, or `permissionMode` frontmatter fields. These fields are ignored when loading agents from a plugin. If you need them, copy the agent file into `.claude/agents/` or `~/.claude/agents/`. You can also add rules to [`permissions.allow`](44-ClaudeCodesettings.md#permission-settings) in `settings.json` or `settings.local.json`, but these rules apply to the entire session, not just the plugin subagent.
...
| `tools`           | No       | [Tools](Getting started/02-ClaudeCodeoverview.md#available-tools) the subagent can use. Inherits all tools if omitted                                                                                                                                                                                               |
| `disallowedTools` | No       | Tools to deny, removed from inherited or specified list                                                                                                                                                                                                                     |
| `model`           | No       | [Model](Getting started/02-ClaudeCodeoverview.md#choose-a-model) to use: `sonnet`, `opus`, `haiku`, a full model ID (for example, `claude-opus-4-6`), or `inherit`. Defaults to `inherit`                                                                                                                           |
| `permissionMode`  | No       | [Permission mode](Getting started/02-ClaudeCodeoverview.md#permission-modes): `default`, `acceptEdits`, `dontAsk`, `bypassPermissions`, or `plan`                                                                                                                                                                   |
| `maxTurns`        | No       | Maximum number of agentic turns before the subagent stops                                                                                                                                                                                                                   |
| `skills`          | No       | [Skills](46-ExtendClaudewithskills.md) to load into the subagent's context at startup. The full skill content is injected, not just made available for invocation. Subagents don't inherit skills from the parent conversation                                                                |
| `mcpServers`      | No       | [MCP servers](30-ConnectClaudeCodetotoolsviaMCP.md) available to this subagent. Each entry is either a server name referencing an already-configured server (e.g., `"slack"`) or an inline definition with the server name as key and a full [MCP server config](30-ConnectClaudeCodetotoolsviaMCP.md#configure-mcp-servers) as value |
| `hooks`           | No       | [Lifecycle hooks](Getting started/02-ClaudeCodeoverview.md#define-hooks-for-subagents) scoped to this subagent                                                                                                                                                                                                      |
| `memory`          | No       | [Persistent memory scope](Getting started/02-ClaudeCodeoverview.md#enable-persistent-memory): `user`, `project`, or `local`. Enables cross-session learning                                                                                                                                                         |
| `background`      | No       | Set to `true` to always run this subagent as a [background task](Getting started/02-ClaudeCodeoverview.md#run-subagents-in-foreground-or-background). Default: `false`                                                                                                                                              |
...
| `isolation`       | No       | Set to `worktree` to run the subagent in a temporary [git worktree](Use Claude Code/02-Commonworkflows.md#run-parallel-claude-code-sessions-with-git-worktrees), giving it an isolated copy of the repository. The worktree is automatically cleaned up if the subagent makes no changes     |
| `initialPrompt`   | No       | Auto-submitted as the first user turn when this agent runs as the main session agent (via `--agent` or the `agent` setting). [Commands](11-Built-incommands.md) and [skills](46-ExtendClaudewithskills.md) are processed. Prepended to any user-provided prompt                                         |

### Choose a model

The `model` field controls which [AI model](32-Modelconfiguration.md) the subagent uses:
...
1. The [`CLAUDE_CODE_SUBAGENT_MODEL`](32-Modelconfiguration.md#environment-variables) environment variable, if set
...
Subagents can use any of Claude Code's [internal tools](51-Toolsreference.md). By default, subagents inherit all tools from the main conversation, including MCP tools.
...
This is an allowlist: only the `worker` and `researcher` subagents can be spawned. If the agent tries to spawn any other type, the request fails and the agent sees only the allowed types in its prompt. To block specific agents while allowing all others, use [`permissions.deny`](Getting started/02-ClaudeCodeoverview.md#disable-specific-subagents) instead.
...
Use the `mcpServers` field to give a subagent access to [MCP](30-ConnectClaudeCodetotoolsviaMCP.md) servers that aren't available in the main conversation. Inline servers defined here are connected when the subagent starts and disconnected when it finishes. String references share the parent session's connection.
...
Claude Code [passes hook input as JSON](24-Hooksreference.md#pretooluse-input) via stdin to hook commands. The validation script reads this JSON, extracts the Bash command, and [exits with code 2](24-Hooksreference.md#exit-code-2-behavior-per-event) to block write operations:
...
See [Hook input](24-Hooksreference.md#pretooluse-input) for the complete input schema and [exit codes](24-Hooksreference.md#exit-code-output) for how exit codes affect behavior.

#### Disable specific subagents

You can prevent Claude from using specific subagents by adding them to the `deny` array in your [settings](44-ClaudeCodesettings.md#permission-settings). Use the format `Agent(subagent-name)` where `subagent-name` matches the subagent's name field.
...
See [Permissions documentation](36-Configurepermissions.md#tool-specific-permission-rules) for more details on permission rules.

### Define hooks for subagents

Subagents can define [hooks](24-Hooksreference.md) that run during the subagent's lifecycle. There are two ways to configure hooks:
...
All [hook events](24-Hooksreference.md#hook-events) are supported. The most common events for subagents are:
...
See [Hooks](24-Hooksreference.md) for the complete hook configuration format.
...
Subagents provided by an enabled [plugin](38-Createplugins.md) appear in the typeahead as `<plugin-name>:<agent-name>`. You can also type the mention manually without using the picker: `@agent-<name>` for local subagents, or `@agent-<plugin-name>:<agent-name>` for plugin subagents.

**Run the whole session as a subagent.** Pass [`--agent <name>`](09-CLIreference.md) to start a session where the main thread itself takes on that subagent's system prompt, tool restrictions, and model:

```bash
claude --agent code-reviewer
```

The subagent's system prompt replaces the default Claude Code system prompt entirely, the same way [`--system-prompt`](09-CLIreference.md) does. `CLAUDE.md` files and project memory still load through the normal message flow. The agent name appears as `@<name>` in the startup header so you can confirm it's active.
...
* **Foreground subagents** block the main conversation until complete. Permission prompts and clarifying questions (like [`AskUserQuestion`](51-Toolsreference.md)) are passed through to you.
...
To disable all background task functionality, set the `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` environment variable to `1`. See [Environment variables](18-Environmentvariables.md).
...
For tasks that need sustained parallelism or exceed your context window, [agent teams](01-OrchestrateteamsofClaudeCodesessions.md) give each worker its own independent context.
...
Consider [Skills](46-ExtendClaudewithskills.md) instead when you want reusable prompts or workflows that run in the main conversation context rather than isolated subagent context.

For a quick question about something already in your conversation, use [`/btw`](26-Interactivemode.md#side-questions-with-btw) instead of a subagent. It sees your full context but has no tool access, and the answer is discarded rather than added to history.

**Note:**
Subagents cannot spawn other subagents. If your workflow requires nested delegation, use [Skills](46-ExtendClaudewithskills.md) or [chain subagents](Getting started/02-ClaudeCodeoverview.md#chain-subagents) from the main conversation.
...
Claude Code [passes hook input as JSON](24-Hooksreference.md#pretooluse-input) via stdin to hook commands. The validation script reads this JSON, extracts the command being executed, and checks it against a list of SQL write operations. If a write operation is detected, the script [exits with code 2](24-Hooksreference.md#exit-code-2-behavior-per-event) to block execution and returns an error message to Claude via stderr.
...
The hook receives JSON via stdin with the Bash command in `tool_input.command`. Exit code 2 blocks the operation and feeds the error message back to Claude. See [Hooks](24-Hooksreference.md#exit-code-output) for details on exit codes and [Hook input](24-Hooksreference.md#pretooluse-input) for the complete input schema.

## Next steps

Now that you understand subagents, explore these related features:

* [Distribute subagents with plugins](38-Createplugins.md) to share subagents across teams or projects
* [Run Claude Code programmatically](23-RunClaudeCodeprogrammatically.md) with the Agent SDK for CI/CD and automation
* [Use MCP servers](30-ConnectClaudeCodetotoolsviaMCP.md) to give subagents access to external tools and data
