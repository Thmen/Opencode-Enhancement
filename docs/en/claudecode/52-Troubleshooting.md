...
<Tip>
  If you'd rather skip the terminal entirely, the [Claude Code Desktop app](15-Getstartedwiththedesktopapp.md) lets you install and use Claude Code through a graphical interface. Download it for [macOS](https://claude.ai/api/desktop/darwin/universal/dmg/latest/redirect?utm_source=claude_code\&utm_medium=docs) or [Windows](https://claude.ai/api/desktop/win32/x64/exe/latest/redirect?utm_source=claude_code\&utm_medium=docs) and start coding without any command-line setup.
</Tip>
...
| :---------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------- |
| `command not found: claude` or `'claude' is not recognized` | [Fix your PATH](Getting started/02-ClaudeCodeoverview.md#command-not-found-claude-after-installation)                                                           |
| `syntax error near unexpected token '<'`                    | [Install script returns HTML](Getting started/02-ClaudeCodeoverview.md#install-script-returns-html-instead-of-a-shell-script)                                   |
| `curl: (56) Failure writing output to destination`          | [Download script first, then run it](Getting started/02-ClaudeCodeoverview.md#curl-56-failure-writing-output-to-destination)                                    |
| `Killed` during install on Linux                            | [Add swap space for low-memory servers](Getting started/02-ClaudeCodeoverview.md#install-killed-on-low-memory-linux-servers)                                    |
| `TLS connect error` or `SSL/TLS secure channel`             | [Update CA certificates](Getting started/02-ClaudeCodeoverview.md#tls-or-ssl-connection-errors)                                                                 |
| `Failed to fetch version` or can't reach download server    | [Check network and proxy settings](Getting started/02-ClaudeCodeoverview.md#check-network-connectivity)                                                         |
| `irm is not recognized` or `&& is not valid`                | [Use the right command for your shell](Getting started/02-ClaudeCodeoverview.md#windows-irm-or--not-recognized)                                                 |
| `Claude Code on Windows requires git-bash`                  | [Install or configure Git Bash](Getting started/02-ClaudeCodeoverview.md#windows-claude-code-on-windows-requires-git-bash)                                      |
| `Error loading shared library`                              | [Wrong binary variant for your system](Getting started/02-ClaudeCodeoverview.md#linux-wrong-binary-variant-installed-muslglibc-mismatch)                        |
| `Illegal instruction` on Linux                              | [Architecture mismatch](Getting started/02-ClaudeCodeoverview.md#illegal-instruction-on-linux)                                                                  |
| `dyld: cannot load` or `Abort trap` on macOS                | [Binary incompatibility](Getting started/02-ClaudeCodeoverview.md#dyld-cannot-load-on-macos)                                                                    |
| `Invoke-Expression: Missing argument in parameter list`     | [Install script returns HTML](Getting started/02-ClaudeCodeoverview.md#install-script-returns-html-instead-of-a-shell-script)                                   |
...
| `unable to get local issuer certificate`                    | [Configure corporate CA certificates](Getting started/02-ClaudeCodeoverview.md#tls-or-ssl-connection-errors)                                                    |
| `OAuth error` or `403 Forbidden`                            | [Fix authentication](Getting started/02-ClaudeCodeoverview.md#authentication-issues)                                                                            |
...
On Linux, check for missing shared libraries. If `ldd` shows missing libraries, you may need to install system packages. On Alpine Linux and other musl-based distributions, see [Alpine Linux setup](45-Advancedsetup.md#alpine-linux-and-musl-based-distributions).
...
This means the install directory isn't in your shell's search path. See [Verify your PATH](Getting started/02-ClaudeCodeoverview.md#verify-your-path) for the fix on each platform.
...
2. **If behind a proxy**, set `HTTPS_PROXY` so the installer can route through it. See [proxy configuration](34-Enterprisenetworkconfiguration.md#proxy-configuration) for details.
   ```bash  theme={null}
   export HTTPS_PROXY=http://proxy.example.com:8080
   curl -fsSL https://claude.ai/install.sh | bash
   ```
...
**If Git is already installed** but Claude Code still can't find it, set the path in your [settings.json file](44-ClaudeCodesettings.md):
...
[Sandboxing](40-Sandboxing.md) is supported on WSL2 but requires installing additional packages. If you see an error like "Sandbox requires socat and bubblewrap" when running `/sandbox`, install the dependencies:
...
If the native installer fails with permission errors, the target directory may not be writable. See [Check directory permissions](Getting started/02-ClaudeCodeoverview.md#check-directory-permissions).
...
to run without approval using the `/permissions` command. See [Permissions docs](36-Configurepermissions.md#manage-permissions).
...
* **Behind a proxy**: corporate proxies can interfere with API requests. See [network configuration](34-Enterprisenetworkconfiguration.md) for proxy setup.
...
| `managed-mcp.json`            | [Managed MCP servers](30-ConnectClaudeCodetotoolsviaMCP.md#managed-mcp-configuration)                                               |
| Managed settings              | [Managed settings](44-ClaudeCodesettings.md#settings-files) (server-managed, MDM/OS-level policies, or file-based) |

On Windows, `~` refers to your user home directory, such as `C:\Users\YourName`.

For details on configuring these files, see [Settings](44-ClaudeCodesettings.md) and [MCP](30-ConnectClaudeCodetotoolsviaMCP.md).
...
Then set `USE_BUILTIN_RIPGREP=0` in your [environment](18-Environmentvariables.md).
...
For additional JetBrains configuration tips, see the [JetBrains IDE guide](Platforms and integrations/02-JetBrainsIDEs.md#plugin-settings).
...
2. **Use post-processing hooks**: set up automatic formatting hooks to detect and add missing language tags. See [Auto-format code after edits](25-Automateworkflowswithhooks.md#auto-format-code-after-edits) for an example of a PostToolUse formatting hook.
...
3. **Specify formatting preferences**: include formatting requirements in your prompts or project [memory](Use Claude Code/03-HowClauderemembersyourproject.md) files.
...
* **Use project conventions**: document your preferred markdown style in [`CLAUDE.md`](Use Claude Code/03-HowClauderemembersyourproject.md)
