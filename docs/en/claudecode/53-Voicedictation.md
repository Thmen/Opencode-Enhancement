[Home](README.md) / Voice dictation
...
While voice dictation is enabled, the input footer shows a `hold Space to speak` hint when the prompt is empty. The hint does not appear if you have a [custom status line](47-Customizeyourstatusline.md) configured.
...
* **Transcription is garbled or in the wrong language**: dictation defaults to English. If you are dictating in another language, set it in `/config` first. See [Change the dictation language](Getting started/02-ClaudeCodeoverview.md#change-the-dictation-language).

## See also

* [Customize keyboard shortcuts](27-Customizekeyboardshortcuts.md): rebind `voice:pushToTalk` and other CLI keyboard actions
* [Configure settings](44-ClaudeCodesettings.md): full reference for `voiceEnabled`, `language`, and other settings keys
* [Interactive mode](26-Interactivemode.md): keyboard shortcuts, input modes, and session controls
* [Built-in commands](11-Built-incommands.md): reference for `/voice`, `/config`, and all other commands
