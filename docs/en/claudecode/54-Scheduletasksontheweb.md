[Home](README.md) / Schedule tasks on the web
...
* **Desktop app**: open the **Schedule** page, click **New task**, and choose **New remote task**. See [Desktop scheduled tasks](14-UseClaudeCodeDesktop.md#schedule-recurring-tasks) for details.
...
Each task runs in a [cloud environment](08-ClaudeCodeontheweb.md#cloud-environment) that controls network access, environment variables, and setup scripts. Configure environments before creating a task to give Claude access to APIs, install dependencies, or restrict network scope. See [cloud environment](08-ClaudeCodeontheweb.md#cloud-environment) for the full setup guide.
...
* [Desktop scheduled tasks](14-UseClaudeCodeDesktop.md#schedule-recurring-tasks): schedule tasks that run on your machine with access to local files. The Desktop app's **Schedule** page shows both local and remote tasks in the same grid.
* [`/loop` and CLI scheduled tasks](41-Runpromptsonaschedule.md): lightweight scheduling within a CLI session
* [Cloud environment](08-ClaudeCodeontheweb.md#cloud-environment): configure the runtime environment for cloud tasks
* [MCP connectors](30-ConnectClaudeCodetotoolsviaMCP.md): connect external services like Slack, Linear, and Google Drive
* [GitHub Actions](20-ClaudeCodeGitHubActions.md): run Claude in your CI pipeline on repo events
