[Home](../README.md) / [Core concepts](README.md) / How Claude Code works
...
This guide covers the core architecture, built-in capabilities, and [tips for working effectively](../Getting started/02-ClaudeCodeoverview.md#work-effectively-with-claude-code). For step-by-step walkthroughs, see [Common workflows](../Use Claude Code/02-Commonworkflows.md). For extensibility features like skills, MCP, and hooks, see [Extend Claude Code](01-ExtendClaudeCode.md).
...
The agentic loop is powered by two components: [models](../Getting started/02-ClaudeCodeoverview.md#models) that reason and [tools](../Getting started/02-ClaudeCodeoverview.md#tools) that act. Claude Code serves as the **agentic harness** around Claude: it provides the tools, context management, and execution environment that turn a language model into a capable coding agent.
...
[Multiple models](../32-Modelconfiguration.md) are available with different tradeoffs. Sonnet handles most coding tasks well. Opus provides stronger reasoning for complex architectural decisions. Switch with `/model` during a session or start with `claude --model <name>`.
...
| **Code intelligence** | See type errors and warnings after edits, jump to definitions, find references (requires [code intelligence plugins](../17-Discoverandinstallprebuiltpluginsthroughmarketplaces.md#code-intelligence)) |

These are the primary capabilities. Claude also has tools for spawning subagents, asking you questions, and other orchestration tasks. See [Tools available to Claude](../51-Toolsreference.md) for the complete list.
...
**Extending the base capabilities:** The built-in tools are the foundation. You can extend what Claude knows with [skills](../46-ExtendClaudewithskills.md), connect to external services with [MCP](../30-ConnectClaudeCodetotoolsviaMCP.md), automate workflows with [hooks](../24-Hooksreference.md), and offload tasks to [subagents](../48-Createcustomsubagents.md). These extensions form a layer on top of the core agentic loop. See [Extend Claude Code](01-ExtendClaudeCode.md) for guidance on choosing the right extension for your needs.

## What Claude can access

This guide focuses on the terminal. Claude Code also runs in [VS Code](../Platforms and integrations/06-UseClaudeCodeinVSCode.md), [JetBrains IDEs](../Platforms and integrations/02-JetBrainsIDEs.md), and other environments.
...
* **Your [CLAUDE.md](../Use Claude Code/03-HowClauderemembersyourproject.md).** A markdown file where you store project-specific instructions, conventions, and context that Claude should know every session.
* **[Auto memory](../Use Claude Code/03-HowClauderemembersyourproject.md#auto-memory).** Learnings Claude saves automatically as you work, like project patterns and your preferences. The first 200 lines of MEMORY.md are loaded at the start of each session.
* **Extensions you configure.** [MCP servers](../30-ConnectClaudeCodetotoolsviaMCP.md) for external services, [skills](../46-ExtendClaudewithskills.md) for workflows, [subagents](../48-Createcustomsubagents.md) for delegated work, and [Claude in Chrome](../Platforms and integrations/01-UseClaudeCodewithChrome(beta).md) for browser interaction.
...
You can access Claude Code through the terminal, the [desktop app](../14-UseClaudeCodeDesktop.md), [IDE extensions](/en/ide-integrations), [claude.ai/code](https://claude.ai/code), [Remote Control](../Platforms and integrations/04-ContinuelocalsessionsfromanydevicewithRemoteControl.md), [Slack](../Platforms and integrations/05-ClaudeCodeinSlack.md), and [CI/CD pipelines](../20-ClaudeCodeGitHubActions.md). The interface determines how you see and interact with Claude, but the underlying agentic loop is identical. See [Use Claude Code everywhere](../Getting started/02-ClaudeCodeoverview.md#use-claude-code-everywhere) for the full list.

## Work with sessions

Claude Code saves your conversation locally as you work. Each message, tool use, and result is stored, which enables [rewinding](../Getting started/02-ClaudeCodeoverview.md#undo-changes-with-checkpoints), [resuming, and forking](../Getting started/02-ClaudeCodeoverview.md#resume-or-fork-sessions) sessions. Before Claude makes code changes, it also snapshots the affected files so you can revert if needed.

**Sessions are independent.** Each new session starts with a fresh context window, without the conversation history from previous sessions. Claude can persist learnings across sessions using [auto memory](../Use Claude Code/03-HowClauderemembersyourproject.md#auto-memory), and you can add your own persistent instructions in [CLAUDE.md](../Use Claude Code/03-HowClauderemembersyourproject.md).
...
Since sessions are tied to directories, you can run parallel Claude sessions by using [git worktrees](../Use Claude Code/02-Commonworkflows.md#run-parallel-claude-code-sessions-with-git-worktrees), which create separate directories for individual branches.
...
Claude's context window holds your conversation history, file contents, command outputs, [CLAUDE.md](../Use Claude Code/03-HowClauderemembersyourproject.md), [auto memory](../Use Claude Code/03-HowClauderemembersyourproject.md#auto-memory), loaded skills, and system instructions. As you work, context fills up. Claude compacts automatically, but instructions from early in the conversation can get lost. Put persistent rules in CLAUDE.md, and run `/context` to see what's using space.
...
[Skills](../46-ExtendClaudewithskills.md) load on demand. Claude sees skill descriptions at session start, but the full content only loads when a skill is used. For skills you invoke manually, set `disable-model-invocation: true` to keep descriptions out of context until you need them.

[Subagents](../48-Createcustomsubagents.md) get their own fresh context, completely separate from your main conversation. Their work doesn't bloat your context. When done, they return a summary. This isolation is why subagents help with long sessions.

See [context costs](01-ExtendClaudeCode.md#understand-context-costs) for what each feature costs, and [reduce token usage](../12-Managecostseffectively.md#reduce-token-usage) for tips on managing context.
...
You can also allow specific commands in `.claude/settings.json` so Claude doesn't ask each time. This is useful for trusted commands like `npm test` or `git status`. Settings can be scoped from organization-wide policies down to personal preferences. See [Permissions](../36-Configurepermissions.md) for details.
...
- [Extend with features](01-ExtendClaudeCode.md): Add Skills, MCP connections, and custom commands

- [Common workflows](../Use Claude Code/02-Commonworkflows.md): Step-by-step guides for typical tasks
