[Home](../README.md) / [Platforms and integrations](README.md) / Continue local sessions from any device with Remote Control
...
* **Use your full local environment remotely**: your filesystem, [MCP servers](../30-ConnectClaudeCodetotoolsviaMCP.md), tools, and project configuration all stay available
...
* **Your Team or Enterprise admin hasn't enabled it**: Remote Control is off by default on these plans. An admin can enable it at [claude.ai/admin-settings/claude-code](https://claude.ai/admin-settings/claude-code) by turning on the **Remote Control** toggle. This is a server-side organization setting, not a [managed settings](../36-Configurepermissions.md#managed-only-settings) key.
...
* [Claude Code on the web](../08-ClaudeCodeontheweb.md): run sessions in Anthropic-managed cloud environments instead of on your machine
* [Channels](../05-Pusheventsintoarunningsessionwithchannels.md): forward Telegram, Discord, or iMessage into a session so Claude reacts to messages while you're away
* [Dispatch](../14-UseClaudeCodeDesktop.md#sessions-from-dispatch): message a task from your phone and it can spawn a Desktop session to handle it
* [Authentication](../04-Authentication.md): set up `/login` and manage credentials for claude.ai
* [CLI reference](../09-CLIreference.md): full list of flags and commands including `claude remote-control`
* [Security](../42-Security.md): how Remote Control sessions fit into the Claude Code security model
* [Data usage](../13-Datausage.md): what data flows through the Anthropic API during local and remote sessions
