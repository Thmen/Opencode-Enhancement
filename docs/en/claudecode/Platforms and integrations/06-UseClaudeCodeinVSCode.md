[Home](../README.md) / [Platforms and integrations](README.md) / Use Claude Code in VS Code
...
* **Command menu**: click `/` or type `/` to open the command menu. Options include attaching files, switching models, toggling extended thinking, viewing plan usage (`/usage`), and starting a [Remote Control](04-ContinuelocalsessionsfromanydevicewithRemoteControl.md) session (`/remote-control`). The Customize section provides access to MCP servers, hooks, memory, permissions, and plugins. Items with a terminal icon open in the integrated terminal.
...
* **Extended thinking**: lets Claude spend more time reasoning through complex problems. Toggle it on via the command menu (`/`). See [Extended thinking](../Use Claude Code/02-Commonworkflows.md#use-extended-thinking-thinking-mode) for details.
...
The VS Code extension includes a graphical interface for installing and managing [plugins](../38-Createplugins.md). Type `/plugins` in the prompt box to open the **Manage plugins** interface.
...
For setup instructions, the full list of capabilities, and troubleshooting, see [Use Claude Code with Chrome](01-UseClaudeCodewithChrome(beta).md).
...
| `initialPermissionMode`           | `default` | Controls approval prompts for new conversations: `default`, `plan`, `acceptEdits`, `auto`, or `bypassPermissions`. See [permission modes](../Use Claude Code/04-Chooseapermissionmode.md).                                                                                                                                              |
...
| ------------------- | ------------------- | ------------------------------------------------------------------------------------ |
| Commands and skills | [All](../11-Built-incommands.md) | Subset (type `/` to see available)                                                   |
...
For full details on how checkpoints work and their limitations, see [Checkpointing](../07-Checkpointing.md).
...
To manage MCP servers without leaving VS Code, type `/mcp` in the chat panel. The MCP management dialog lets you enable or disable servers, reconnect to a server, and manage OAuth authentication. See the [MCP documentation](../30-ConnectClaudeCodetotoolsviaMCP.md) for available servers.
...
Your code stays private. Claude Code processes your code to provide assistance but does not use it to train models. For details on data handling and how to opt out of logging, see [Data and privacy](../13-Datausage.md).
...
For additional help, see the [troubleshooting guide](../52-Troubleshooting.md).

## Next steps

Now that you have Claude Code set up in VS Code:

* [Explore common workflows](../Use Claude Code/02-Commonworkflows.md) to get the most out of Claude Code
* [Set up MCP servers](../30-ConnectClaudeCodetotoolsviaMCP.md) to extend Claude's capabilities with external tools. Add servers using the CLI, then manage them with `/mcp` in the chat panel.
* [Configure Claude Code settings](../44-ClaudeCodesettings.md) to customize allowed commands, hooks, and more. These settings are shared between the extension and CLI.
