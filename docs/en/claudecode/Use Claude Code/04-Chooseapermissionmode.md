[Home](../README.md) / [Use Claude Code](README.md) / Choose a permission mode
...
* [Switch modes](../Getting started/02-ClaudeCodeoverview.md#switch-permission-modes) during a session, at startup, or as a default
* [Choose a mode](../Getting started/02-ClaudeCodeoverview.md#available-modes) based on what Claude should be able to do without asking
* [Run auto mode](../Getting started/02-ClaudeCodeoverview.md#eliminate-prompts-with-auto-mode) with background safety checks, and see what it [blocks by default](../Getting started/02-ClaudeCodeoverview.md#what-the-classifier-blocks-by-default)
* [Plan changes read-only](../Getting started/02-ClaudeCodeoverview.md#analyze-before-you-edit-with-plan-mode) before approving edits
* [Restrict Claude to pre-approved tools](../Getting started/02-ClaudeCodeoverview.md#allow-only-pre-approved-tools-with-dontask-mode) for locked-down environments
* [Skip checks entirely](../Getting started/02-ClaudeCodeoverview.md#skip-all-checks-with-bypasspermissions-mode) in isolated environments
...
| [`plan`](../Getting started/02-ClaudeCodeoverview.md#analyze-before-you-edit-with-plan-mode)                   | Read files                                 | Exploring a codebase, planning a refactor   |
| [`auto`](../Getting started/02-ClaudeCodeoverview.md#eliminate-prompts-with-auto-mode)                         | All actions, with background safety checks | Long-running tasks, reducing prompt fatigue |
| [`bypassPermissions`](../Getting started/02-ClaudeCodeoverview.md#skip-all-checks-with-bypasspermissions-mode) | All actions, no checks                     | Isolated containers and VMs only            |
| [`dontAsk`](../Getting started/02-ClaudeCodeoverview.md#allow-only-pre-approved-tools-with-dontask-mode)       | Only pre-approved tools                    | Locked-down environments                    |
...
1. Actions matching your [allow or deny rules](../36-Configurepermissions.md#manage-permissions) resolve immediately
...
When Claude spawns a [subagent](../48-Createcustomsubagents.md), the classifier evaluates the delegated task before the subagent starts. A task description that looks dangerous on its own, like "delete all remote branches matching this pattern", is blocked at spawn time.
...
**Permission rules** are the first stop. Add `allow`, `ask`, or `deny` entries to your settings file to pre-approve safe commands, force a prompt for risky ones, or block specific tools entirely. Rules apply in every mode except `bypassPermissions`, which skips the permission layer entirely, and are matched by tool name and argument pattern. See [Manage permissions](../36-Configurepermissions.md#manage-permissions) for syntax and examples.

**Hooks** cover logic that pattern-matching rules can't express. A [`PreToolUse` hook](../24-Hooksreference.md#pretooluse-decision-control) runs before every tool call and can allow, deny, or escalate based on command content, file paths, time of day, or a response from an external policy service. A [`PermissionRequest` hook](../24-Hooksreference.md#permissionrequest) intercepts the permission dialog itself and answers on your behalf. See [Hooks](../24-Hooksreference.md) for configuration.

## See also

* [Permissions](../36-Configurepermissions.md): permission rules, syntax, managed policies
* [Hooks](../24-Hooksreference.md): custom permission logic, lifecycle scripting
* [Security](../42-Security.md): security safeguards and best practices
* [Sandboxing](../40-Sandboxing.md): filesystem and network isolation for Bash commands
* [Non-interactive mode](../23-RunClaudeCodeprogrammatically.md): run Claude Code programmatically with the `-p` flag
