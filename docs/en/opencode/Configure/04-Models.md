[Home](../README.md) / [Configure](README.md) / Models
...
Learn more about [providers](../03-Providers.md).
...
Here the full ID is `provider_id/model_id`. For example, if you’re using [OpenCode Zen](../Usage/06-Zen.md), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.

If you’ve configured a [custom provider](../03-Providers.md#custom), the `provider_id` is key from the `provider` part of your config, and the `model_id` is the key from `provider.models`.
...
You can also configure these options for any agents that you are using. The agent config overrides any global options here. [Learn more](03-Agents.md#additional).
...
Use the keybind `variant_cycle` to quickly switch between variants. [Learn more](06-Keybinds.md).
