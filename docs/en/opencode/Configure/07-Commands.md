[Home](../README.md) / [Configure](README.md) / Commands
...
Custom commands are in addition to the built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`. [Learn more](../Usage/02-TUI.md#commands).
...
Use the `command` option in your OpenCode [config](../02-Config.md):
...
Use *!`command`* to inject [bash command](../Usage/02-TUI.md#bash-commands) output into your prompt.
...
Use the `agent` config to optionally specify which [agent](03-Agents.md) should execute this command.
If this is a [subagent](03-Agents.md#subagents) the command will trigger a subagent invocation by default.
...
Use the `subtask` boolean to force the command to trigger a [subagent](03-Agents.md#subagents) invocation.
This is useful if you want the command to not pollute your primary context and will **force** the agent to act as a subagent,
even if `mode` is set to `primary` on the [agent](03-Agents.md) configuration.
...
opencode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](../Usage/02-TUI.md#commands).
