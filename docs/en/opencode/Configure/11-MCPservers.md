[Home](../README.md) / [Configure](README.md) / MCP servers
...
You can define MCP servers in your [OpenCode Config](../02-Config.md) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Your local config values override the remote defaults. See [config precedence](../02-Config.md#precedence-order) for more details.
...
2. In your [agent config](03-Agents.md#tools), enable the MCP server as a tool.
...
Alternatively, you can add something like this to your [AGENTS.md](02-Rules.md).
