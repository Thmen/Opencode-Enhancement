[Home](../README.md) / [Develop](README.md) / Server
...
endpoint. This endpoint is also used to generate an [SDK](01-SDK.md).
...
When you start the TUI it randomly assigns a port and hostname. You can instead pass in the `--hostname` and `--port` [flags](../Usage/03-CLI.md). Then use this to connect to its server.

The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OpenCode [IDE](../Usage/05-IDE.md) plugins.
