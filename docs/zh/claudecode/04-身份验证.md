...
[安装 Claude Code](44-高级设置.md#install-claude-code) 后，在终端中运行 `claude`。首次启动时，Claude Code 会打开浏览器窗口供您登录。
...
* **云提供商**：如果您的组织使用 [Amazon Bedrock](02-AmazonBedrock上的ClaudeCode.md)、[Google Vertex AI](21-GoogleVertexAI上的ClaudeCode.md) 或 [Microsoft Foundry](30-ClaudeCodeonMicrosoftFoundry.md)，请在运行 `claude` 之前设置所需的环境变量。不需要浏览器登录。

要登出并重新身份验证，请在 Claude Code 提示符处输入 `/logout`。

如果您在登录时遇到问题，请参阅 [身份验证故障排除](51-故障排除.md#authentication-issues)。
...
<Steps>
  ...
  <Step title="用户完成设置">
    ...
    * 接受 Console 邀请
    * [检查系统要求](44-高级设置.md#system-requirements)
    * [安装 Claude Code](44-高级设置.md#install-claude-code)
    * 使用 Console 账户凭证登录
  </Step>
</Steps>
...
<Steps>
  <Step title="遵循提供商设置">
    遵循 [Bedrock 文档](02-AmazonBedrock上的ClaudeCode.md)、[Vertex 文档](21-GoogleVertexAI上的ClaudeCode.md) 或 [Microsoft Foundry 文档](30-ClaudeCodeonMicrosoftFoundry.md)。
  </Step>

  <Step title="分发配置">
    将环境变量和生成云凭证的说明分发给您的用户。阅读有关如何 [在此处管理配置](43-ClaudeCode设置.md) 的更多信息。
  </Step>

  <Step title="安装 Claude Code">
    用户可以 [安装 Claude Code](44-高级设置.md#install-claude-code)。
  </Step>
</Steps>
...
* **自定义凭证脚本**：[`apiKeyHelper`](43-ClaudeCode设置.md#available-settings) 设置可以配置为运行返回 API 密钥的 shell 脚本。
