...
**Note:**
Channels 处于[研究预览](06-使用channels将事件推送到运行中的会话.md#research-preview)阶段，需要 Claude Code v2.1.80 或更高版本。它们需要 claude.ai 登录。不支持控制台和 API 密钥身份验证。Team 和 Enterprise 组织必须[明确启用它们](06-使用channels将事件推送到运行中的会话.md#enterprise-controls)。

Channel 是一个 MCP 服务器，它将事件推送到 Claude Code 会话中，以便 Claude 可以对终端外发生的事情做出反应。

您可以构建单向或双向频道。单向频道转发警报、webhooks 或监控事件供 Claude 处理。双向频道（如聊天桥接）也[公开回复工具](快速开始/01-ClaudeCode概述.md#expose-a-reply-tool)，以便 Claude 可以发送消息回复。具有受信任发送者路径的频道也可以选择加入[中继权限提示](快速开始/01-ClaudeCode概述.md#relay-permission-prompts)，以便您可以远程批准或拒绝工具使用。

本页涵盖：

* [概述](快速开始/01-ClaudeCode概述.md#overview)：频道如何工作
* [您需要什么](快速开始/01-ClaudeCode概述.md#what-you-need)：要求和一般步骤
* [示例：构建 webhook 接收器](快速开始/01-ClaudeCode概述.md#example-build-a-webhook-receiver)：最小单向演练
* [服务器选项](快速开始/01-ClaudeCode概述.md#server-options)：构造函数字段
* [通知格式](快速开始/01-ClaudeCode概述.md#notification-format)：事件有效负载
* [公开回复工具](快速开始/01-ClaudeCode概述.md#expose-a-reply-tool)：让 Claude 发送消息回复
* [门控入站消息](快速开始/01-ClaudeCode概述.md#gate-inbound-messages)：发送者检查以防止提示注入
* [中继权限提示](快速开始/01-ClaudeCode概述.md#relay-permission-prompts)：将工具批准提示转发到远程频道

要使用现有频道而不是构建一个，请参阅 [Channels](06-使用channels将事件推送到运行中的会话.md)。Telegram、Discord、iMessage 和 fakechat 包含在研究预览中。
...
[服务器选项](快速开始/01-ClaudeCode概述.md#server-options)和[通知格式](快速开始/01-ClaudeCode概述.md#notification-format)部分详细介绍了每一项。有关完整演练，请参阅[示例：构建 webhook 接收器](快速开始/01-ClaudeCode概述.md#example-build-a-webhook-receiver)。

在研究预览期间，自定义频道不在[批准的允许列表](06-使用channels将事件推送到运行中的会话.md#supported-channels)上。使用 `--dangerously-load-development-channels` 在本地测试。有关详细信息，请参阅[在研究预览期间测试](快速开始/01-ClaudeCode概述.md#test-during-the-research-preview)。
...
* **服务器配置**：使用 `claude/channel` 在其能力中创建 MCP 服务器，这是告诉 Claude Code 这是一个频道的原因。[`instructions`](快速开始/01-ClaudeCode概述.md#server-options) 字符串进入 Claude 的系统提示：告诉 Claude 期望什么事件、是否回复以及如果应该回复，使用哪个工具和传回哪个属性。
...
如果您看到"被组织政策阻止"，您的 Team 或 Enterprise 管理员需要[启用频道](06-使用channels将事件推送到运行中的会话.md#enterprise-controls)。
...
在您的 Claude Code 终端中，您会看到 Claude 接收消息并开始响应：读取文件、运行命令或消息要求的任何操作。这是一个单向频道，因此 Claude 在您的会话中行动，但不会通过 webhook 发送任何内容回复。要添加回复，请参阅[公开回复工具](快速开始/01-ClaudeCode概述.md#expose-a-reply-tool)。
...
在研究预览期间，每个频道都必须在[批准的允许列表](06-使用channels将事件推送到运行中的会话.md#research-preview)上才能注册。开发标志在确认提示后绕过特定条目的允许列表。此示例显示两种条目类型：
...
| `capabilities.experimental['claude/channel/permission']` | `object` | 可选。始终为 `{}`。声明此频道可以接收权限中继请求。声明后，Claude Code 将工具批准提示转发到您的频道，以便您可以远程批准或拒绝它们。请参阅[中继权限提示](快速开始/01-ClaudeCode概述.md#relay-permission-prompts)。 |
| `capabilities.tools`                                     | `object` | 仅双向。始终为 `{}`。标准 MCP 工具能力。请参阅[公开回复工具](快速开始/01-ClaudeCode概述.md#expose-a-reply-tool)。                                                       |
...
要将这些添加到上面的[webhook 接收器](快速开始/01-ClaudeCode概述.md#example-build-a-webhook-receiver)：
...
来自 Claude Code 的出站通知是 `notifications/claude/channel/permission_request`。与[频道通知](快速开始/01-ClaudeCode概述.md#notification-format)一样，传输是标准 MCP，但方法和架构是 Claude Code 扩展。`params` 对象有四个字符串字段，您的服务器将其格式化为出站提示：
...
仅在您的频道[验证发送者](快速开始/01-ClaudeCode概述.md#gate-inbound-messages)时声明该能力，因为任何可以通过您的频道回复的人都可以批准或拒绝您会话中的工具使用。

要将这些添加到在[公开回复工具](快速开始/01-ClaudeCode概述.md#expose-a-reply-tool)中组装的双向聊天桥接：
...
#### 处理传入请求
在您的 `Server` 构造函数和 `mcp.connect()` 之间注册一个通知处理程序。当权限对话打开时，Claude Code 使用[四个请求字段](快速开始/01-ClaudeCode概述.md#permission-request-fields)调用它。您的处理程序为您的平台格式化提示，并包括使用 ID 回复的说明：
...
#### 在您的入站处理程序中拦截判决
您的入站处理程序是接收来自您的平台的消息的循环或回调：与您[根据发送者进行门控](快速开始/01-ClaudeCode概述.md#gate-inbound-messages)和发出 `notifications/claude/channel` 以将聊天转发给 Claude 的地方相同。在聊天转发调用之前添加一个检查，识别判决格式并改为发出权限通知。
...
下面组装的 `webhook.ts` 结合了本页的所有三个扩展：回复工具、发送者门控和权限中继。如果您从这里开始，您还需要初始演练中的[项目设置和 `.mcp.json` 条目](快速开始/01-ClaudeCode概述.md#example-build-a-webhook-receiver)。
...
在三个终端中测试判决路径。第一个是您的 Claude Code 会话，使用[开发标志](快速开始/01-ClaudeCode概述.md#test-during-the-research-preview)启动，以便它生成 `webhook.ts`：
...
要使您的频道可安装和可共享，请将其包装在[插件](39-创建插件.md)中并将其发布到[市场](38-创建和分发pluginmarketplace.md)。用户使用 `/plugin install` 安装它，然后使用 `--channels plugin:<name>@<marketplace>` 按会话启用它。

发布到您自己的市场的频道仍然需要 `--dangerously-load-development-channels` 来运行，因为它不在[批准的允许列表](06-使用channels将事件推送到运行中的会话.md#supported-channels)上。要将其添加，[将其提交到官方市场](39-创建插件.md#submit-your-plugin-to-the-official-marketplace)。频道插件在被批准之前经过安全审查。在 Team 和 Enterprise 计划上，管理员可以改为将您的插件包含在组织自己的 [`allowedChannelPlugins`](06-使用channels将事件推送到运行中的会话.md#restrict-which-channel-plugins-can-run) 列表中，该列表替换默认的 Anthropic 允许列表。

## 另请参阅

* [Channels](06-使用channels将事件推送到运行中的会话.md) 安装和使用 Telegram、Discord、iMessage 或 fakechat 演示，以及为 Team 或 Enterprise 组织启用频道
* [工作频道实现](https://github.com/anthropics/claude-plugins-official/tree/main/external_plugins)用于具有配对流、回复工具和文件附件的完整服务器代码
* [MCP](31-通过MCP将ClaudeCode连接到工具.md) 用于频道服务器实现的基础协议
* [Plugins](39-创建插件.md) 打包您的频道，以便用户可以使用 `/plugin install` 安装它
