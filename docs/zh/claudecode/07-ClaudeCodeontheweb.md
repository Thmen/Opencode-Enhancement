...
使用设置脚本来安装云需要但你的笔记本电脑已有的东西，如语言运行时或 CLI 工具。使用[SessionStart hook](23-Hooks参考.md#sessionstart)进行应该在任何地方运行的项目设置，云和本地，如 `npm install`。
...
自定义环境镜像和快照尚不支持。使用[设置脚本](快速开始/01-ClaudeCode概述.md#setup-scripts)在会话启动时安装包，或使用[SessionStart hooks](23-Hooks参考.md#sessionstart)进行也应在本地环境中运行的依赖安装。SessionStart hooks 有[已知限制](快速开始/01-ClaudeCode概述.md#dependency-management-limitations)。
...
SessionStart hooks 可以通过写入 `CLAUDE_ENV_FILE` 环境变量中指定的文件来为后续 Bash 命令持久化环境变量。有关详情，请参阅 hooks 参考中的[SessionStart hooks](23-Hooks参考.md#sessionstart)。
...
1. **自动化环境设置**：使用[设置脚本](快速开始/01-ClaudeCode概述.md#setup-scripts)在 Claude Code 启动之前安装依赖和配置工具。对于更高级的场景，配置[SessionStart hooks](23-Hooks参考.md#sessionstart)。
2. **记录要求**：在你的 `CLAUDE.md` 文件中清楚地指定依赖和命令。如果你有 `AGENTS.md` 文件，你可以在你的 `CLAUDE.md` 中使用 `@AGENTS.md` 来源它以维护单一的真实来源。

## 相关资源

* [Hooks 配置](23-Hooks参考.md)
* [设置参考](43-ClaudeCode设置.md)
* [安全](41-安全性.md)
* [数据使用](12-数据使用.md)
