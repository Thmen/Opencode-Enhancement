...
**Note:**
总结将您保持在同一会话中并压缩上下文。如果您想尝试不同的方法，同时保持原始会话完整，请改用 [fork](核心概念/02-ClaudeCode如何工作.md#resume-or-fork-sessions)（`claude --continue --fork-session`）。
...
* [Interactive mode](27-交互模式.md) - 快捷键和会话控制
* [Built-in commands](12-内置命令.md) - 使用 `/rewind` 访问 checkpoints
