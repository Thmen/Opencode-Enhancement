...
**Note:**
Code Review 处于研究预览阶段，仅适用于 [Teams 和 Enterprise](https://claude.ai/admin-settings/claude-code) 订阅。对于启用了 [Zero Data Retention](56-零数据保留.md) 的组织，此功能不可用。
...
要在您自己的 CI 基础设施中运行 Claude 而不是使用此托管服务，请参阅 [GitHub Actions](21-ClaudeCodeGitHubActions.md) 或 [GitLab CI/CD](22-ClaudeCodeGitLabCICD.md)。
...
Code Review 使用对内容的读取访问权限和对 pull request 的写入访问权限。更广泛的权限集也支持 [GitHub Actions](21-ClaudeCodeGitHubActions.md)，如果您稍后启用的话。
...
* [Plugins](18-通过市场发现和安装预构建插件.md)：浏览插件市场，包括用于在推送前本地运行按需审查的 `code-review` 插件
* [GitHub Actions](21-ClaudeCodeGitHubActions.md)：在您自己的 GitHub Actions 工作流中运行 Claude，以实现超越代码审查的自定义自动化
* [GitLab CI/CD](22-ClaudeCodeGitLabCICD.md)：GitLab 管道的自托管 Claude 集成
