...
在 Bedrock、Vertex 和 Foundry 上，Claude Code 不会从您的云中发送指标。为了获取成本指标，几家大型企业报告使用[LiteLLM](28-LLMgateway配置.md#litellm-configuration)，这是一个开源工具，可帮助公司[按密钥跟踪支出](https://docs.litellm.ai/docs/proxy/virtual_keys#tracking-spend)。此项目与 Anthropic 无关，尚未进行安全审计。
...
* Agent 团队默认被禁用。在您的[settings.json](43-ClaudeCode设置.md)或环境中设置 `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` 以启用它们。请参阅[启用 agent 团队](01-协调ClaudeCode会话团队.md#enable-agent-teams)。
...
使用 `/cost` 检查您当前的令牌使用情况，或[配置您的状态行](46-自定义你的状态行.md#context-window-usage)以连续显示它。
...
Sonnet 处理大多数编码任务效果很好，成本低于 Opus。为复杂的架构决策或多步推理保留 Opus。使用 `/model` 在会话中途切换模型，或在 `/config` 中设置默认值。对于简单的 subagent 任务，在您的[subagent 配置](47-创建自定义subagents.md#choose-a-model)中指定 `model: haiku`。
...
* **工具搜索是自动的**：当 MCP 工具描述超过您的上下文窗口的 10% 时，Claude Code 会自动延迟它们并通过[工具搜索](29-通过MCP将ClaudeCode连接到工具.md#scale-with-mcp-tool-search)按需加载工具。由于延迟的工具仅在实际使用时进入上下文，较低的阈值意味着较少的空闲工具定义消耗空间。使用 `ENABLE_TOOL_SEARCH=auto:<N>` 设置较低的阈值（例如，`auto:5` 在工具超过您的上下文窗口的 5% 时触发）。

### 为类型化语言安装代码智能插件

[代码智能插件](16-通过市场发现和安装预构建插件.md#code-intelligence)为 Claude 提供精确的符号导航，而不是基于文本的搜索，减少在探索不熟悉的代码时不必要的文件读取。单个"转到定义"调用替代了可能需要的 grep 后跟读取多个候选文件。已安装的语言服务器还会在编辑后自动报告类型错误，因此 Claude 无需运行编译器即可捕获错误。

### 将处理卸载到 hooks 和 skills

自定义[hooks](23-Hooks参考.md)可以在 Claude 看到数据之前对其进行预处理。Claude 不是读取 10,000 行日志文件来查找错误，hook 可以 grep `ERROR` 并仅返回匹配的行，将上下文从数万个令牌减少到数百个。

[skill](45-使用skills扩展Claude.md)可以为 Claude 提供领域知识，这样它就不必进行探索。例如，"codebase-overview" skill 可以描述您的项目架构、关键目录和命名约定。当 Claude 调用该 skill 时，它会立即获得此上下文，而不是花费令牌读取多个文件来理解结构。

例如，此 PreToolUse hook 过滤测试输出以仅显示失败：

<Tabs>
  <Tab title="settings.json">
    将此添加到您的[settings.json](43-ClaudeCode设置.md#settings-files)以在每个 Bash 命令之前运行 hook：
...
您的[CLAUDE.md](使用 Claude Code/03-Claude如何记住你的项目.md)文件在会话开始时加载到上下文中。如果它包含特定工作流的详细指令（如 PR 审查或数据库迁移），即使您在做不相关的工作时，这些令牌也会存在。[Skills](45-使用skills扩展Claude.md)仅在调用时按需加载，因此将专门指令移动到 skills 中可以保持您的基础上下文较小。目标是通过仅包含必要内容来将 CLAUDE.md 保持在约 500 行以下。

### 调整扩展思考

扩展思考默认启用，预算为 31,999 个令牌，因为它显著改进了复杂规划和推理任务的性能。但是，思考令牌作为输出令牌计费，因此对于不需要深度推理的更简单任务，您可以通过在 `/effort` 中或在 `/model` 中降低[努力级别](31-模型配置.md#adjust-effort-level)、在 `/config` 中禁用思考或降低预算（例如，`MAX_THINKING_TOKENS=8000`）来降低成本。

### 将冗长的操作委托给 subagents

运行测试、获取文档或处理日志文件可能会消耗大量上下文。将这些委托给[subagents](47-创建自定义subagents.md#isolate-high-volume-operations)，以便冗长的输出保留在 subagent 的上下文中，而只有摘要返回到您的主对话。
