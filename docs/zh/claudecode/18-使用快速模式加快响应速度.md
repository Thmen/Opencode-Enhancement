...
<Note>
  快速模式处于[研究预览](快速开始/01-ClaudeCode概述.md#research-preview)阶段。该功能、定价和可用性可能会根据反馈而改变。
</Note>
...
本页涵盖如何[切换快速模式](快速开始/01-ClaudeCode概述.md#toggle-fast-mode)、其[成本权衡](快速开始/01-ClaudeCode概述.md#understand-the-cost-tradeoff)、[何时使用](快速开始/01-ClaudeCode概述.md#decide-when-to-use-fast-mode)、[要求](快速开始/01-ClaudeCode概述.md#requirements)、[每个会话选择加入](快速开始/01-ClaudeCode概述.md#require-per-session-opt-in)和[速率限制行为](快速开始/01-ClaudeCode概述.md#handle-rate-limits)。
...
* 在您的[用户设置文件](43-ClaudeCode设置.md)中设置 `"fastMode": true`

默认情况下，快速模式在会话之间保持。管理员可以配置快速模式在每个会话时重置。有关详细信息，请参阅[要求每个会话选择加入](快速开始/01-ClaudeCode概述.md#require-per-session-opt-in)。

为了获得最佳成本效率，在会话开始时启用快速模式，而不是在对话中途切换。有关详细信息，请参阅[了解成本权衡](快速开始/01-ClaudeCode概述.md#understand-the-cost-tradeoff)。
...
您可以结合两者：在直接任务上使用快速模式和较低的[努力级别](31-模型配置.md#adjust-effort-level)以获得最大速度。
...
* **团队和企业的管理员启用**：快速模式默认对团队和企业组织禁用。管理员必须明确[启用快速模式](快速开始/01-ClaudeCode概述.md#enable-fast-mode-for-your-organization)，用户才能访问它。
...
* [模型配置](31-模型配置.md)：切换模型并调整努力级别
* [有效管理成本](11-有效管理成本.md)：跟踪令牌使用情况并降低成本
* [状态行配置](46-自定义你的状态行.md)：显示模型和上下文信息
