...
Claude Code GitHub Actions 为您的 GitHub 工作流带来了 AI 驱动的自动化。只需在任何 PR 或 issue 中简单地提及 `@claude`，Claude 就可以分析您的代码、创建拉取请求、实现功能和修复错误 - 所有这些都遵循您项目的标准。如需在每个 PR 上自动发布评论而无需触发器，请参阅 [GitHub Code Review](09-CodeReview.md)。
...
* **Skills** - 直接从提示调用已安装的 [skills](45-使用skills扩展Claude.md)
...
| 参数                  | 描述                                         | 必需    |
| ------------------- | ------------------------------------------ | ----- |
| `prompt`            | Claude 的说明（纯文本或 [skill](45-使用skills扩展Claude.md) 名称） | 否\*   |
