[首页](README.md) / Google Vertex AI 上的 Claude Code
...
<Note>
  如果您要将 Claude Code 部署给多个用户，请[固定您的模型版本](快速开始/01-ClaudeCode概述.md#5-pin-model-versions)，以防止在 Anthropic 发布新模型时出现中断。
</Note>
...
有关当前和旧版模型 ID，请参阅[模型概览](https://platform.claude.com/docs/en/about-claude/models/overview)。有关完整的环境变量列表，请参阅[模型配置](31-模型配置.md#pin-models-for-third-party-deployments)。
...
要为您固定的模型启用 1M context window，请在模型 ID 后附加 `[1m]`。有关详细信息，请参阅[为第三方部署固定模型](31-模型配置.md#pin-models-for-third-party-deployments)。
