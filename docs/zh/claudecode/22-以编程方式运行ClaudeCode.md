...
要从 CLI 以编程方式运行 Claude Code，请使用 `-p` 传递您的提示和任何 [CLI 选项](08-CLI参考.md)：
...
将 `-p`（或 `--print`）标志添加到任何 `claude` 命令以非交互方式运行它。所有 [CLI 选项](08-CLI参考.md) 都适用于 `-p`，包括：
...
`--allowedTools` 标志使用 [权限规则语法](43-ClaudeCode设置.md#permission-rule-syntax)。尾部的 ` *` 启用前缀匹配，因此 `Bash(git diff *)` 允许任何以 `git diff` 开头的命令。空格在 `*` 之前很重要：没有它，`Bash(git diff*)` 也会匹配 `git diff-index`。

<Note>
  用户调用的 [skills](45-使用skills扩展Claude.md) 如 `/commit` 和 [内置命令](10-内置命令.md) 仅在交互模式下可用。在 `-p` 模式下，改为描述您想要完成的任务。
</Note>
...
有关更多选项（包括 `--system-prompt` 以完全替换默认提示），请参阅 [系统提示标志](08-CLI参考.md#system-prompt-flags)。
...
* [CLI 参考](08-CLI参考.md)：所有 CLI 标志和选项
* [GitHub Actions](19-ClaudeCodeGitHubActions.md)：在 GitHub 工作流中使用 Agent SDK
* [GitLab CI/CD](20-ClaudeCodeGitLabCICD.md)：在 GitLab 管道中使用 Agent SDK
