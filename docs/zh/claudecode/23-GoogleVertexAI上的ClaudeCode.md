...
有关当前和旧版模型 ID，请参阅[模型概览](https://platform.claude.com/docs/en/about-claude/models/overview)。有关完整的环境变量列表，请参阅[模型配置](33-模型配置.md#pin-models-for-third-party-deployments)。
...
要为您固定的模型启用 1M context window，请在模型 ID 后附加 `[1m]`。有关详细信息，请参阅[为第三方部署固定模型](33-模型配置.md#pin-models-for-third-party-deployments)。
