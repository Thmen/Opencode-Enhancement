...
<Tip>
  有关包含示例的快速入门指南，请参阅[使用 hooks 自动化工作流](24-使用hooks自动化工作流.md)。
</Tip>

Hooks 是用户定义的 shell 命令、HTTP 端点或 LLM 提示，在 Claude Code 生命周期中的特定点自动执行。使用此参考查找事件架构、配置选项、JSON 输入/输出格式以及异步 hooks、HTTP hooks 和 MCP 工具 hooks 等高级功能。如果您是第一次设置 hooks，请改为从[指南](24-使用hooks自动化工作流.md)开始。
...
| 托管策略设置                                                      | 组织范围   | 是，管理员控制      |
| [Plugin](37-创建插件.md) `hooks/hooks.json`                 | 启用插件时  | 是，与插件捆绑      |
| [Skill](45-使用skills扩展Claude.md) 或[代理](47-创建自定义subagents.md) frontmatter | 组件活跃时  | 是，在组件文件中定义   |

有关设置文件解析的详细信息，请参阅[设置](43-ClaudeCode设置.md)。企业管理员可以使用 `allowManagedHooksOnly` 来阻止用户、项目和插件 hooks。请参阅[Hook 配置](43-ClaudeCode设置.md#hook-configuration)。
...
[MCP](29-通过MCP将ClaudeCode连接到工具.md) 服务器工具在工具事件中显示为常规工具（`PreToolUse`、`PostToolUse`、`PostToolUseFailure`、`PermissionRequest`），因此您可以像匹配任何其他工具名称一样匹配它们。
...
* `${CLAUDE_PLUGIN_ROOT}`：插件的安装目录，用于与[插件](37-创建插件.md)捆绑的脚本。在每次插件更新时更改。
* `${CLAUDE_PLUGIN_DATA}`：插件的[持久数据目录](38-Plugins参考.md#persistent-data-directory)，用于应该在插件更新后保留的依赖项和状态。
...
<Tabs>
  ...
  <Tab title="插件脚本">
    ...
    有关创建插件 hooks 的详细信息，请参阅[插件组件参考](38-Plugins参考.md#hooks)。
  </Tab>
</Tabs>

### Skills 和代理中的 Hooks

除了设置文件和插件外，hooks 还可以使用 frontmatter 直接在[skills](45-使用skills扩展Claude.md)和[subagents](47-创建自定义subagents.md)中定义。这些 hooks 的范围限于组件的生命周期，仅在该组件活跃时运行。
...
| `permission_mode` | 当前[权限模式](35-配置权限.md#permission-modes)：`"default"`、`"plan"`、`"acceptEdits"`、`"auto"`、`"dontAsk"` 或 `"bypassPermissions"`。并非所有事件都接收此字段：请参阅下面每个事件的 JSON 示例以检查 |
...
您的 hook 的 stdout 必须仅包含 JSON 对象。如果您的 shell 配置文件在启动时打印文本，它可能会干扰 JSON 解析。请参阅故障排除指南中的[JSON 验证失败](24-使用hooks自动化工作流.md#json-validation-failed)。
...
有关扩展示例，包括 Bash 命令验证、提示过滤和自动批准脚本，请参阅指南中的[您可以自动化的内容](24-使用hooks自动化工作流.md#what-you-can-automate)以及[Bash 命令验证器参考实现](https://github.com/anthropics/claude-code/blob/main/examples/hooks/bash_command_validator_example.py)。
...
生成一个[subagent](47-创建自定义subagents.md)。
...
| 字段                         | 描述                                                                                                                        |
| :------------------------- | :------------------------------------------------------------------------------------------------------------------------ |
| `permissionDecision`       | `"allow"` 绕过权限系统，`"deny"` 防止工具调用，`"ask"` 提示用户确认。[拒绝和询问规则](35-配置权限.md#manage-permissions)在 hook 返回 `"allow"` 时仍然适用 |
...
当您运行 `claude --worktree` 或[subagent 使用 `isolation: "worktree"`](47-创建自定义subagents.md#choose-the-subagent-scope)时，Claude Code 使用 `git worktree` 创建隔离的工作副本。如果您配置 WorktreeCreate hook，它替换默认的 git 行为，让您使用不同的版本控制系统，如 SVN、Perforce 或 Mercurial。
...
此 `Stop` hook 使用详细提示检查三个条件，然后允许 Claude 停止。如果 `"ok"` 为 `false`，Claude 继续工作，提供的原因作为其下一条指令。`SubagentStop` hooks 使用相同的格式来评估[subagent](47-创建自定义subagents.md)是否应该停止：
...
有关故障排除常见问题，如 hooks 不触发、无限 Stop hook 循环或配置错误，请参阅指南中的[限制和故障排除](24-使用hooks自动化工作流.md#limitations-and-troubleshooting)。
