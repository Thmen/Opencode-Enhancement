[首页](README.md) / 以编程方式运行 Claude Code
...
要从 CLI 以编程方式运行 Claude Code，请使用 `-p` 传递您的提示和任何 [CLI 选项](10-CLI参考.md)：
...
将 `-p`（或 `--print`）标志添加到任何 `claude` 命令以非交互方式运行它。所有 [CLI 选项](10-CLI参考.md) 都适用于 `-p`，包括：

* `--continue` 用于 [继续对话](快速开始/01-ClaudeCode概述.md#continue-conversations)
* `--allowedTools` 用于 [自动批准工具](快速开始/01-ClaudeCode概述.md#auto-approve-tools)
* `--output-format` 用于 [获取结构化输出](快速开始/01-ClaudeCode概述.md#get-structured-output)
...
`--allowedTools` 标志使用 [权限规则语法](45-ClaudeCode设置.md#permission-rule-syntax)。尾部的 ` *` 启用前缀匹配，因此 `Bash(git diff *)` 允许任何以 `git diff` 开头的命令。空格在 `*` 之前很重要：没有它，`Bash(git diff*)` 也会匹配 `git diff-index`。

**Note:**
用户调用的 [skills](47-使用skills扩展Claude.md) 如 `/commit` 和 [内置命令](12-内置命令.md) 仅在交互模式下可用。在 `-p` 模式下，改为描述您想要完成的任务。
...
有关更多选项（包括 `--system-prompt` 以完全替换默认提示），请参阅 [系统提示标志](10-CLI参考.md#system-prompt-flags)。
...
* [CLI 参考](10-CLI参考.md)：所有 CLI 标志和选项
* [GitHub Actions](21-ClaudeCodeGitHubActions.md)：在 GitHub 工作流中使用 Agent SDK
* [GitLab CI/CD](22-ClaudeCodeGitLabCICD.md)：在 GitLab 管道中使用 Agent SDK
