...
**Tip:**
有关包含示例的快速入门指南，请参阅[使用 hooks 自动化工作流](26-使用hooks自动化工作流.md)。

Hooks 是用户定义的 shell 命令、HTTP 端点或 LLM 提示，在 Claude Code 生命周期中的特定点自动执行。使用此参考查找事件架构、配置选项、JSON 输入/输出格式以及异步 hooks、HTTP hooks 和 MCP 工具 hooks 等高级功能。如果您是第一次设置 hooks，请改为从[指南](26-使用hooks自动化工作流.md)开始。
...
下表总结了每个事件何时触发。[Hook 事件](快速开始/01-ClaudeCode概述.md#hook-events)部分记录了每个事件的完整输入架构和决定控制选项。
...
| `TeammateIdle`       | When an [agent team](01-协调ClaudeCode会话团队.md) teammate is about to go idle                                                                                     |
...
下面的[配置](快速开始/01-ClaudeCode概述.md#configuration)部分记录了完整的架构，每个[hook 事件](快速开始/01-ClaudeCode概述.md#hook-events)部分记录了您的命令接收的输入以及它可以返回的输出。

## 配置

Hooks 在 JSON 设置文件中定义。配置有三个嵌套级别：

1. 选择要响应的[hook 事件](快速开始/01-ClaudeCode概述.md#hook-events)，如 `PreToolUse` 或 `Stop`
2. 添加[匹配器组](快速开始/01-ClaudeCode概述.md#matcher-patterns)以过滤何时触发，如"仅针对 Bash 工具"
3. 定义一个或多个[hook 处理程序](快速开始/01-ClaudeCode概述.md#hook-handler-fields)以在匹配时运行

有关完整的演练和带注释的示例，请参阅上面的[Hook 如何解析](快速开始/01-ClaudeCode概述.md#how-a-hook-resolves)。
...
| 托管策略设置                                                      | 组织范围   | 是，管理员控制      |
| [Plugin](39-创建插件.md) `hooks/hooks.json`                 | 启用插件时  | 是，与插件捆绑      |
| [Skill](47-使用skills扩展Claude.md) 或[代理](49-创建自定义subagents.md) frontmatter | 组件活跃时  | 是，在组件文件中定义   |

有关设置文件解析的详细信息，请参阅[设置](45-ClaudeCode设置.md)。企业管理员可以使用 `allowManagedHooksOnly` 来阻止用户、项目和插件 hooks。请参阅[Hook 配置](45-ClaudeCode设置.md#hook-configuration)。
...
匹配器是一个正则表达式，所以 `Edit|Write` 匹配任一工具，`Notebook.*` 匹配任何以 Notebook 开头的工具。匹配器针对 Claude Code 在 stdin 上发送给您的 hook 的[JSON 输入](快速开始/01-ClaudeCode概述.md#hook-input-and-output)中的字段运行。对于工具事件，该字段是 `tool_name`。每个[hook 事件](快速开始/01-ClaudeCode概述.md#hook-events)部分列出了完整的匹配器值集和该事件的输入架构。
...
[MCP](31-通过MCP将ClaudeCode连接到工具.md) 服务器工具在工具事件中显示为常规工具（`PreToolUse`、`PostToolUse`、`PostToolUseFailure`、`PermissionRequest`），因此您可以像匹配任何其他工具名称一样匹配它们。
...
* **[命令 hooks](快速开始/01-ClaudeCode概述.md#command-hook-fields)**（`type: "command"`）：运行 shell 命令。您的脚本在 stdin 上接收事件的[JSON 输入](快速开始/01-ClaudeCode概述.md#hook-input-and-output)，并通过退出代码和 stdout 传回结果。
* **[HTTP hooks](快速开始/01-ClaudeCode概述.md#http-hook-fields)**（`type: "http"`）：将事件的 JSON 输入作为 HTTP POST 请求发送到 URL。端点通过使用与命令 hooks 相同的[JSON 输出格式](快速开始/01-ClaudeCode概述.md#json-output)的响应体传回结果。
* **[提示 hooks](快速开始/01-ClaudeCode概述.md#prompt-and-agent-hook-fields)**（`type: "prompt"`）：向 Claude 模型发送提示以进行单轮评估。模型返回 yes/no 决定作为 JSON。请参阅[基于提示的 hooks](快速开始/01-ClaudeCode概述.md#prompt-based-hooks)。
* **[代理 hooks](快速开始/01-ClaudeCode概述.md#prompt-and-agent-hook-fields)**（`type: "agent"`）：生成一个可以使用 Read、Grep 和 Glob 等工具来验证条件的 subagent，然后返回决定。请参阅[基于代理的 hooks](快速开始/01-ClaudeCode概述.md#agent-based-hooks)。
...
| `once`          | 否  | 如果为 `true`，每个会话仅运行一次，然后被移除。仅限 skills，不是代理。请参阅[skills 和代理中的 Hooks](快速开始/01-ClaudeCode概述.md#hooks-in-skills-and-agents) |

#### 命令 hook 字段

除了[通用字段](快速开始/01-ClaudeCode概述.md#common-fields)外，命令 hooks 还接受这些字段：

| 字段        | 必需 | 描述                                                                  |
| :-------- | :- | :------------------------------------------------------------------ |
| `command` | 是  | 要执行的 shell 命令                                                       |
| `async`   | 否  | 如果为 `true`，在后台运行而不阻止。请参阅[在后台运行 hooks](快速开始/01-ClaudeCode概述.md#run-hooks-in-the-background) |

#### HTTP hook 字段

除了[通用字段](快速开始/01-ClaudeCode概述.md#common-fields)外，HTTP hooks 还接受这些字段：
...
Claude Code 使用 `Content-Type: application/json` 将 hook 的[JSON 输入](快速开始/01-ClaudeCode概述.md#hook-input-and-output)作为 POST 请求体发送。响应体使用与命令 hooks 相同的[JSON 输出格式](快速开始/01-ClaudeCode概述.md#json-output)。
...
除了[通用字段](快速开始/01-ClaudeCode概述.md#common-fields)外，提示和代理 hooks 还接受这些字段：
...
* `${CLAUDE_PLUGIN_ROOT}`：插件的安装目录，用于与[插件](39-创建插件.md)捆绑的脚本。在每次插件更新时更改。
* `${CLAUDE_PLUGIN_DATA}`：插件的[持久数据目录](40-Plugins参考.md#persistent-data-directory)，用于应该在插件更新后保留的依赖项和状态。
...
有关创建插件 hooks 的详细信息，请参阅[插件组件参考](40-Plugins参考.md#hooks)。

### Skills 和代理中的 Hooks

除了设置文件和插件外，hooks 还可以使用 frontmatter 直接在[skills](47-使用skills扩展Claude.md)和[subagents](49-创建自定义subagents.md)中定义。这些 hooks 的范围限于组件的生命周期，仅在该组件活跃时运行。
...
命令 hooks 通过 stdin 接收 JSON 数据，并通过退出代码、stdout 和 stderr 传回结果。HTTP hooks 接收相同的 JSON 作为 POST 请求体，并通过 HTTP 响应体传回结果。本部分涵盖所有事件通用的字段和行为。每个事件在[Hook 事件](快速开始/01-ClaudeCode概述.md#hook-events)下的部分包括其特定的输入架构和决定控制选项。

### 通用输入字段

所有 hook 事件都接收这些字段作为 JSON，除了每个[hook 事件](快速开始/01-ClaudeCode概述.md#hook-events)部分中记录的事件特定字段。对于命令 hooks，此 JSON 通过 stdin 到达。对于 HTTP hooks，它作为 POST 请求体到达。
...
| `permission_mode` | 当前[权限模式](37-配置权限.md#permission-modes)：`"default"`、`"plan"`、`"acceptEdits"`、`"auto"`、`"dontAsk"` 或 `"bypassPermissions"`。并非所有事件都接收此字段：请参阅下面每个事件的 JSON 示例以检查 |
...
`tool_name` 和 `tool_input` 字段是事件特定的。每个[hook 事件](快速开始/01-ClaudeCode概述.md#hook-events)部分记录了该事件的额外字段。

### 退出代码输出

您的 hook 命令的退出代码告诉 Claude Code 操作是否应该继续、被阻止或被忽略。

**退出 0** 表示成功。Claude Code 解析 stdout 以获取[JSON 输出字段](快速开始/01-ClaudeCode概述.md#json-output)。JSON 输出仅在退出 0 时处理。对于大多数事件，stdout 仅在详细模式（`Ctrl+O`）中显示。例外是 `UserPromptSubmit` 和 `SessionStart`，其中 stdout 作为 Claude 可以看到和作用的上下文添加。

**退出 2** 表示阻止错误。Claude Code 忽略 stdout 和其中的任何 JSON。相反，stderr 文本被反馈给 Claude 作为错误消息。效果取决于事件：`PreToolUse` 阻止工具调用，`UserPromptSubmit` 拒绝提示，等等。有关完整列表，请参阅[每个事件的退出代码 2 行为](快速开始/01-ClaudeCode概述.md#exit-code-2-behavior-per-event)。
...
* **2xx 带 JSON 体**：成功，使用与命令 hooks 相同的[JSON 输出](快速开始/01-ClaudeCode概述.md#json-output)架构解析
* **非 2xx 状态**：非阻止错误，执行继续
* **连接失败或超时**：非阻止错误，执行继续
...
退出代码让您允许或阻止，但 JSON 输出提供更细粒度的控制。与其使用代码 2 退出来阻止，不如退出 0 并将 JSON 对象打印到 stdout。Claude Code 从该 JSON 读取特定字段以控制行为，包括[决定控制](快速开始/01-ClaudeCode概述.md#decision-control)以阻止、允许或升级给用户。

**Note:**
您必须为每个 hook 选择一种方法，而不是两种：要么单独使用退出代码进行信号传递，要么退出 0 并打印 JSON 以进行结构化控制。Claude Code 仅在退出 0 时处理 JSON。如果您退出 2，任何 JSON 都会被忽略。

您的 hook 的 stdout 必须仅包含 JSON 对象。如果您的 shell 配置文件在启动时打印文本，它可能会干扰 JSON 解析。请参阅故障排除指南中的[JSON 验证失败](26-使用hooks自动化工作流.md#json-validation-failed)。
...
#### PreToolUse
使用 `hookSpecificOutput` 以获得更丰富的控制：允许、拒绝或升级给用户。您还可以在运行前修改工具输入或为 Claude 注入额外上下文。有关完整的选项集，请参阅[PreToolUse 决定控制](快速开始/01-ClaudeCode概述.md#pretooluse-decision-control)。
...
#### PermissionRequest
使用 `hookSpecificOutput` 代表用户允许或拒绝权限请求。允许时，您还可以修改工具的输入或应用权限规则，以便用户不会再次被提示。有关完整的选项集，请参阅[PermissionRequest 决定控制](快速开始/01-ClaudeCode概述.md#permissionrequest-decision-control)。
...
有关扩展示例，包括 Bash 命令验证、提示过滤和自动批准脚本，请参阅指南中的[您可以自动化的内容](26-使用hooks自动化工作流.md#what-you-can-automate)以及[Bash 命令验证器参考实现](https://github.com/anthropics/claude-code/blob/main/examples/hooks/bash_command_validator_example.py)。
...
在 Claude Code 启动新会话或恢复现有会话时运行。用于加载开发上下文，如现有问题或代码库的最近更改，或设置环境变量。对于不需要脚本的静态上下文，请改用[CLAUDE.md](使用 Claude Code/03-Claude如何记住你的项目.md)。
...
除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，SessionStart hooks 还接收 `source`、`model` 和可选的 `agent_type`。`source` 字段指示会话如何启动：新会话为 `"startup"`，恢复会话为 `"resume"`，`/clear` 后为 `"clear"`，压缩后为 `"compact"`。`model` 字段包含模型标识符。如果您使用 `claude --agent <name>` 启动 Claude Code，`agent_type` 字段包含代理名称。
...
您的 hook 脚本打印到 stdout 的任何文本都作为 Claude 的上下文添加。除了所有 hooks 可用的[JSON 输出字段](快速开始/01-ClaudeCode概述.md#json-output)外，您还可以返回这些事件特定字段：
...
**Note:**
`CLAUDE_ENV_FILE` 可用于 SessionStart、[CwdChanged](快速开始/01-ClaudeCode概述.md#cwdchanged) 和 [FileChanged](快速开始/01-ClaudeCode概述.md#filechanged) hooks。其他 hook 类型无法访问此变量。
...
除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，InstructionsLoaded hooks 还接收这些字段：
...
除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，UserPromptSubmit hooks 还接收包含用户提交的文本的 `prompt` 字段。
...
`UserPromptSubmit` hooks 可以控制用户提示是否被处理并添加上下文。所有[JSON 输出字段](快速开始/01-ClaudeCode概述.md#json-output)都可用。
...
在 Claude 创建工具参数后和处理工具调用之前运行。在工具名称上匹配：`Bash`、`Edit`、`Write`、`Read`、`Glob`、`Grep`、`Agent`、`WebFetch`、`WebSearch` 和任何[MCP 工具名称](快速开始/01-ClaudeCode概述.md#match-mcp-tools)。

使用[PreToolUse 决定控制](快速开始/01-ClaudeCode概述.md#pretooluse-decision-control)来允许、拒绝或要求使用工具的权限。

#### PreToolUse 输入

除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，PreToolUse hooks 还接收 `tool_name`、`tool_input` 和 `tool_use_id`。`tool_input` 字段取决于工具：
...
生成一个[subagent](49-创建自定义subagents.md)。
...
| 字段                         | 描述                                                                                                                        |
| :------------------------- | :------------------------------------------------------------------------------------------------------------------------ |
| `permissionDecision`       | `"allow"` 绕过权限系统，`"deny"` 防止工具调用，`"ask"` 提示用户确认。[拒绝和询问规则](37-配置权限.md#manage-permissions)在 hook 返回 `"allow"` 时仍然适用 |
...
在向用户显示权限对话框时运行。使用[PermissionRequest 决定控制](快速开始/01-ClaudeCode概述.md#permissionrequest-decision-control)代表用户允许或拒绝。
...
`PermissionRequest` hooks 可以允许或拒绝权限请求。除了所有 hooks 可用的[JSON 输出字段](快速开始/01-ClaudeCode概述.md#json-output)外，您的 hook 脚本可以返回一个 `decision` 对象，其中包含这些事件特定字段：

| 字段                   | 描述                                                                             |
| :------------------- | :----------------------------------------------------------------------------- |
| `behavior`           | `"allow"` 授予权限，`"deny"` 拒绝它                                                    |
| `updatedInput`       | 仅对 `"allow"`：在执行前修改工具的输入参数                                                     |
| `updatedPermissions` | 仅对 `"allow"`：应用权限规则更新的[权限更新条目](快速开始/01-ClaudeCode概述.md#permission-update-entries)数组，例如添加允许规则或更改会话权限模式 |
...
`updatedPermissions` 输出字段和[`permission_suggestions` 输入字段](快速开始/01-ClaudeCode概述.md#permissionrequest-input)都使用相同的条目对象数组。每个条目都有一个 `type` 来确定其其他字段，以及一个 `destination` 来控制更改的写入位置。
...
`PostToolUse` hooks 可以在工具执行后向 Claude 提供反馈。除了所有 hooks 可用的[JSON 输出字段](快速开始/01-ClaudeCode概述.md#json-output)外，您的 hook 脚本可以返回这些事件特定字段：
...
| `updatedMCPToolOutput` | 仅对[MCP 工具](快速开始/01-ClaudeCode概述.md#match-mcp-tools)：用提供的值替换工具的输出 |
...
`PostToolUseFailure` hooks 可以在工具失败后向 Claude 提供上下文。除了所有 hooks 可用的[JSON 输出字段](快速开始/01-ClaudeCode概述.md#json-output)外，您的 hook 脚本可以返回这些事件特定字段：
...
除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，Notification hooks 还接收 `message` 和通知文本、可选的 `title` 和 `notification_type` 指示哪个类型触发。
...
Notification hooks 无法阻止或修改通知。除了所有 hooks 可用的[JSON 输出字段](快速开始/01-ClaudeCode概述.md#json-output)外，您可以返回 `additionalContext` 以向对话添加上下文：
...
除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，SubagentStart hooks 还接收 `agent_id` 和 subagent 的唯一标识符以及 `agent_type` 和代理名称（内置代理如 `"Bash"`、`"Explore"`、`"Plan"` 或自定义代理名称）。
...
SubagentStart hooks 无法阻止 subagent 创建，但它们可以向 subagent 注入上下文。除了所有 hooks 可用的[JSON 输出字段](快速开始/01-ClaudeCode概述.md#json-output)外，您可以返回：
...
除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，SubagentStop hooks 还接收 `stop_hook_active`、`agent_id`、`agent_type`、`agent_transcript_path` 和 `last_assistant_message`。`agent_type` 字段是用于匹配器过滤的值。`transcript_path` 是主会话的成绩单，而 `agent_transcript_path` 是 subagent 自己的成绩单，存储在嵌套的 `subagents/` 文件夹中。`last_assistant_message` 字段包含 subagent 最终响应的文本内容，因此 hooks 可以访问它而无需解析成绩单文件。
...
SubagentStop hooks 使用与[Stop hooks](快速开始/01-ClaudeCode概述.md#stop-decision-control)相同的决定控制格式。

### Stop

在主 Claude Code 代理完成响应时运行。如果停止是由于用户中断，则不运行。API 错误触发[StopFailure](快速开始/01-ClaudeCode概述.md#stopfailure)。

#### Stop 输入

除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，Stop hooks 还接收 `stop_hook_active` 和 `last_assistant_message`。`stop_hook_active` 字段在 Claude Code 已经作为 stop hook 的结果继续时为 `true`。检查此值或处理成绩单以防止 Claude Code 无限运行。`last_assistant_message` 字段包含 Claude 最终响应的文本内容，因此 hooks 可以访问它而无需解析成绩单文件。
...
`Stop` 和 `SubagentStop` hooks 可以控制 Claude 是否继续。除了所有 hooks 可用的[JSON 输出字段](快速开始/01-ClaudeCode概述.md#json-output)外，您的 hook 脚本可以返回这些事件特定字段：
...
当轮次因 API 错误而结束时运行，而不是[Stop](快速开始/01-ClaudeCode概述.md#stop)。输出和退出代码被忽略。使用此来记录失败、发送警报或在 Claude 因速率限制、身份验证问题或其他 API 错误而无法完成响应时采取恢复操作。

#### StopFailure 输入

除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，StopFailure hooks 还接收 `error`、可选的 `error_details` 和可选的 `last_assistant_message`。`error` 字段标识错误类型，用于匹配器过滤。
...
当[代理团队](01-协调ClaudeCode会话团队.md)队友在完成其轮次后即将空闲时运行。使用此来强制执行质量门，如要求通过 lint 检查或验证输出文件存在。
...
除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，TeammateIdle hooks 还接收 `teammate_name` 和 `team_name`。
...
当任务被标记为已完成时运行。这在两种情况下触发：当任何代理通过 TaskUpdate 工具显式标记任务为已完成时，或当[代理团队](01-协调ClaudeCode会话团队.md)队友完成其轮次且有进行中的任务时。使用此来强制执行完成标准，如通过测试或 lint 检查，然后任务才能关闭。
...
除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，TaskCompleted hooks 还接收 `task_id`、`task_subject` 和可选的 `task_description`、`teammate_name` 和 `team_name`。
...
除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，ConfigChange hooks 还接收 `source` 和可选的 `file_path`。`source` 字段指示哪种配置类型更改，`file_path` 提供被修改的特定文件的路径。
...
当会话期间工作目录更改时运行，例如当 Claude 执行 `cd` 命令时。使用此来对目录更改做出反应：重新加载环境变量、激活项目特定的工具链或自动运行设置脚本。与[FileChanged](快速开始/01-ClaudeCode概述.md#filechanged)配对，用于[direnv](https://direnv.net/)等管理每个目录环境的工具。

CwdChanged hooks 可以访问 `CLAUDE_ENV_FILE`。写入该文件的变量持久化到会话的后续 Bash 命令中，就像在[SessionStart hooks](快速开始/01-ClaudeCode概述.md#persist-environment-variables)中一样。仅支持 `type: "command"` hooks。

CwdChanged 不支持匹配器，在每次目录更改时触发。

#### CwdChanged 输入

除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，CwdChanged hooks 还接收 `old_cwd` 和 `new_cwd`。
...
除了所有 hooks 可用的[JSON 输出字段](快速开始/01-ClaudeCode概述.md#json-output)外，CwdChanged hooks 还可以返回 `watchPaths` 来动态设置[FileChanged](快速开始/01-ClaudeCode概述.md#filechanged)监视的文件路径：
...
FileChanged hooks 可以访问 `CLAUDE_ENV_FILE`。写入该文件的变量持久化到会话的后续 Bash 命令中，就像在[SessionStart hooks](快速开始/01-ClaudeCode概述.md#persist-environment-variables)中一样。仅支持 `type: "command"` hooks。

#### FileChanged 输入

除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，FileChanged hooks 还接收 `file_path` 和 `event`。
...
除了所有 hooks 可用的[JSON 输出字段](快速开始/01-ClaudeCode概述.md#json-output)外，FileChanged hooks 还可以返回 `watchPaths` 来动态更新监视的文件路径：
...
当您运行 `claude --worktree` 或[subagent 使用 `isolation: "worktree"`](49-创建自定义subagents.md#choose-the-subagent-scope)时，Claude Code 使用 `git worktree` 创建隔离的工作副本。如果您配置 WorktreeCreate hook，它替换默认的 git 行为，让您使用不同的版本控制系统，如 SVN、Perforce 或 Mercurial。
...
除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，WorktreeCreate hooks 还接收 `name` 字段。这是新 worktree 的 slug 标识符，由用户指定或自动生成（例如，`bold-oak-a3f2`）。
...
[WorktreeCreate](快速开始/01-ClaudeCode概述.md#worktreecreate) 的清理对应物。此 hook 在 worktree 被移除时触发，要么当您退出 `--worktree` 会话并选择移除它时，要么当具有 `isolation: "worktree"` 的 subagent 完成时。对于基于 git 的 worktrees，Claude 使用 `git worktree remove` 自动处理清理。如果您为非 git 版本控制系统配置了 WorktreeCreate hook，将其与 WorktreeRemove hook 配对以处理清理。没有它，worktree 目录留在磁盘上。
...
除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，WorktreeRemove hooks 还接收 `worktree_path` 字段，这是被移除的 worktree 的绝对路径。
...
除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，PreCompact hooks 还接收 `trigger` 和 `custom_instructions`。对于 `manual`，`custom_instructions` 包含用户传入 `/compact` 的内容。对于 `auto`，`custom_instructions` 为空。
...
除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，PostCompact hooks 还接收 `trigger` 和 `compact_summary`。`compact_summary` 字段包含压缩操作生成的对话摘要。
...
除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，SessionEnd hooks 还接收 `reason` 字段，指示会话为何结束。有关所有值，请参阅上面的[原因表](快速开始/01-ClaudeCode概述.md#sessionend)。
...
除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，Elicitation hooks 还接收 `mcp_server_name`、`message` 和可选的 `mode`、`url`、`elicitation_id` 和 `requested_schema` 字段。
...
除了[通用输入字段](快速开始/01-ClaudeCode概述.md#common-input-fields)外，ElicitationResult hooks 还接收 `mcp_server_name`、`action` 和可选的 `mode`、`elicitation_id` 和 `content` 字段。
...
此 `Stop` hook 使用详细提示检查三个条件，然后允许 Claude 停止。如果 `"ok"` 为 `false`，Claude 继续工作，提供的原因作为其下一条指令。`SubagentStop` hooks 使用相同的格式来评估[subagent](49-创建自定义subagents.md)是否应该停止：
...
将 `type` 设置为 `"agent"` 并提供 `prompt` 字符串。配置字段与[提示 hooks](快速开始/01-ClaudeCode概述.md#prompt-hook-configuration)相同，但超时更长：
...
有关故障排除常见问题，如 hooks 不触发、无限 Stop hook 循环或配置错误，请参阅指南中的[限制和故障排除](26-使用hooks自动化工作流.md#limitations-and-troubleshooting)。
