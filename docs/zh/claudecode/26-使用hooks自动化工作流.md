...
对于需要判断而不是确定性规则的决策，你也可以使用 [基于提示的 hooks](快速开始/01-ClaudeCode概述.md#prompt-based-hooks) 或 [基于代理的 hooks](快速开始/01-ClaudeCode概述.md#agent-based-hooks)，它们使用 Claude 模型来评估条件。

有关扩展 Claude Code 的其他方式，请参阅 [skills](47-使用skills扩展Claude.md)，用于为 Claude 提供额外的指令和可执行命令，[subagents](49-创建自定义subagents.md) 用于在隔离的上下文中运行任务，以及 [plugins](39-创建插件.md) 用于打包要在项目间共享的扩展。

**Tip:**
本指南涵盖常见用例和入门方法。有关完整的事件架构、JSON 输入/输出格式和异步 hooks 和 MCP 工具 hooks 等高级功能，请参阅 [Hooks 参考](25-Hooks参考.md)。

## 设置你的第一个 hook

要创建 hook，请将 `hooks` 块添加到 [设置文件](快速开始/01-ClaudeCode概述.md#configure-hook-location)。本演练创建一个桌面通知 hook，这样每当 Claude 等待你的输入而不是监视终端时，你都会收到警报。

#### 将 hook 添加到你的设置
打开 `~/.claude/settings.json` 并添加一个 `Notification` hook。下面的示例使用 `osascript` 用于 macOS；有关 Linux 和 Windows 命令，请参阅 [在 Claude 需要输入时获得通知](快速开始/01-ClaudeCode概述.md#get-notified-when-claude-needs-input)。
...
Hooks 让你在 Claude Code 生命周期中的关键点运行代码：编辑后格式化文件、在执行前阻止命令、在 Claude 需要输入时发送通知、在会话开始时注入上下文等。有关完整的 hook 事件列表，请参阅 [Hooks 参考](25-Hooks参考.md#hook-lifecycle)。

每个示例都包含一个现成的配置块，你可以将其添加到 [设置文件](快速开始/01-ClaudeCode概述.md#configure-hook-location)。最常见的模式：

* [在 Claude 需要输入时获得通知](快速开始/01-ClaudeCode概述.md#get-notified-when-claude-needs-input)
* [编辑后自动格式化代码](快速开始/01-ClaudeCode概述.md#auto-format-code-after-edits)
* [阻止对受保护文件的编辑](快速开始/01-ClaudeCode概述.md#block-edits-to-protected-files)
* [压缩后重新注入上下文](快速开始/01-ClaudeCode概述.md#re-inject-context-after-compaction)
* [审计配置更改](快速开始/01-ClaudeCode概述.md#audit-configuration-changes)
* [当目录或文件更改时重新加载环境](快速开始/01-ClaudeCode概述.md#reload-environment-when-directory-or-files-change)
* [自动批准特定权限提示](快速开始/01-ClaudeCode概述.md#auto-approve-specific-permission-prompts)
...
你可以用任何产生动态输出的命令替换 `echo`，如 `git log --oneline -5` 来显示最近的提交。有关在每个会话开始时注入上下文，请考虑改用 [CLAUDE.md](使用 Claude Code/03-Claude如何记住你的项目.md)。有关环境变量，请参阅参考中的 [`CLAUDE_ENV_FILE`](25-Hooks参考.md#persist-environment-variables)。
...
匹配器按配置类型过滤：`user_settings`、`project_settings`、`local_settings`、`policy_settings` 或 `skills`。要阻止更改生效，以代码 2 退出或返回 `{"decision": "block"}`。有关完整的输入架构，请参阅 [ConfigChange 参考](25-Hooks参考.md#configchange)。
...
有关输入架构、`watchPaths` 输出和 `CLAUDE_ENV_FILE` 详情，请参阅 [CwdChanged](25-Hooks参考.md#cwdchanged) 和 [FileChanged](25-Hooks参考.md#filechanged) 参考条目。
...
保持匹配器尽可能狭窄。匹配 `.*` 或留下匹配器为空会自动批准每个权限提示，包括文件写入和 shell 命令。有关完整的决策字段集，请参阅 [PermissionRequest 参考](25-Hooks参考.md#permissionrequest-decision-control)。
...
* `"type": "http"`：将事件数据 POST 到 URL。请参阅 [HTTP hooks](快速开始/01-ClaudeCode概述.md#http-hooks)。
* `"type": "prompt"`：单轮 LLM 评估。请参阅 [基于提示的 hooks](快速开始/01-ClaudeCode概述.md#prompt-based-hooks)。
* `"type": "agent"`：具有工具访问权限的多轮验证。请参阅 [基于代理的 hooks](快速开始/01-ClaudeCode概述.md#agent-based-hooks)。
...
你的脚本可以解析该 JSON 并对任何这些字段进行操作。`UserPromptSubmit` hooks 获取 `prompt` 文本，`SessionStart` hooks 获取 `source`（启动、恢复、清除、压缩），等等。有关共享字段，请参阅参考中的 [常见输入字段](25-Hooks参考.md#common-input-fields)，以及每个事件的部分了解事件特定的架构。
...
返回 `"allow"` 跳过交互式提示但不覆盖 [权限规则](37-配置权限.md#manage-permissions)。如果拒绝规则与工具调用匹配，即使你的 hook 返回 `"allow"`，调用也会被阻止。如果询问规则匹配，用户仍然会被提示。这意味着来自任何设置范围的拒绝规则，包括 [托管设置](45-ClaudeCode设置.md#settings-files)，总是优先于 hook 批准。

其他事件使用不同的决策模式。例如，`PostToolUse` 和 `Stop` hooks 使用顶级 `decision: "block"` 字段，而 `PermissionRequest` 使用 `hookSpecificOutput.decision.behavior`。有关按事件的完整分解，请参阅参考中的 [摘要表](25-Hooks参考.md#decision-control)。

对于 `UserPromptSubmit` hooks，改用 `additionalContext` 将文本注入到 Claude 的上下文中。基于提示的 hooks（`type: "prompt"`）处理输出的方式不同：请参阅 [基于提示的 hooks](快速开始/01-ClaudeCode概述.md#prompt-based-hooks)。
...
#### 匹配 MCP 工具
MCP 工具使用与内置工具不同的命名约定：`mcp__<server>__<tool>`，其中 `<server>` 是 MCP 服务器名称，`<tool>` 是它提供的工具。例如，`mcp__github__search_repositories` 或 `mcp__filesystem__read_file`。使用正则表达式匹配器来针对来自特定服务器的所有工具，或使用 `mcp__.*__write.*` 之类的模式跨服务器匹配。有关完整的示例列表，请参阅参考中的 [匹配 MCP 工具](25-Hooks参考.md#match-mcp-tools)。
...
有关完整的匹配器语法，请参阅 [Hooks 参考](25-Hooks参考.md#configuration)。
...
| [Skill](47-使用skills扩展Claude.md) 或 [agent](49-创建自定义subagents.md) frontmatter | 当 skill 或 agent 处于活动状态时 | 是，在组件文件中定义   |

在 Claude Code 中运行 [`/hooks`](25-Hooks参考.md#the-hooks-menu) 以浏览所有按事件分组的配置 hooks。要一次禁用所有 hooks，在设置文件中设置 `"disableAllHooks": true`。
...
有关完整的配置选项，请参阅参考中的 [基于提示的 hooks](25-Hooks参考.md#prompt-based-hooks)。
...
有关完整的配置选项，请参阅参考中的 [基于代理的 hooks](25-Hooks参考.md#agent-based-hooks)。
...
端点应使用与命令 hooks 相同的 [输出格式](25-Hooks参考.md#json-output) 返回 JSON 响应体。要阻止工具调用，返回 2xx 响应，包含适当的 `hookSpecificOutput` 字段。HTTP 状态代码本身无法阻止操作。

标头值支持使用 `$VAR_NAME` 或 `${VAR_NAME}` 语法的环境变量插值。仅解析 `allowedEnvVars` 数组中列出的变量；所有其他 `$VAR` 引用保持为空。

有关完整的配置选项和响应处理，请参阅参考中的 [HTTP hooks](25-Hooks参考.md#http-hook-fields)。
...
* `PermissionRequest` hooks 不在 [非交互模式](24-以编程方式运行ClaudeCode.md)（`-p`）中触发。对于自动化权限决策，使用 `PreToolUse` hooks。
* `Stop` hooks 在 Claude 完成响应时触发，而不仅仅在任务完成时。它们不在用户中断时触发。API 错误触发 [StopFailure](25-Hooks参考.md#stopfailure) 代替。
...
* [Hooks 参考](25-Hooks参考.md)：完整的事件架构、JSON 输出格式、异步 hooks 和 MCP 工具 hooks
* [安全考虑](25-Hooks参考.md#security-considerations)：在共享或生产环境中部署 hooks 之前查看
