...
企业管理员可以在[托管或策略设置](43-ClaudeCode设置.md#settings-files)中使用 `availableModels` 来限制用户可以选择的模型。
...
要在 Opus 4.6 和 Sonnet 4.6 上禁用自适应推理并恢复到之前的固定思考预算，请设置 `CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING=1`。禁用时，这些模型使用由 `MAX_THINKING_TOKENS` 控制的固定预算。请参阅[环境变量](17-环境变量.md)。
...
要完全禁用 1M 上下文，请设置 `CLAUDE_CODE_DISABLE_1M_CONTEXT=1`。这会从模型选择器中删除 1M 模型变体。请参阅[环境变量](17-环境变量.md)。
...
1. 在[状态行](46-自定义你的状态行.md)中（如果已配置）
...
| `ANTHROPIC_DEFAULT_HAIKU_MODEL`  | 用于 `haiku` 的模型，或[后台功能](11-有效管理成本.md#background-token-usage) |
| `CLAUDE_CODE_SUBAGENT_MODEL`     | 用于 [subagents](47-创建自定义subagents.md) 的模型                       |

注意：`ANTHROPIC_SMALL_FAST_MODEL` 已弃用，改为使用 `ANTHROPIC_DEFAULT_HAIKU_MODEL`。

### 为第三方部署固定模型

通过 [Bedrock](02-AmazonBedrock上的ClaudeCode.md)、[Vertex AI](21-GoogleVertexAI上的ClaudeCode.md) 或 [Foundry](30-ClaudeCodeonMicrosoftFoundry.md) 部署 Claude Code 时，在向用户推出前固定模型版本。
...
在您的[设置文件](43-ClaudeCode设置.md#settings-files)中设置 `modelOverrides`：
