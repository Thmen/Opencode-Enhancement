...
企业管理员可以在[托管或策略设置](45-ClaudeCode设置.md#settings-files)中使用 `availableModels` 来限制用户可以选择的模型。
...
模型选择器中的"默认"选项不受 `availableModels` 影响。它始终保持可用，并代表系统的运行时默认值[基于用户的订阅层级](快速开始/01-ClaudeCode概述.md#default-model-setting)。
...
* **Skill 和 subagent frontmatter**：在 [skill](47-使用skills扩展Claude.md#frontmatter-reference) 或 [subagent](49-创建自定义subagents.md#supported-frontmatter-fields) markdown 文件中设置 `effort` 以在该 skill 或 subagent 运行时覆盖工作量级别
...
要在 Opus 4.6 和 Sonnet 4.6 上禁用自适应推理并恢复到之前的固定思考预算，请设置 `CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING=1`。禁用时，这些模型使用由 `MAX_THINKING_TOKENS` 控制的固定预算。请参阅[环境变量](19-环境变量.md)。
...
要完全禁用 1M 上下文，请设置 `CLAUDE_CODE_DISABLE_1M_CONTEXT=1`。这会从模型选择器中删除 1M 模型变体。请参阅[环境变量](19-环境变量.md)。
...
1. 在[状态行](48-自定义你的状态行.md)中（如果已配置）
...
| `ANTHROPIC_DEFAULT_HAIKU_MODEL`  | 用于 `haiku` 的模型，或[后台功能](13-有效管理成本.md#background-token-usage) |
| `CLAUDE_CODE_SUBAGENT_MODEL`     | 用于 [subagents](49-创建自定义subagents.md) 的模型                       |

注意：`ANTHROPIC_SMALL_FAST_MODEL` 已弃用，改为使用 `ANTHROPIC_DEFAULT_HAIKU_MODEL`。

### 为第三方部署固定模型

通过 [Bedrock](02-AmazonBedrock上的ClaudeCode.md)、[Vertex AI](23-GoogleVertexAI上的ClaudeCode.md) 或 [Foundry](32-ClaudeCodeonMicrosoftFoundry.md) 部署 Claude Code 时，在向用户推出前固定模型版本。
...
要为固定模型启用[扩展上下文](快速开始/01-ClaudeCode概述.md#extended-context)，请在 `ANTHROPIC_DEFAULT_OPUS_MODEL` 或 `ANTHROPIC_DEFAULT_SONNET_MODEL` 中的模型 ID 后附加 `[1m]`：
...
在您的[设置文件](45-ClaudeCode设置.md#settings-files)中设置 `modelOverrides`：
