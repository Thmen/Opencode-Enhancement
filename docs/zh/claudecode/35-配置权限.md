...
Claude Code 支持多种权限模式来控制工具的批准方式。请参阅[权限模式](使用 Claude Code/04-选择权限模式.md)了解何时使用每种模式。在您的[设置文件](43-ClaudeCode设置.md#settings-files)中设置 `defaultMode`：
...
<Warning>
  `bypassPermissions` 模式跳过权限提示。对 `.git`、`.claude`、`.vscode` 和 `.idea` 目录的写入仍然会提示确认，以防止意外损坏存储库状态和本地配置。对 `.claude/commands`、`.claude/agents` 和 `.claude/skills` 的写入被豁免，不会提示，因为 Claude 在创建技能、子代理和命令时经常在那里写入。仅在隔离环境（如容器或虚拟机）中使用此模式，其中 Claude Code 无法造成损害。管理员可以通过在[托管设置](快速开始/01-ClaudeCode概述.md#managed-settings)中将 `disableBypassPermissionsMode` 设置为 `"disable"` 来防止此模式。
</Warning>

为了防止使用 `bypassPermissions` 或 `auto` 模式，在任何[设置文件](43-ClaudeCode设置.md#settings-files)中将 `permissions.disableBypassPermissionsMode` 或 `disableAutoMode` 设置为 `"disable"`。这些在[托管设置](快速开始/01-ClaudeCode概述.md#managed-settings)中最有用，因为它们无法被覆盖。
...
<Warning>
  Read 和 Edit deny 规则适用于 Claude 的内置文件工具，不适用于 Bash 子进程。`Read(./.env)` deny 规则阻止 Read 工具，但不会阻止 Bash 中的 `cat .env`。为了获得阻止所有进程访问路径的 OS 级别强制执行，请[启用沙箱](39-Sandboxing.md)。
</Warning>
...
使用 `Agent(AgentName)` 规则来控制 Claude 可以使用哪些[子代理](47-创建自定义subagents.md)：
...
[Claude Code hooks](24-使用hooks自动化工作流.md)提供了一种方法来注册自定义 shell 命令以在运行时执行权限评估。当 Claude Code 进行工具调用时，PreToolUse hooks 在权限提示之前运行。hook 输出可以拒绝工具调用、强制提示或跳过提示以让调用继续。

跳过提示不会绕过权限规则。Deny 和 ask 规则在 hook 返回 `"allow"` 后仍然被评估，因此匹配的 deny 规则仍然会阻止调用。这保留了[管理权限](快速开始/01-ClaudeCode概述.md#manage-permissions)中描述的 deny 优先级，包括在托管设置中设置的 deny 规则。

阻止 hook 也优先于 allow 规则。以退出代码 2 退出的 hook 在权限规则被评估之前停止工具调用，因此即使 allow 规则会让调用继续，阻止也适用。要运行所有 Bash 命令而无需提示，除了您想要阻止的少数几个，将 `"Bash"` 添加到您的 allow 列表，并注册一个 PreToolUse hook 来拒绝那些特定命令。请参见[阻止对受保护文件的编辑](24-使用hooks自动化工作流.md#block-edits-to-protected-files)以获取您可以调整的 hook 脚本。
...
* **会话期间**：使用 `/add-dir` 命令
* **持久配置**：添加到[设置文件](43-ClaudeCode设置.md#settings-files)中的 `additionalDirectories`

其他目录中的文件遵循与原始工作目录相同的权限规则：它们变为可读的而无需提示，文件编辑权限遵循当前权限模式。

## 权限如何与沙箱交互

权限和[沙箱](39-Sandboxing.md)是互补的安全层：
...
对于需要对 Claude Code 配置进行集中控制的组织，管理员可以部署无法被用户或项目设置覆盖的托管设置。这些策略设置遵循与常规设置文件相同的格式，可以通过 MDM/OS 级别策略、托管设置文件或[服务器托管设置](42-配置服务器管理的设置（公开测试版）.md)传递。有关传递机制和文件位置，请参见[设置文件](43-ClaudeCode设置.md#settings-files)。
...
| `allowManagedMcpServersOnly`                   | 当为 `true` 时，仅尊重来自托管设置的 `allowedMcpServers`。`deniedMcpServers` 仍然从所有来源合并。请参见[托管 MCP 配置](29-通过MCP将ClaudeCode连接到工具.md#managed-mcp-configuration) |
| `blockedMarketplaces`                          | 市场来源的黑名单。在下载前检查被阻止的来源，因此它们永远不会接触文件系统。请参见[托管市场限制](36-创建和分发pluginmarketplace.md#managed-marketplace-restrictions)                |
...
| `strictKnownMarketplaces`                      | 控制用户可以添加哪些插件市场。请参见[托管市场限制](36-创建和分发pluginmarketplace.md#managed-marketplace-restrictions)                                      |

<Note>
  对[远程控制](平台和集成/04-使用RemoteControl从任何设备继续本地会话.md)和[网络会话](07-ClaudeCodeontheweb.md)的访问不由托管设置密钥控制。在 Team 和 Enterprise 计划上，管理员在[Claude Code 管理设置](https://claude.ai/admin-settings/claude-code)中启用或禁用这些功能。
</Note>

## 配置自动模式分类器

[自动模式](使用 Claude Code/04-选择权限模式.md#eliminate-prompts-with-auto-mode)使用分类器模型来决定每个操作是否可以安全运行而无需提示。开箱即用，它仅信任工作目录和（如果存在）当前存储库的远程。诸如推送到您公司的源代码控制组织或写入团队云存储桶之类的操作将被阻止为潜在的数据泄露。`autoMode` 设置块让您告诉分类器您的组织信任哪些基础设施。
...
权限规则遵循与所有其他 Claude Code 设置相同的[设置优先级](43-ClaudeCode设置.md#settings-precedence)：
...
* [Settings](43-ClaudeCode设置.md)：完整的配置参考，包括权限设置表
* [Sandboxing](39-Sandboxing.md)：Bash 命令的 OS 级文件系统和网络隔离
* [Authentication](04-身份验证.md)：设置用户对 Claude Code 的访问
* [Security](41-安全性.md)：安全保障和最佳实践
* [Hooks](24-使用hooks自动化工作流.md)：自动化工作流并扩展权限评估
