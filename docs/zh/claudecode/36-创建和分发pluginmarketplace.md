[首页](README.md) / 创建和分发 plugin marketplace
...
想要从现有 marketplace 安装 plugins？请参阅[发现和安装预构建的 plugins](16-通过市场发现和安装预构建插件.md)。

## 概述

创建和分发 marketplace 涉及：

1. **创建 plugins**：使用命令、agents、hooks、MCP servers 或 LSP servers 构建一个或多个 plugins。本指南假设你已经有要分发的 plugins；有关如何创建 plugins 的详细信息，请参阅[创建 plugins](37-创建插件.md)。
2. **创建 marketplace 文件**：定义一个 `marketplace.json`，列出你的 plugins 及其位置（请参阅[创建 marketplace 文件](快速开始/01-ClaudeCode概述.md#create-the-marketplace-file)）。
3. **托管 marketplace**：推送到 GitHub、GitLab 或其他 git 主机（请参阅[托管和分发 marketplaces](快速开始/01-ClaudeCode概述.md#host-and-distribute-marketplaces)）。
4. **与用户共享**：用户使用 `/plugin marketplace add` 添加你的 marketplace 并安装单个 plugins（请参阅[发现和安装 plugins](16-通过市场发现和安装预构建插件.md)）。
...
要了解更多关于 plugins 可以做什么的信息，包括 hooks、agents、MCP servers 和 LSP servers，请参阅 [Plugins](37-创建插件.md)。

<Note>
  **plugins 如何安装**：当用户安装 plugin 时，Claude Code 将 plugin 目录复制到缓存位置。这意味着 plugins 无法使用 `../shared-utils` 之类的路径引用其目录外的文件，因为这些文件不会被复制。

  如果你需要在 plugins 之间共享文件，请使用符号链接（在复制期间会被跟踪）。有关详细信息，请参阅 [Plugin 缓存和文件解析](38-Plugins参考.md#plugin-caching-and-file-resolution)。
</Note>
...
每个 plugin 条目至少需要一个 `name` 和 `source`（从哪里获取它）。有关所有可用字段，请参阅下面的[完整架构](快速开始/01-ClaudeCode概述.md#marketplace-schema)。
...
| `owner`   | object | Marketplace 维护者信息（[见下面的字段](快速开始/01-ClaudeCode概述.md#owner-fields)）                                                                  |                |
...
`plugins` 数组中的每个 plugin 条目描述一个 plugin 及其位置。你可以包含 [plugin manifest 架构](38-Plugins参考.md#plugin-manifest-schema)中的任何字段（如 `description`、`version`、`author`、`commands`、`hooks` 等），加上这些 marketplace 特定的字段：`source`、`category`、`tags` 和 `strict`。
...
| `source` | string\|object | 从哪里获取 plugin（见下面的 [Plugin 源](快速开始/01-ClaudeCode概述.md#plugin-sources)）                                             |
...
| `strict`      | boolean | 控制 `plugin.json` 是否是组件定义的权威（默认：true）。见下面的 [Strict 模式](快速开始/01-ClaudeCode概述.md#strict-mode)。 |
...
<Note>
  相对路径仅在用户通过 Git（GitHub、GitLab 或 git URL）添加你的 marketplace 时有效。如果用户通过直接 URL 添加你的 marketplace 到 `marketplace.json` 文件，相对路径将无法正确解析。对于基于 URL 的分发，请改用 GitHub、npm 或 git URL 源。有关详细信息，请参阅[故障排除](快速开始/01-ClaudeCode概述.md#plugins-with-relative-paths-fail-in-url-based-marketplaces)。
</Note>
...
* **`${CLAUDE_PLUGIN_ROOT}`**：在 hooks 和 MCP server 配置中使用此变量来引用 plugin 安装目录中的文件。这是必要的，因为 plugins 在安装时被复制到缓存位置。对于应该在 plugin 更新后保留的依赖项或状态，请改用 [`${CLAUDE_PLUGIN_DATA}`](38-Plugins参考.md#persistent-data-directory)。
* **`strict: false`**：由于这设置为 false，plugin 不需要自己的 `plugin.json`。marketplace 条目定义了一切。见下面的 [Strict 模式](快速开始/01-ClaudeCode概述.md#strict-mode)。
...
有关完整的添加命令范围（GitHub、Git URL、本地路径、远程 URL），请参阅[添加 marketplaces](16-通过市场发现和安装预构建插件.md#add-marketplaces)。
...
有关完整的配置选项，请参阅 [Plugin 设置](43-ClaudeCode设置.md#plugin-settings)。
...
对于需要严格控制 plugin 源的组织，管理员可以使用托管设置中的 [`strictKnownMarketplaces`](43-ClaudeCode设置.md#strictknownmarketplaces) 设置限制用户允许添加哪些 plugin marketplaces。
...
<Note>
  `strictKnownMarketplaces` 限制用户可以添加的内容，但不会自行注册 marketplaces。要使允许的 marketplaces 自动可用而无需用户运行 `/plugin marketplace add`，请在同一 `managed-settings.json` 中将其与 [`extraKnownMarketplaces`](43-ClaudeCode设置.md#extraknownmarketplaces) 配对。见[同时使用两者](43-ClaudeCode设置.md#strictknownmarketplaces)。
</Note>
...
因为 `strictKnownMarketplaces` 在[托管设置](43-ClaudeCode设置.md#settings-files)中设置，个别用户和项目配置无法覆盖这些限制。

有关完整的配置详细信息，包括所有支持的源类型和与 `extraKnownMarketplaces` 的比较，请参阅 [strictKnownMarketplaces 参考](43-ClaudeCode设置.md#strictknownmarketplaces)。
...
要为你的 plugins 支持"稳定"和"最新"发布渠道，你可以设置两个指向同一 repo 的不同 refs 或 SHAs 的 marketplaces。然后，你可以通过[托管设置](43-ClaudeCode设置.md#settings-files)将两个 marketplaces 分配给不同的用户组。
...
有关完整的 plugin 测试工作流，请参阅[本地测试你的 plugins](37-创建插件.md#test-your-plugins-locally)。有关技术故障排除，请参阅 [Plugins 参考](38-Plugins参考.md)。
...
| `plugins[0].source: Path contains ".."`           | 源路径包含 `..`                         | 使用相对于 marketplace 根目录的路径，不包含 `..`。见[相对路径](快速开始/01-ClaudeCode概述.md#relative-paths) |
...
**解决方案**：见 [Plugin 缓存和文件解析](38-Plugins参考.md#plugin-caching-and-file-resolution) 了解解决方法，包括符号链接和目录重组。

有关其他调试工具和常见问题，请参阅[调试和开发工具](38-Plugins参考.md#debugging-and-development-tools)。

## 另见

* [发现和安装预构建的 plugins](16-通过市场发现和安装预构建插件.md) - 从现有 marketplaces 安装 plugins
* [Plugins](37-创建插件.md) - 创建你自己的 plugins
* [Plugins 参考](38-Plugins参考.md) - 完整的技术规范和架构
* [Plugin 设置](43-ClaudeCode设置.md#plugin-settings) - Plugin 配置选项
* [strictKnownMarketplaces 参考](43-ClaudeCode设置.md#strictknownmarketplaces) - 托管 marketplace 限制
