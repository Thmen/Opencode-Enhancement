...
想要安装现有插件？请参阅[发现和安装插件](16-通过市场发现和安装预构建插件.md)。有关完整的技术规范，请参阅[插件参考](38-Plugins参考.md)。
...
<Tip>
  从 `.claude/` 中的独立配置开始进行快速迭代，然后在准备好共享时[转换为插件](快速开始/01-ClaudeCode概述.md#convert-existing-configurations-to-plugins)。
</Tip>
...
* Claude Code [已安装并已认证](快速开始/02-快速开始.md#step-1-install-claude-code)
* Claude Code 版本 1.0.33 或更高版本（运行 `claude --version` 检查）

<Note>
  如果你没有看到 `/plugin` 命令，请将 Claude Code 更新到最新版本。有关升级说明，请参阅[故障排除](51-故障排除.md)。
</Note>
...
<Steps>
  ...
  <Step title="创建插件清单">
    ...
    | `version`     | 使用[语义版本控制](38-Plugins参考.md#version-management)跟踪发布。 |
    | `author`      | 可选。有助于归属。                                                    |

    有关 `homepage`、`repository` 和 `license` 等其他字段，请参阅[完整清单架构](38-Plugins参考.md#plugin-manifest-schema)。
  </Step>
  ...
  <Step title="添加 skill 参数">
    ...
    Claude 将按名字问候你。有关向 skills 传递参数的更多信息，请参阅 [Skills](45-使用skills扩展Claude.md#pass-arguments-to-skills)。
  </Step>
</Steps>
...
<Tip>
  `--plugin-dir` 标志对开发和测试很有用。当你准备好与他人共享你的插件时，请参阅[创建和分发插件市场](36-创建和分发pluginmarketplace.md)。
</Tip>
...
| `settings.json`   | 插件根 | 启用插件时应用的默认[设置](43-ClaudeCode设置.md)     |

<Note>
  **后续步骤**：准备好添加更多功能了吗？跳转到[开发更复杂的插件](快速开始/01-ClaudeCode概述.md#develop-more-complex-plugins)以添加 agents、hooks、MCP servers 和 LSP servers。有关所有插件组件的完整技术规范，请参阅[插件参考](38-Plugins参考.md)。
</Note>
...
插件可以包含 [Agent Skills](45-使用skills扩展Claude.md) 以扩展 Claude 的功能。Skills 是模型调用的：Claude 根据任务上下文自动使用它们。
...
安装插件后，运行 `/reload-plugins` 以加载 Skills。有关完整的 Skill 编写指南，包括渐进式披露和工具限制，请参阅 [Agent Skills](45-使用skills扩展Claude.md)。
...
有关完整的 LSP 配置选项，请参阅 [LSP servers](38-Plugins参考.md#lsp-servers)。

### 使用你的插件提供默认设置

插件可以在插件根目录包含一个 `settings.json` 文件，以在启用插件时应用默认配置。目前仅支持 `agent` 键。

设置 `agent` 激活插件的[自定义 agents](47-创建自定义subagents.md) 之一作为主线程，应用其系统提示、工具限制和模型。这让插件在启用时通过改变 Claude Code 的默认行为方式。
...
对于具有许多组件的插件，按功能组织你的目录结构。有关完整的目录布局和组织模式，请参阅[插件目录结构](38-Plugins参考.md#plugin-directory-structure)。
...
3. **使用验证和调试工具**：有关 CLI 命令和故障排除技术，请参阅[调试和开发工具](38-Plugins参考.md#debugging-and-development-tools)
...
2. **版本化你的插件**：在你的 `plugin.json` 中使用[语义版本控制](38-Plugins参考.md#version-management)
3. **创建或使用市场**：通过[插件市场](36-创建和分发pluginmarketplace.md)分发以供安装
4. **与他人测试**：在更广泛分发之前让团队成员测试插件

一旦你的插件在市场中，其他人可以使用[发现和安装插件](16-通过市场发现和安装预构建插件.md)中的说明安装它。
...
<Note>
  有关完整的技术规范、调试技术和分发策略，请参阅[插件参考](38-Plugins参考.md)。
</Note>
...
* [发现和安装插件](16-通过市场发现和安装预构建插件.md)：浏览市场并安装插件
* [配置团队市场](16-通过市场发现和安装预构建插件.md#configure-team-marketplaces)：为你的团队设置存储库级别的插件

### 对于插件开发者

* [创建和分发市场](36-创建和分发pluginmarketplace.md)：打包和共享你的插件
* [插件参考](38-Plugins参考.md)：完整的技术规范
* 深入了解特定的插件组件：
