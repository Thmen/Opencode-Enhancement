...
Claude Code 支持多种权限模式来控制工具的批准方式。请参阅[权限模式](使用 Claude Code/04-选择权限模式.md)了解何时使用每种模式。在您的[设置文件](45-ClaudeCode设置.md#settings-files)中设置 `defaultMode`：
...
为了防止使用 `bypassPermissions` 或 `auto` 模式，在任何[设置文件](45-ClaudeCode设置.md#settings-files)中将 `permissions.disableBypassPermissionsMode` 或 `disableAutoMode` 设置为 `"disable"`。这些在[托管设置](快速开始/01-ClaudeCode概述.md#managed-settings)中最有用，因为它们无法被覆盖。
...
**Warning:**
Read 和 Edit deny 规则适用于 Claude 的内置文件工具，不适用于 Bash 子进程。`Read(./.env)` deny 规则阻止 Read 工具，但不会阻止 Bash 中的 `cat .env`。为了获得阻止所有进程访问路径的 OS 级别强制执行，请[启用沙箱](41-Sandboxing.md)。
...
使用 `Agent(AgentName)` 规则来控制 Claude 可以使用哪些[子代理](49-创建自定义subagents.md)：
...
[Claude Code hooks](26-使用hooks自动化工作流.md)提供了一种方法来注册自定义 shell 命令以在运行时执行权限评估。当 Claude Code 进行工具调用时，PreToolUse hooks 在权限提示之前运行。hook 输出可以拒绝工具调用、强制提示或跳过提示以让调用继续。
...
阻止 hook 也优先于 allow 规则。以退出代码 2 退出的 hook 在权限规则被评估之前停止工具调用，因此即使 allow 规则会让调用继续，阻止也适用。要运行所有 Bash 命令而无需提示，除了您想要阻止的少数几个，将 `"Bash"` 添加到您的 allow 列表，并注册一个 PreToolUse hook 来拒绝那些特定命令。请参见[阻止对受保护文件的编辑](26-使用hooks自动化工作流.md#block-edits-to-protected-files)以获取您可以调整的 hook 脚本。
...
* **会话期间**：使用 `/add-dir` 命令
* **持久配置**：添加到[设置文件](45-ClaudeCode设置.md#settings-files)中的 `additionalDirectories`

其他目录中的文件遵循与原始工作目录相同的权限规则：它们变为可读的而无需提示，文件编辑权限遵循当前权限模式。

## 权限如何与沙箱交互

权限和[沙箱](41-Sandboxing.md)是互补的安全层：
...
对于需要对 Claude Code 配置进行集中控制的组织，管理员可以部署无法被用户或项目设置覆盖的托管设置。这些策略设置遵循与常规设置文件相同的格式，可以通过 MDM/OS 级别策略、托管设置文件或[服务器托管设置](44-配置服务器管理的设置（公开测试版）.md)传递。有关传递机制和文件位置，请参见[设置文件](45-ClaudeCode设置.md#settings-files)。
...
| `allowManagedMcpServersOnly`                   | 当为 `true` 时，仅尊重来自托管设置的 `allowedMcpServers`。`deniedMcpServers` 仍然从所有来源合并。请参见[托管 MCP 配置](31-通过MCP将ClaudeCode连接到工具.md#managed-mcp-configuration) |
| `blockedMarketplaces`                          | 市场来源的黑名单。在下载前检查被阻止的来源，因此它们永远不会接触文件系统。请参见[托管市场限制](38-创建和分发pluginmarketplace.md#managed-marketplace-restrictions)                |
...
| `strictKnownMarketplaces`                      | 控制用户可以添加哪些插件市场。请参见[托管市场限制](38-创建和分发pluginmarketplace.md#managed-marketplace-restrictions)                                      |

**Note:**
对[远程控制](平台和集成/04-使用RemoteControl从任何设备继续本地会话.md)和[网络会话](09-ClaudeCodeontheweb.md)的访问不由托管设置密钥控制。在 Team 和 Enterprise 计划上，管理员在[Claude Code 管理设置](https://claude.ai/admin-settings/claude-code)中启用或禁用这些功能。
...
权限规则遵循与所有其他 Claude Code 设置相同的[设置优先级](45-ClaudeCode设置.md#settings-precedence)：
...
* [Settings](45-ClaudeCode设置.md)：完整的配置参考，包括权限设置表
* [Sandboxing](41-Sandboxing.md)：Bash 命令的 OS 级文件系统和网络隔离
* [Authentication](04-身份验证.md)：设置用户对 Claude Code 的访问
* [Security](43-安全性.md)：安全保障和最佳实践
* [Hooks](26-使用hooks自动化工作流.md)：自动化工作流并扩展权限评估
