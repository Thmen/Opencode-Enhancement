...
**Tip:**
想要安装插件？请参阅[发现和安装插件](18-通过市场发现和安装预构建插件.md)。如需创建插件，请参阅[Plugins](39-创建插件.md)。如需分发插件，请参阅[Plugin marketplaces](38-创建和分发pluginmarketplace.md)。
...
有关完整详情，请参阅[Skills](47-使用skills扩展Claude.md)。
...
有关完整详情，请参阅[Subagents](49-创建自定义subagents.md)。
...
Plugin hooks 响应与[用户定义的 hooks](25-Hooks参考.md)相同的生命周期事件：
...
| `managed` | [Managed settings](45-ClaudeCode设置.md#settings-files) | 托管 plugins（只读，仅更新）       |

Plugins 使用与其他 Claude Code 配置相同的范围系统。有关安装说明和范围标志，请参阅[安装 plugins](18-通过市场发现和安装预构建插件.md#install-plugins)。有关范围的完整说明，请参阅[Configuration scopes](45-ClaudeCode设置.md#configuration-scopes)。
...
清单是可选的。如果省略，Claude Code 会自动发现[默认位置](快速开始/01-ClaudeCode概述.md#file-locations-reference)中的组件，并从目录名称派生 plugin 名称。当您需要提供元数据或自定义组件路径时，使用清单。
...
| `userConfig`   | object                | 用户可配置的值，在启用时提示。请参阅[用户配置](快速开始/01-ClaudeCode概述.md#user-configuration)                                                          | 见下文                                   |
| `channels`     | array                 | 消息注入的频道声明（Telegram、Slack、Discord 风格）。请参阅[Channels](快速开始/01-ClaudeCode概述.md#channels)                                          | 见下文                                   |
...
当您从最后一个安装了插件的范围卸载插件时，数据目录会自动删除。`/plugin` 界面显示目录大小并在删除前提示。CLI 默认删除；传递 [`--keep-data`](快速开始/01-ClaudeCode概述.md#plugin-uninstall) 以保留它。
...
| **设置**          | `settings.json`              | 启用 plugin 时应用的默认配置。目前仅支持[`agent`](49-创建自定义subagents.md)设置 |
...
| `--keep-data`         | 保留插件的[持久数据目录](快速开始/01-ClaudeCode概述.md#persistent-data-directory) |        |
| `-h, --help`          | 显示命令帮助                                    |        |
...
如果您的 plugin 在[市场](38-创建和分发pluginmarketplace.md)目录中，您可以通过 `marketplace.json` 管理版本，而不是从 `plugin.json` 中省略 `version` 字段。

***

## 另请参阅

* [Plugins](39-创建插件.md) - 教程和实际用法
* [Plugin marketplaces](38-创建和分发pluginmarketplace.md) - 创建和管理市场
* [Skills](47-使用skills扩展Claude.md) - Skill 开发详情
* [Subagents](49-创建自定义subagents.md) - Agent 配置和功能
* [Hooks](25-Hooks参考.md) - 事件处理和自动化
* [MCP](31-通过MCP将ClaudeCode连接到工具.md) - 外部工具集成
* [Settings](45-ClaudeCode设置.md) - Plugins 的配置选项
