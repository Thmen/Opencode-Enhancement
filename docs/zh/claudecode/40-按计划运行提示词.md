...
计划任务让 Claude 按间隔自动重新运行提示词。使用它们来轮询部署、监督 PR、检查长时间运行的构建，或在会话中稍后提醒自己做某事。要对事件进行实时反应而不是轮询，请参阅 [Channels](05-使用channels将事件推送到运行中的会话.md)：您的 CI 可以直接将失败推送到会话中。

任务是会话范围的：它们存在于当前 Claude Code 进程中，当您退出时就会消失。对于需要在重启后继续运行的持久调度，请使用 [Cloud](52-在网络上安排任务.md) 或 [Desktop](13-使用ClaudeCodeDesktop.md#schedule-recurring-tasks) 计划任务，或 [GitHub Actions](19-ClaudeCodeGitHubActions.md)。
...
`/loop` [捆绑技能](45-使用skills扩展Claude.md#bundled-skills)是计划重复提示词的最快方式。传递可选的间隔和提示词，Claude 会设置一个在后台运行的 cron 作业，同时会话保持打开。
...
重复任务在创建后 3 天自动过期。任务最后触发一次，然后删除自己。这限制了被遗忘的循环可以运行多长时间。如果您需要重复任务持续更长时间，请在过期前取消并重新创建它，或使用 [Cloud 计划任务](52-在网络上安排任务.md) 或 [Desktop 计划任务](13-使用ClaudeCodeDesktop.md#schedule-recurring-tasks) 进行持久调度。
...
在您的环境中设置 `CLAUDE_CODE_DISABLE_CRON=1` 以完全禁用调度程序。cron 工具和 `/loop` 变得不可用，任何已计划的任务都停止触发。有关禁用标志的完整列表，请参阅[环境变量](17-环境变量.md)。
...
* [Cloud 计划任务](52-在网络上安排任务.md)：在 Anthropic 管理的基础设施上运行
* [GitHub Actions](19-ClaudeCodeGitHubActions.md)：在 CI 中使用 `schedule` 触发器
* [Desktop 计划任务](13-使用ClaudeCodeDesktop.md#schedule-recurring-tasks)：在您的机器上本地运行
