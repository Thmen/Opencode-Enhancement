...
有关详细的权限配置，请参阅 [Permissions](37-配置权限.md)。

### 内置保护

为了降低代理系统中的风险：

* **沙箱化 bash 工具**：使用文件系统和网络隔离的 [Sandbox](41-Sandboxing.md) bash 命令，减少权限提示同时保持安全性。使用 `/sandbox` 启用以定义 Claude Code 可以自主工作的边界
...
* **命令黑名单**：默认阻止从网络获取任意内容的风险命令，如 `curl` 和 `wget`。显式允许时，请注意 [权限模式限制](37-配置权限.md#tool-specific-permission-rules)
...
* **安全凭证存储**：API 密钥和令牌已加密。请参阅 [Credential Management](04-身份验证.md#credential-management)
...
有关在 IDE 中运行 Claude Code 的更多信息，请参阅 [VS Code security and privacy](平台和集成/06-在VSCode中使用ClaudeCode.md#security-and-privacy)。

## 云执行安全性

使用 [Claude Code on the web](09-ClaudeCodeontheweb.md) 时，会实施额外的安全控制：
...
有关云执行的更多详情，请参阅 [Claude Code on the web](09-ClaudeCodeontheweb.md)。

[Remote Control](平台和集成/04-使用RemoteControl从任何设备继续本地会话.md) 会话的工作方式不同：Web 界面连接到在您本地机器上运行的 Claude Code 进程。所有代码执行和文件访问都保持本地，任何本地 Claude Code 会话期间流动的相同数据通过 TLS 上的 Anthropic API 传输。不涉及云 VM 或沙箱。连接使用多个短期的、范围狭窄的凭证，每个凭证限制于特定目的并独立过期，以限制任何单个受损凭证的影响范围。
...
* 在批准前审查所有建议的更改
* 为敏感存储库使用项目特定的权限设置
* 考虑使用 [devcontainers](17-开发容器.md) 以获得额外隔离
* 使用 `/permissions` 定期审计您的权限设置

### 团队安全

* 使用 [managed settings](45-ClaudeCode设置.md#settings-files) 来强制执行组织标准
* 通过版本控制共享批准的权限配置
* 培训团队成员了解安全最佳实践
* 通过 [OpenTelemetry metrics](34-监控.md) 监控 Claude Code 使用情况
* 使用 [`ConfigChange` hooks](25-Hooks参考.md#configchange) 审计或阻止会话期间的设置更改
...
* [Sandboxing](41-Sandboxing.md) - bash 命令的文件系统和网络隔离
* [Permissions](37-配置权限.md) - 配置权限和访问控制
* [Monitoring usage](34-监控.md) - 跟踪和审计 Claude Code 活动
* [Development containers](17-开发容器.md) - 安全、隔离的环境
