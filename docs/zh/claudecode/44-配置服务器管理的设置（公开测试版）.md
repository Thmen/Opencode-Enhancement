...
Claude Code 支持两种集中配置方法。服务器管理的设置从 Anthropic 的服务器传递配置。[端点管理的设置](45-ClaudeCode设置.md#settings-files)通过本机操作系统策略（macOS 托管首选项、Windows 注册表）或托管设置文件直接部署到设备。

| 方法                                            | 最适合                   | 安全模型                             |
| :-------------------------------------------- | :-------------------- | :------------------------------- |
| **服务器管理的设置**                                  | 没有 MDM 的组织，或非托管设备上的用户 | 在身份验证时从 Anthropic 的服务器传递的设置      |
| **[端点管理的设置](45-ClaudeCode设置.md#settings-files)** | 具有 MDM 或端点管理的组织       | 通过 MDM 配置文件、注册表策略或托管设置文件部署到设备的设置 |
...
#### 定义您的设置
将您的配置添加为 JSON。支持 [`settings.json` 中可用的所有设置](45-ClaudeCode设置.md#available-settings)，包括 [hooks](25-Hooks参考.md)、[环境变量](19-环境变量.md) 和[仅限托管的设置](37-配置权限.md#managed-only-settings)，如 `allowManagedPermissionRulesOnly`。
...
要配置 [auto mode](使用 Claude Code/04-选择权限模式.md#eliminate-prompts-with-auto-mode) 分类器，使其了解您的组织信任的存储库、存储桶和域：
...
由于 hooks 执行 shell 命令，用户在应用前会看到[安全批准对话框](快速开始/01-ClaudeCode概述.md#security-approval-dialogs)。有关 `autoMode` 条目如何影响分类器阻止的内容以及关于 `allow` 和 `soft_deny` 字段的重要警告，请参阅[配置 auto mode 分类器](37-配置权限.md#configure-the-auto-mode-classifier)。
...
要确认设置正在被应用，请要求用户重新启动 Claude Code。如果配置包含触发[安全批准对话框](快速开始/01-ClaudeCode概述.md#security-approval-dialogs)的设置，用户会在启动时看到描述托管设置的提示。您还可以通过让用户运行 `/permissions` 来验证托管权限规则是否处于活动状态，以查看其有效的权限规则。
...
* [MCP 服务器配置](31-通过MCP将ClaudeCode连接到工具.md#managed-mcp-configuration)无法通过服务器管理的设置分发。

## 设置传递

### 设置优先级

服务器管理的设置和[端点管理的设置](45-ClaudeCode设置.md#settings-files)都占据 Claude Code [设置层次结构](45-ClaudeCode设置.md#settings-precedence)中的最高层。没有其他设置级别可以覆盖它们，包括命令行参数。当两者都存在时，服务器管理的设置优先，端点管理的设置不被使用。
...
* Google Vertex AI
* Microsoft Foundry
* 通过 `ANTHROPIC_BASE_URL` 或 [LLM gateways](30-LLMgateway配置.md) 的自定义 API 端点
...
要检测运行时配置更改，请使用 [`ConfigChange` hooks](25-Hooks参考.md#configchange) 来记录修改或在未授权的更改生效前阻止它们。

为了获得更强的强制执行保证，请在已在 MDM 解决方案中注册的设备上使用[端点管理的设置](45-ClaudeCode设置.md#settings-files)。

## 另请参阅

用于管理 Claude Code 配置的相关页面：

* [Settings](45-ClaudeCode设置.md)：完整的配置参考，包括所有可用的设置
* [Endpoint-managed settings](45-ClaudeCode设置.md#settings-files)：由 IT 部门部署到设备的托管设置
