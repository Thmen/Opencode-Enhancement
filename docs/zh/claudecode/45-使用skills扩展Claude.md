...
<Note>
  对于内置命令（如 `/help` 和 `/compact`），请参阅[内置命令参考](10-内置命令.md)。
...
捆绑 skills 随 Claude Code 一起提供，在每个会话中都可用。与[内置命令](10-内置命令.md)不同，内置命令直接执行固定逻辑，捆绑 skills 是基于提示的：它们为 Claude 提供详细的剧本，让它使用其工具来编排工作。这意味着捆绑 skills 可以生成并行代理、读取文件并适应你的代码库。
...
| `/loop [interval] <prompt>` | 在会话保持打开状态时按间隔重复运行提示。适用于轮询部署、监督 PR 或定期重新运行另一个 skill。示例：`/loop 5m check if the deploy finished`。请参阅[按计划运行提示](40-按计划运行提示词.md)                                                                                                                  |
...
| 位置 | 路径                                        | 适用于        |
| :- | :---------------------------------------- | :--------- |
| 企业 | 请参阅[托管设置](43-ClaudeCode设置.md#settings-files) | 你的组织中的所有用户 |
...
| `hooks`                    | 否  | 限定于此 skill 生命周期的 hooks。有关配置格式，请参阅 [Skills 和代理中的 Hooks](23-Hooks参考.md#hooks-in-skills-and-agents)。 |
...
<Note>
  在常规会话中，skill 描述被加载到上下文中，以便 Claude 知道什么可用，但完整 skill 内容仅在调用时加载。[预加载 skills 的 Subagents](47-创建自定义subagents.md#preload-skills-into-subagents) 的工作方式不同：完整 skill 内容在启动时注入。
</Note>
...
Skills 和 [subagents](47-创建自定义subagents.md) 以两个方向协同工作：
...
使用 `context: fork`，你在你的 skill 中编写任务并选择一个代理类型来执行它。对于反向（定义使用 skills 作为参考资料的自定义 subagent），请参阅 [Subagents](47-创建自定义subagents.md#preload-skills-into-subagents)。
...
默认情况下，Claude 可以调用任何没有设置 `disable-model-invocation: true` 的 skill。定义 `allowed-tools` 的 Skills 在 skill 处于活动状态时向 Claude 授予对这些工具的访问权限，无需每次使用批准。你的[权限设置](35-配置权限.md)仍然管理所有其他工具的基线批准行为。内置命令如 `/compact` 和 `/init` 不能通过 Skill 工具获得。
...
**使用[权限规则](35-配置权限.md)允许或拒绝特定 skills**：
...
* **插件**：在你的[插件](37-创建插件.md)中创建 `skills/` 目录
* **托管**：通过[托管设置](43-ClaudeCode设置.md#settings-files)部署组织范围内
...
* **[Subagents](47-创建自定义subagents.md)**：将任务委派给专门的代理
* **[Plugins](37-创建插件.md)**：打包和分发 skills 与其他扩展
* **[Hooks](23-Hooks参考.md)**：围绕工具事件自动化工作流
* **[Memory](使用 Claude Code/03-Claude如何记住你的项目.md)**：管理 CLAUDE.md 文件以获得持久上下文
* **[Built-in commands](10-内置命令.md)**：内置 `/` 命令的参考
* **[Permissions](35-配置权限.md)**：控制工具和 skill 访问
