...
将 `statusLine` 字段添加到你的用户设置（`~/.claude/settings.json`，其中 `~` 是你的主目录）或[项目设置](43-ClaudeCode设置.md#settings-files)。将 `type` 设置为 `"command"` 并将 `command` 指向脚本路径或内联 shell 命令。有关创建脚本的完整演练，请参阅[逐步构建状态行](快速开始/01-ClaudeCode概述.md#build-a-status-line-step-by-step)。
...
<Note>使用[`/statusline`](快速开始/01-ClaudeCode概述.md#use-the-statusline-command)和你想要的内容的描述会自动为你配置所有这些。</Note>
...
| `vim.mode`                                                                | 启用[vim 模式](25-交互模式.md#vim-editor-mode)时的当前 vim 模式（`NORMAL` 或 `INSERT`） |
