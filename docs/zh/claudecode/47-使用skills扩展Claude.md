[首页](README.md) / 使用 skills 扩展 Claude
...
**Note:**
对于内置命令（如 `/help` 和 `/compact`），请参阅[内置命令参考](12-内置命令.md)。

**自定义命令已合并到 skills 中。** `.claude/commands/deploy.md` 中的文件和 `.claude/skills/deploy/SKILL.md` 中的 skill 都会创建 `/deploy` 并以相同的方式工作。你现有的 `.claude/commands/` 文件继续工作。Skills 添加了可选功能：支持文件的目录、[控制你或 Claude 是否调用它们](快速开始/01-ClaudeCode概述.md#control-who-invokes-a-skill)的 frontmatter，以及 Claude 在相关时自动加载它们的能力。

Claude Code skills 遵循 [Agent Skills](https://agentskills.io) 开放标准，该标准适用于多个 AI 工具。Claude Code 使用额外功能扩展了该标准，如[调用控制](快速开始/01-ClaudeCode概述.md#control-who-invokes-a-skill)、[subagent 执行](快速开始/01-ClaudeCode概述.md#run-skills-in-a-subagent)和[动态上下文注入](快速开始/01-ClaudeCode概述.md#inject-dynamic-context)。

## 捆绑 skills

捆绑 skills 随 Claude Code 一起提供，在每个会话中都可用。与[内置命令](12-内置命令.md)不同，内置命令直接执行固定逻辑，捆绑 skills 是基于提示的：它们为 Claude 提供详细的剧本，让它使用其工具来编排工作。这意味着捆绑 skills 可以生成并行代理、读取文件并适应你的代码库。

你调用捆绑 skills 的方式与调用任何其他 skill 相同：输入 `/` 后跟 skill 名称。在下表中，`<arg>` 表示必需的参数，`[arg]` 表示可选参数。

| Skill                       | 目的                                                                                                                                                                                                                                                  |
| :-------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/batch <instruction>`      | 在代码库中并行编排大规模更改。研究代码库，将工作分解为 5 到 30 个独立单元，并呈现计划。获得批准后，在隔离的 [git worktree](使用 Claude Code/02-常见工作流程.md#run-parallel-claude-code-sessions-with-git-worktrees) 中为每个单元生成一个后台代理。每个代理实现其单元、运行测试并打开拉取请求。需要 git 存储库。示例：`/batch migrate src/ from Solid to React` |
...
| `/loop [interval] <prompt>` | 在会话保持打开状态时按间隔重复运行提示。适用于轮询部署、监督 PR 或定期重新运行另一个 skill。示例：`/loop 5m check if the deploy finished`。请参阅[按计划运行提示](42-按计划运行提示词.md)                                                                                                                  |
...
| 位置 | 路径                                        | 适用于        |
| :- | :---------------------------------------- | :--------- |
| 企业 | 请参阅[托管设置](45-ClaudeCode设置.md#settings-files) | 你的组织中的所有用户 |
...
`SKILL.md` 包含主要说明，是必需的。其他文件是可选的，让你构建更强大的 skills：Claude 要填写的模板、显示预期格式的示例输出、Claude 可以执行的脚本或详细的参考文档。从你的 `SKILL.md` 中引用支持文件，以便 Claude 知道每个文件包含什么以及何时加载它。有关更多详细信息，请参阅[添加支持文件](快速开始/01-ClaudeCode概述.md#add-supporting-files)。

**Note:**
`.claude/commands/` 中的文件仍然有效，并支持相同的 [frontmatter](快速开始/01-ClaudeCode概述.md#frontmatter-reference)。建议使用 Skills，因为它们支持额外的功能，如支持文件。
...
**Note:**
来自 `--add-dir` 目录的 CLAUDE.md 文件默认不加载。要加载它们，请设置 `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1`。请参阅[从其他目录加载](使用 Claude Code/03-Claude如何记住你的项目.md#load-from-additional-directories)。
...
你的 `SKILL.md` 可以包含任何内容，但思考你想如何调用该 skill（由你、由 Claude 或两者）以及你想在哪里运行它（内联或在 subagent 中）有助于指导要包含的内容。对于复杂的 skills，你也可以[添加支持文件](快速开始/01-ClaudeCode概述.md#add-supporting-files)以保持主 skill 的专注。
...
| `effort`                   | 否  | 当此 skill 处于活动状态时的[工作量级别](33-模型配置.md#adjust-effort-level)。覆盖会话工作量级别。默认值：继承自会话。选项：`low`、`medium`、`high`、`max`（仅 Opus 4.6）。 |
| `context`                  | 否  | 设置为 `fork` 以在分叉的 subagent 上下文中运行。                                                                                                 |
| `agent`                    | 否  | 当设置 `context: fork` 时要使用的 subagent 类型。                                                                                            |
| `hooks`                    | 否  | 限定于此 skill 生命周期的 hooks。有关配置格式，请参阅 [Skills 和代理中的 Hooks](25-Hooks参考.md#hooks-in-skills-and-agents)。                                  |
...
**Tip:** 将 `SKILL.md` 保持在 500 行以下。将详细的参考资料移到单独的文件中。
...
**Note:**
在常规会话中，skill 描述被加载到上下文中，以便 Claude 知道什么可用，但完整 skill 内容仅在调用时加载。[预加载 skills 的 Subagents](49-创建自定义subagents.md#preload-skills-into-subagents) 的工作方式不同：完整 skill 内容在启动时注入。
...
**Tip:**
要在 skill 中启用[扩展思考](使用 Claude Code/02-常见工作流程.md#use-extended-thinking-thinking-mode)，在你的 skill 内容中的任何地方包含单词"ultrathink"。
...
Skills 和 [subagents](49-创建自定义subagents.md) 以两个方向协同工作：
...
使用 `context: fork`，你在你的 skill 中编写任务并选择一个代理类型来执行它。对于反向（定义使用 skills 作为参考资料的自定义 subagent），请参阅 [Subagents](49-创建自定义subagents.md#preload-skills-into-subagents)。
...
默认情况下，Claude 可以调用任何没有设置 `disable-model-invocation: true` 的 skill。定义 `allowed-tools` 的 Skills 在 skill 处于活动状态时向 Claude 授予对这些工具的访问权限，无需每次使用批准。你的[权限设置](37-配置权限.md)仍然管理所有其他工具的基线批准行为。内置命令如 `/compact` 和 `/init` 不能通过 Skill 工具获得。
...
**使用[权限规则](37-配置权限.md)允许或拒绝特定 skills**：
...
* **插件**：在你的[插件](39-创建插件.md)中创建 `skills/` 目录
* **托管**：通过[托管设置](45-ClaudeCode设置.md#settings-files)部署组织范围内
...
* **[Subagents](49-创建自定义subagents.md)**：将任务委派给专门的代理
* **[Plugins](39-创建插件.md)**：打包和分发 skills 与其他扩展
* **[Hooks](25-Hooks参考.md)**：围绕工具事件自动化工作流
* **[Memory](使用 Claude Code/03-Claude如何记住你的项目.md)**：管理 CLAUDE.md 文件以获得持久上下文
* **[Built-in commands](12-内置命令.md)**：内置 `/` 命令的参考
* **[Permissions](37-配置权限.md)**：控制工具和 skill 访问
