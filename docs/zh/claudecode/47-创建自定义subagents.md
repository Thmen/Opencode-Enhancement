...
| Plugin 的 `agents/` 目录 | 启用 plugin 的位置 | 4（最低）    | 与 [plugins](37-创建插件.md) 一起安装 |
...
`--agents` 标志接受 JSON，具有与基于文件的 subagents 相同的 [frontmatter](快速开始/01-ClaudeCode概述.md#supported-frontmatter-fields) 字段：`description`、`prompt`、`tools`、`disallowedTools`、`model`、`permissionMode`、`mcpServers`、`hooks`、`maxTurns`、`skills`、`memory`、`effort`、`background` 和 `isolation`。对系统提示使用 `prompt`，等同于基于文件的 subagents 中的 markdown 正文。

**Plugin subagents** 来自您已安装的 [plugins](37-创建插件.md)。它们与您的自定义 subagents 一起出现在 `/agents` 中。有关创建 plugin subagents 的详细信息，请参阅 [plugin 组件参考](38-Plugins参考.md#agents)。

<Note>
  出于安全原因，plugin subagents 不支持 `hooks`、`mcpServers` 或 `permissionMode` frontmatter 字段。加载来自 plugin 的代理时，这些字段被忽略。如果您需要它们，请将代理文件复制到 `.claude/agents/` 或 `~/.claude/agents/`。您也可以在 `settings.json` 或 `settings.local.json` 中向 [`permissions.allow`](43-ClaudeCode设置.md#permission-settings) 添加规则，但这些规则适用于整个会话，而不仅仅是 plugin subagent。
</Note>
...
| `skills`          | No       | [Skills](45-使用skills扩展Claude.md) 在启动时加载到 subagent 的上下文中。注入完整的技能内容，而不仅仅是可用于调用。Subagents 不继承来自父对话的技能                                                                                     |
| `mcpServers`      | No       | [MCP servers](29-通过MCP将ClaudeCode连接到工具.md) 对此 subagent 可用。每个条目要么是引用已配置服务器的服务器名称（例如，`"slack"`），要么是内联定义，其中服务器名称为键，完整的 [MCP server config](29-通过MCP将ClaudeCode连接到工具.md#configure-mcp-servers) 为值                   |
...
`model` 字段控制 subagent 使用的 [AI model](31-模型配置.md)：
...
Subagents 可以使用 Claude Code 的任何 [internal tools](50-工具参考.md)。默认情况下，subagents 继承主对话的所有工具，包括 MCP 工具。
...
使用 `mcpServers` 字段为 subagent 提供对主对话中不可用的 [MCP](29-通过MCP将ClaudeCode连接到工具.md) 服务器的访问。此处定义的内联服务器在 subagent 启动时连接，在完成时断开连接。字符串引用共享父会话的连接。
...
<Note>
  这与 [在 subagent 中运行技能](45-使用skills扩展Claude.md#run-skills-in-a-subagent) 相反。使用 subagent 中的 `skills`，subagent 控制系统提示并加载技能内容。使用技能中的 `context: fork`，技能内容被注入到您指定的代理中。两者都使用相同的底层系统。
</Note>
...
Claude Code [通过 stdin 将 hook 输入作为 JSON 传递](23-Hooks参考.md#pretooluse-input) 给 hook 命令。验证脚本读取此 JSON，提取 Bash 命令，并 [以代码 2 退出](23-Hooks参考.md#exit-code-2-behavior-per-event) 以阻止写入操作：
...
有关完整的输入架构，请参阅 [Hook input](23-Hooks参考.md#pretooluse-input)，有关退出代码如何影响行为，请参阅 [exit codes](23-Hooks参考.md#exit-code-output)。

#### 禁用特定 subagents

您可以通过将 subagents 添加到您的 [settings](43-ClaudeCode设置.md#permission-settings) 中的 `deny` 数组来防止 Claude 使用特定 subagents。使用格式 `Agent(subagent-name)`，其中 `subagent-name` 与 subagent 的 name 字段匹配。
...
有关权限规则的更多详细信息，请参阅 [Permissions documentation](35-配置权限.md#tool-specific-permission-rules)。

### 为 subagents 定义 hooks

Subagents 可以定义在 subagent 的生命周期中运行的 [hooks](23-Hooks参考.md)。有两种方式来配置 hooks：
...
所有 [hook events](23-Hooks参考.md#hook-events) 都被支持。subagents 最常见的事件是：
...
有关完整的 hook 配置格式，请参阅 [Hooks](23-Hooks参考.md)。
...
由启用的 [plugin](37-创建插件.md) 提供的 Subagents 在类型提前中显示为 `<plugin-name>:<agent-name>`。您也可以手动输入提及而不使用选择器：`@agent-<name>` 用于本地 subagents，或 `@agent-<plugin-name>:<agent-name>` 用于 plugin subagents。

**将整个会话作为 subagent 运行。** 传递 [`--agent <name>`](08-CLI参考.md) 以启动一个会话，其中主线程本身采用该 subagent 的系统提示、工具限制和模型：

```bash  theme={null}
claude --agent code-reviewer
```

Subagent 的系统提示完全替换默认 Claude Code 系统提示，就像 [`--system-prompt`](08-CLI参考.md) 一样。`CLAUDE.md` 文件和项目内存仍然通过正常消息流加载。代理名称在启动标题中显示为 `@<name>`，以便您可以确认它是活跃的。
...
* **前台 subagents** 阻塞主对话直到完成。权限提示和澄清问题（如 [`AskUserQuestion`](50-工具参考.md)）会传递给您。
...
要禁用所有后台任务功能，请将 `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` 环境变量设置为 `1`。请参阅 [Environment variables](17-环境变量.md)。
...
当您想要可重用的提示或在主对话上下文中运行的工作流而不是隔离的 subagent 上下文时，请改为考虑 [Skills](45-使用skills扩展Claude.md)。

对于关于对话中已有内容的快速问题，使用 [`/btw`](25-交互模式.md#side-questions-with-btw) 而不是 subagent。它看到您的完整上下文但没有工具访问，答案被丢弃而不是添加到历史记录。

<Note>
  Subagents 无法生成其他 subagents。如果您的工作流需要嵌套委托，请使用 [Skills](45-使用skills扩展Claude.md) 或从主对话 [链接 subagents](快速开始/01-ClaudeCode概述.md#chain-subagents)。
</Note>
...
Subagents 支持使用与主对话相同的逻辑进行自动压缩。默认情况下，自动压缩在大约 95% 容量时触发。要更早触发压缩，请将 `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` 设置为较低的百分比（例如，`50`）。有关详细信息，请参阅 [environment variables](17-环境变量.md)。
...
Claude Code [通过 stdin 将 hook 输入作为 JSON 传递](23-Hooks参考.md#pretooluse-input) 给 hook 命令。验证脚本读取此 JSON，提取正在执行的命令，并根据 SQL 写入操作列表检查它。如果检测到写入操作，脚本 [以代码 2 退出](23-Hooks参考.md#exit-code-2-behavior-per-event) 以阻止执行，并通过 stderr 向 Claude 返回错误消息。
...
Hook 通过 stdin 接收 JSON，Bash 命令在 `tool_input.command` 中。退出代码 2 阻止操作并将错误消息反馈给 Claude。有关退出代码和 [Hook input](23-Hooks参考.md#pretooluse-input) 的详细信息，请参阅 [Hooks](23-Hooks参考.md#exit-code-output) 以获取完整的输入架构。

## 后续步骤

现在您了解了 subagents，探索这些相关功能：

* [使用 plugins 分发 subagents](37-创建插件.md) 以在团队或项目中共享 subagents
* [以编程方式运行 Claude Code](22-以编程方式运行ClaudeCode.md)，使用 Agent SDK 进行 CI/CD 和自动化
* [使用 MCP 服务器](29-通过MCP将ClaudeCode连接到工具.md) 为 subagents 提供对外部工具和数据的访问
