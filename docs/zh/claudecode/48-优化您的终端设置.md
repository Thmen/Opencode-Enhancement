...
为了进一步自定义 Claude Code 界面本身，您可以配置一个[自定义状态行](46-自定义你的状态行.md)来显示上下文信息，如当前模型、工作目录或 git 分支在您的终端底部。
...
当 Claude 完成工作并等待您的输入时，它会触发一个通知事件。您可以通过您的终端将此事件显示为桌面通知，或使用[通知钩子](23-Hooks参考.md#notification)运行自定义逻辑。
...
其他终端，包括默认的 macOS 终端，不支持原生通知。请改用[通知钩子](23-Hooks参考.md#notification)。

#### 通知钩子

要在通知触发时添加自定义行为，例如播放声音或发送消息，请配置一个[通知钩子](23-Hooks参考.md#notification)。钩子与终端通知一起运行，而不是作为替代品。
...
请参阅[交互模式](25-交互模式.md#vim-editor-mode)获取完整参考。
