...
这是一个[多行状态行](快速开始/01-ClaudeCode概述.md#display-multiple-lines)的示例，它在第一行显示 git 信息，在第二行显示颜色编码的上下文栏。
...
本页面介绍了[设置基本状态行](快速开始/01-ClaudeCode概述.md#set-up-a-status-line)，解释了[数据如何从 Claude Code 流向你的脚本](快速开始/01-ClaudeCode概述.md#how-status-lines-work)，列出了[你可以显示的所有字段](快速开始/01-ClaudeCode概述.md#available-data)，并提供了[常见模式的现成示例](快速开始/01-ClaudeCode概述.md#examples)，如 git 状态、成本跟踪和进度条。

## 设置状态行

使用[`/statusline` 命令](快速开始/01-ClaudeCode概述.md#use-the-statusline-command)让 Claude Code 为你生成脚本，或[手动创建脚本](快速开始/01-ClaudeCode概述.md#manually-configure-a-status-line)并将其添加到你的设置中。
...
将 `statusLine` 字段添加到你的用户设置（`~/.claude/settings.json`，其中 `~` 是你的主目录）或[项目设置](45-ClaudeCode设置.md#settings-files)。将 `type` 设置为 `"command"` 并将 `command` 指向脚本路径或内联 shell 命令。有关创建脚本的完整演练，请参阅[逐步构建状态行](快速开始/01-ClaudeCode概述.md#build-a-status-line-step-by-step)。
...
**Note:** 使用[`/statusline`](快速开始/01-ClaudeCode概述.md#use-the-statusline-command)和你想要的内容的描述会自动为你配置所有这些。

这些示例使用 Bash 脚本，在 macOS 和 Linux 上工作。在 Windows 上，请参阅[Windows 配置](快速开始/01-ClaudeCode概述.md#windows-configuration)了解 PowerShell 和 Git Bash 示例。
...
Claude Code 运行你的脚本并通过 stdin 向其传输[JSON 会话数据](快速开始/01-ClaudeCode概述.md#available-data)。你的脚本读取 JSON，提取它需要的内容，并将文本打印到 stdout。Claude Code 显示你的脚本打印的任何内容。
...
* **多行**：每个 `echo` 或 `print` 语句显示为单独的行。请参阅[多行示例](快速开始/01-ClaudeCode概述.md#display-multiple-lines)。
* **颜色**：使用[ANSI 转义码](https://en.wikipedia.org/wiki/ANSI_escape_code#Colors)，如 `\033[32m` 表示绿色（终端必须支持它们）。请参阅[git 状态示例](快速开始/01-ClaudeCode概述.md#git-status-with-colors)。
* **链接**：使用[OSC 8 转义序列](https://en.wikipedia.org/wiki/ANSI_escape_code#OSC)使文本可点击（macOS 上为 Cmd+click，Windows/Linux 上为 Ctrl+click）。需要支持超链接的终端，如 iTerm2、Kitty 或 WezTerm。请参阅[可点击链接示例](快速开始/01-ClaudeCode概述.md#clickable-links)。

**Note:** 状态行在本地运行，不消耗 API 令牌。在某些 UI 交互期间，它会临时隐藏，包括自动完成建议、帮助菜单和权限提示。
...
| `context_window.current_usage`                                            | 来自最后一次 API 调用的令牌计数，在[上下文窗口字段](快速开始/01-ClaudeCode概述.md#context-window-fields)中描述                            |
...
| `vim.mode`                                                                | 启用[vim 模式](27-交互模式.md#vim-editor-mode)时的当前 vim 模式（`NORMAL` 或 `INSERT`） |
...
3. 将路径添加到你的[设置](快速开始/01-ClaudeCode概述.md#manually-configure-a-status-line)
...
* **缓存慢速操作**：你的脚本在活跃会话期间频繁运行，所以像 `git status` 这样的命令可能会导致延迟。请参阅[缓存示例](快速开始/01-ClaudeCode概述.md#cache-expensive-operations)了解如何处理这个问题。
