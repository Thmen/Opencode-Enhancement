...
* [Claude for Teams 或 Enterprise](04-身份验证.md#claude-for-teams-or-enterprise)
* [Anthropic Console](04-身份验证.md#claude-console-authentication)
* [Amazon Bedrock](02-AmazonBedrock上的ClaudeCode.md)
* [Google Vertex AI](21-GoogleVertexAI上的ClaudeCode.md)
* [Microsoft Foundry](30-ClaudeCodeonMicrosoftFoundry.md)

## 配置代理和网关

大多数组织可以直接使用云提供商，无需额外配置。但是，如果您的组织有特定的网络或管理要求，您可能需要配置企业代理或 LLM 网关。这些是可以一起使用的不同配置：

* **企业代理**：通过 HTTP/HTTPS 代理路由流量。如果您的组织要求所有出站流量通过代理服务器以进行安全监控、合规性或网络策略执行，请使用此选项。使用 `HTTPS_PROXY` 或 `HTTP_PROXY` 环境变量进行配置。在[企业网络配置](33-企业网络配置.md)中了解更多。
* **LLM 网关**：位于 Claude Code 和云提供商之间的服务，用于处理身份验证和路由。如果您需要跨团队的集中使用情况跟踪、自定义速率限制或预算或集中身份验证管理，请使用此选项。使用 `ANTHROPIC_BASE_URL`、`ANTHROPIC_BEDROCK_BASE_URL` 或 `ANTHROPIC_VERTEX_BASE_URL` 环境变量进行配置。在[LLM 网关配置](28-LLMgateway配置.md)中了解更多。

以下示例显示在 shell 或 shell 配置文件（`.bashrc`、`.zshrc`）中设置的环境变量。有关其他配置方法，请参阅[设置](43-ClaudeCode设置.md)。

### Amazon Bedrock

<Tabs>
  <Tab title="企业代理">
    通过设置以下[环境变量](17-环境变量.md)，将 Bedrock 流量路由通过您的企业代理：
  ...
  <Tab title="LLM 网关">
    通过设置以下[环境变量](17-环境变量.md)，将 Bedrock 流量路由通过您的 LLM 网关：
...
<Tabs>
  <Tab title="企业代理">
    通过设置以下[环境变量](17-环境变量.md)，将 Foundry 流量路由通过您的企业代理：
  ...
  <Tab title="LLM 网关">
    通过设置以下[环境变量](17-环境变量.md)，将 Foundry 流量路由通过您的 LLM 网关：
...
<Tabs>
  <Tab title="企业代理">
    通过设置以下[环境变量](17-环境变量.md)，将 Vertex AI 流量路由通过您的企业代理：
  ...
  <Tab title="LLM 网关">
    通过设置以下[环境变量](17-环境变量.md)，将 Vertex AI 流量路由通过您的 LLM 网关：
...
在[内存和 CLAUDE.md 文件](使用 Claude Code/03-Claude如何记住你的项目.md)中了解更多。
...
如果您通过 [Bedrock](02-AmazonBedrock上的ClaudeCode.md)、[Vertex AI](21-GoogleVertexAI上的ClaudeCode.md) 或 [Foundry](30-ClaudeCodeonMicrosoftFoundry.md) 部署，请使用 `ANTHROPIC_DEFAULT_OPUS_MODEL`、`ANTHROPIC_DEFAULT_SONNET_MODEL` 和 `ANTHROPIC_DEFAULT_HAIKU_MODEL` 固定特定模型版本。如果不固定，Claude Code 别名会解析为最新版本，当 Anthropic 发布您的账户中尚未启用的新模型时，可能会破坏用户。有关详细信息，请参阅[模型配置](31-模型配置.md#pin-models-for-third-party-deployments)。

### 配置安全策略

安全团队可以配置托管权限，定义 Claude Code 允许和不允许做什么，这不能被本地配置覆盖。[了解更多](41-安全性.md)。

### 利用 MCP 进行集成

MCP 是为 Claude Code 提供更多信息的好方法，例如连接到票证管理系统或错误日志。我们建议一个中央团队配置 MCP servers 并将 `.mcp.json` 配置检入代码库，以便所有用户受益。[了解更多](29-通过MCP将ClaudeCode连接到工具.md)。
...
1. **向您的团队推出**：分享安装说明，让团队成员[安装 Claude Code](44-高级设置.md) 并使用其凭证进行身份验证。
2. **设置共享配置**：在您的存储库中创建 [CLAUDE.md 文件](使用 Claude Code/03-Claude如何记住你的项目.md)，以帮助 Claude Code 理解您的代码库和编码标准。
3. **配置权限**：查看[安全设置](41-安全性.md)以定义 Claude Code 在您的环境中可以和不能做什么。
