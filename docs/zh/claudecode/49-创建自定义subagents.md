...
**Note:**
如果您需要多个代理并行工作并相互通信，请参阅 [agent teams](01-协调ClaudeCode会话团队.md) 代替。Subagents 在单个会话中工作；agent teams 跨多个会话进行协调。
...
Claude Code 包括几个内置 subagents，如 **Explore**、**Plan** 和 **general-purpose**。您也可以创建自定义 subagents 来处理特定任务。本页涵盖 [内置 subagents](快速开始/01-ClaudeCode概述.md#built-in-subagents)、[如何创建您自己的](快速开始/01-ClaudeCode概述.md#quickstart-create-your-first-subagent)、[完整配置选项](快速开始/01-ClaudeCode概述.md#configure-subagents)、[使用 subagents 的模式](快速开始/01-ClaudeCode概述.md#work-with-subagents) 和 [示例 subagents](快速开始/01-ClaudeCode概述.md#example-subagents)。
...
#### Plan
一个研究代理，在 [plan mode](使用 Claude Code/02-常见工作流程.md#use-plan-mode-for-safe-code-analysis) 期间使用，以在呈现计划之前收集上下文。
...
Subagents 在带有 YAML frontmatter 的 Markdown 文件中定义。您可以 [手动创建它们](快速开始/01-ClaudeCode概述.md#write-subagent-files) 或使用 `/agents` 命令。
...
#### 配置内存
选择 **User scope** 为 subagent 提供一个 [persistent memory directory](快速开始/01-ClaudeCode概述.md#enable-persistent-memory)，位于 `~/.claude/agent-memory/`。Subagent 使用这个来在对话中积累见解，例如代码库模式和重复出现的问题。如果您不希望 subagent 保留学习，请选择 **None**。
...
| Plugin 的 `agents/` 目录 | 启用 plugin 的位置 | 4（最低）    | 与 [plugins](39-创建插件.md) 一起安装 |
...
`--agents` 标志接受 JSON，具有与基于文件的 subagents 相同的 [frontmatter](快速开始/01-ClaudeCode概述.md#supported-frontmatter-fields) 字段：`description`、`prompt`、`tools`、`disallowedTools`、`model`、`permissionMode`、`mcpServers`、`hooks`、`maxTurns`、`skills`、`initialPrompt`、`memory`、`effort`、`background` 和 `isolation`。对系统提示使用 `prompt`，等同于基于文件的 subagents 中的 markdown 正文。

**Plugin subagents** 来自您已安装的 [plugins](39-创建插件.md)。它们与您的自定义 subagents 一起出现在 `/agents` 中。有关创建 plugin subagents 的详细信息，请参阅 [plugin 组件参考](40-Plugins参考.md#agents)。

**Note:**
出于安全原因，plugin subagents 不支持 `hooks`、`mcpServers` 或 `permissionMode` frontmatter 字段。加载来自 plugin 的代理时，这些字段被忽略。如果您需要它们，请将代理文件复制到 `.claude/agents/` 或 `~/.claude/agents/`。您也可以在 `settings.json` 或 `settings.local.json` 中向 [`permissions.allow`](45-ClaudeCode设置.md#permission-settings) 添加规则，但这些规则适用于整个会话，而不仅仅是 plugin subagent。
...
| `tools`           | No       | [Tools](快速开始/01-ClaudeCode概述.md#available-tools) subagent 可以使用。如果省略，继承所有工具                                                                                                                           |
| `disallowedTools` | No       | 要拒绝的工具，从继承或指定的列表中删除                                                                                                                                                           |
| `model`           | No       | [Model](快速开始/01-ClaudeCode概述.md#choose-a-model) 使用：`sonnet`、`opus`、`haiku`、完整模型 ID（例如，`claude-opus-4-6`）或 `inherit`。默认为 `inherit`                                                                    |
| `permissionMode`  | No       | [Permission mode](快速开始/01-ClaudeCode概述.md#permission-modes)：`default`、`acceptEdits`、`dontAsk`、`bypassPermissions` 或 `plan`                                                                           |
| `maxTurns`        | No       | subagent 停止前的最大代理轮数                                                                                                                                                           |
| `skills`          | No       | [Skills](47-使用skills扩展Claude.md) 在启动时加载到 subagent 的上下文中。注入完整的技能内容，而不仅仅是可用于调用。Subagents 不继承来自父对话的技能                                                                                     |
| `mcpServers`      | No       | [MCP servers](31-通过MCP将ClaudeCode连接到工具.md) 对此 subagent 可用。每个条目要么是引用已配置服务器的服务器名称（例如，`"slack"`），要么是内联定义，其中服务器名称为键，完整的 [MCP server config](31-通过MCP将ClaudeCode连接到工具.md#configure-mcp-servers) 为值                   |
| `hooks`           | No       | [Lifecycle hooks](快速开始/01-ClaudeCode概述.md#define-hooks-for-subagents) 限定于此 subagent                                                                                                                  |
| `memory`          | No       | [Persistent memory scope](快速开始/01-ClaudeCode概述.md#enable-persistent-memory)：`user`、`project` 或 `local`。启用跨会话学习                                                                                       |
| `background`      | No       | 设置为 `true` 以始终将此 subagent 作为 [background task](快速开始/01-ClaudeCode概述.md#run-subagents-in-foreground-or-background) 运行。默认：`false`                                                                      |
| `effort`          | No       | 此 subagent 活跃时的努力级别。覆盖会话努力级别。默认：从会话继承。选项：`low`、`medium`、`high`、`max`（仅 Opus 4.6）                                                                                              |
| `isolation`       | No       | 设置为 `worktree` 以在临时 [git worktree](使用 Claude Code/02-常见工作流程.md#run-parallel-claude-code-sessions-with-git-worktrees) 中运行 subagent，为其提供存储库的隔离副本。如果 subagent 不进行任何更改，worktree 会自动清理 |
| `initialPrompt`   | No       | 当此代理作为主会话代理运行时（通过 `--agent` 或 `agent` 设置），自动提交为第一个用户轮次。[Commands](12-内置命令.md) 和 [skills](47-使用skills扩展Claude.md) 被处理。前置于任何用户提供的提示                                                 |

### 选择模型

`model` 字段控制 subagent 使用的 [AI model](33-模型配置.md)：
...
1. [`CLAUDE_CODE_SUBAGENT_MODEL`](33-模型配置.md#environment-variables) 环境变量，如果设置
2. 每次调用的 `model` 参数
...
Subagents 可以使用 Claude Code 的任何 [internal tools](52-工具参考.md)。默认情况下，subagents 继承主对话的所有工具，包括 MCP 工具。
...
**Note:** 在版本 2.1.63 中，Task 工具被重命名为 Agent。设置和代理定义中的现有 `Task(...)` 引用仍然作为别名工作。
...
这是一个允许列表：只有 `worker` 和 `researcher` subagents 可以被生成。如果代理尝试生成任何其他类型，请求失败，代理在其提示中仅看到允许的类型。要在允许所有其他类型的同时阻止特定代理，请改用 [`permissions.deny`](快速开始/01-ClaudeCode概述.md#disable-specific-subagents)。
...
使用 `mcpServers` 字段为 subagent 提供对主对话中不可用的 [MCP](31-通过MCP将ClaudeCode连接到工具.md) 服务器的访问。此处定义的内联服务器在 subagent 启动时连接，在完成时断开连接。字符串引用共享父会话的连接。
...
**Warning:**
谨慎使用 `bypassPermissions`。它跳过权限提示，允许 subagent 在没有批准的情况下执行操作。对 `.git`、`.claude`、`.vscode` 和 `.idea` 目录的写入仍然会提示确认，除了 `.claude/commands`、`.claude/agents` 和 `.claude/skills`。有关详细信息，请参阅 [permission modes](使用 Claude Code/04-选择权限模式.md#skip-all-checks-with-bypasspermissions-mode)。

如果父级使用 `bypassPermissions`，这优先并且无法被覆盖。如果父级使用 [auto mode](使用 Claude Code/04-选择权限模式.md#eliminate-prompts-with-auto-mode)，subagent 继承 auto mode，其 frontmatter 中的任何 `permissionMode` 被忽略：分类器使用与父会话相同的块和允许规则评估 subagent 的工具调用。
...
**Note:**
这与 [在 subagent 中运行技能](47-使用skills扩展Claude.md#run-skills-in-a-subagent) 相反。使用 subagent 中的 `skills`，subagent 控制系统提示并加载技能内容。使用技能中的 `context: fork`，技能内容被注入到您指定的代理中。两者都使用相同的底层系统。
...
Claude Code [通过 stdin 将 hook 输入作为 JSON 传递](25-Hooks参考.md#pretooluse-input) 给 hook 命令。验证脚本读取此 JSON，提取 Bash 命令，并 [以代码 2 退出](25-Hooks参考.md#exit-code-2-behavior-per-event) 以阻止写入操作：
...
有关完整的输入架构，请参阅 [Hook input](25-Hooks参考.md#pretooluse-input)，有关退出代码如何影响行为，请参阅 [exit codes](25-Hooks参考.md#exit-code-output)。

#### 禁用特定 subagents

您可以通过将 subagents 添加到您的 [settings](45-ClaudeCode设置.md#permission-settings) 中的 `deny` 数组来防止 Claude 使用特定 subagents。使用格式 `Agent(subagent-name)`，其中 `subagent-name` 与 subagent 的 name 字段匹配。
...
有关权限规则的更多详细信息，请参阅 [Permissions documentation](37-配置权限.md#tool-specific-permission-rules)。

### 为 subagents 定义 hooks

Subagents 可以定义在 subagent 的生命周期中运行的 [hooks](25-Hooks参考.md)。有两种方式来配置 hooks：
...
所有 [hook events](25-Hooks参考.md#hook-events) 都被支持。subagents 最常见的事件是：
...
有关完整的 hook 配置格式，请参阅 [Hooks](25-Hooks参考.md)。
...
由启用的 [plugin](39-创建插件.md) 提供的 Subagents 在类型提前中显示为 `<plugin-name>:<agent-name>`。您也可以手动输入提及而不使用选择器：`@agent-<name>` 用于本地 subagents，或 `@agent-<plugin-name>:<agent-name>` 用于 plugin subagents。

**将整个会话作为 subagent 运行。** 传递 [`--agent <name>`](10-CLI参考.md) 以启动一个会话，其中主线程本身采用该 subagent 的系统提示、工具限制和模型：

```bash
claude --agent code-reviewer
```

Subagent 的系统提示完全替换默认 Claude Code 系统提示，就像 [`--system-prompt`](10-CLI参考.md) 一样。`CLAUDE.md` 文件和项目内存仍然通过正常消息流加载。代理名称在启动标题中显示为 `@<name>`，以便您可以确认它是活跃的。
...
* **前台 subagents** 阻塞主对话直到完成。权限提示和澄清问题（如 [`AskUserQuestion`](52-工具参考.md)）会传递给您。
...
要禁用所有后台任务功能，请将 `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` 环境变量设置为 `1`。请参阅 [Environment variables](19-环境变量.md)。
...
对于需要持续并行性或超过您的 context window 的任务，[agent teams](01-协调ClaudeCode会话团队.md) 为每个工作者提供自己的独立上下文。
...
当您想要可重用的提示或在主对话上下文中运行的工作流而不是隔离的 subagent 上下文时，请改为考虑 [Skills](47-使用skills扩展Claude.md)。

对于关于对话中已有内容的快速问题，使用 [`/btw`](27-交互模式.md#side-questions-with-btw) 而不是 subagent。它看到您的完整上下文但没有工具访问，答案被丢弃而不是添加到历史记录。

**Note:**
Subagents 无法生成其他 subagents。如果您的工作流需要嵌套委托，请使用 [Skills](47-使用skills扩展Claude.md) 或从主对话 [链接 subagents](快速开始/01-ClaudeCode概述.md#chain-subagents)。
...
* **会话持久性**：Subagent 转录在其会话中持久化。您可以通过恢复相同的会话在重启 Claude Code 后 [恢复 subagent](快速开始/01-ClaudeCode概述.md#resume-subagents)。
* **自动清理**：转录根据 `cleanupPeriodDays` 设置（默认：30 天）进行清理。

#### 自动压缩

Subagents 支持使用与主对话相同的逻辑进行自动压缩。默认情况下，自动压缩在大约 95% 容量时触发。要更早触发压缩，请将 `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` 设置为较低的百分比（例如，`50`）。有关详细信息，请参阅 [environment variables](19-环境变量.md)。
...
Claude Code [通过 stdin 将 hook 输入作为 JSON 传递](25-Hooks参考.md#pretooluse-input) 给 hook 命令。验证脚本读取此 JSON，提取正在执行的命令，并根据 SQL 写入操作列表检查它。如果检测到写入操作，脚本 [以代码 2 退出](25-Hooks参考.md#exit-code-2-behavior-per-event) 以阻止执行，并通过 stderr 向 Claude 返回错误消息。
...
Hook 通过 stdin 接收 JSON，Bash 命令在 `tool_input.command` 中。退出代码 2 阻止操作并将错误消息反馈给 Claude。有关退出代码和 [Hook input](25-Hooks参考.md#pretooluse-input) 的详细信息，请参阅 [Hooks](25-Hooks参考.md#exit-code-output) 以获取完整的输入架构。

## 后续步骤

现在您了解了 subagents，探索这些相关功能：

* [使用 plugins 分发 subagents](39-创建插件.md) 以在团队或项目中共享 subagents
* [以编程方式运行 Claude Code](24-以编程方式运行ClaudeCode.md)，使用 Agent SDK 进行 CI/CD 和自动化
* [使用 MCP 服务器](31-通过MCP将ClaudeCode连接到工具.md) 为 subagents 提供对外部工具和数据的访问
