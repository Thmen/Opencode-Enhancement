...
<Tip>
  如果您想完全跳过终端，[Claude Code 桌面应用](14-开始使用桌面应用.md)让您可以通过图形界面安装和使用 Claude Code。下载 [macOS](https://claude.ai/api/desktop/darwin/universal/dmg/latest/redirect?utm_source=claude_code\&utm_medium=docs) 或 [Windows](https://claude.ai/api/desktop/win32/x64/exe/latest/redirect?utm_source=claude_code\&utm_medium=docs) 版本，无需任何命令行设置即可开始编码。
</Tip>
...
| 您看到的内容                                                     | 解决方案                                                                                  |
| :--------------------------------------------------------- | :------------------------------------------------------------------------------------ |
| `command not found: claude` 或 `'claude' is not recognized` | [修复您的 PATH](快速开始/01-ClaudeCode概述.md#command-not-found-claude-after-installation)                             |
| `syntax error near unexpected token '<'`                   | [安装脚本返回 HTML](快速开始/01-ClaudeCode概述.md#install-script-returns-html-instead-of-a-shell-script)                 |
| `curl: (56) Failure writing output to destination`         | [先下载脚本，然后运行](快速开始/01-ClaudeCode概述.md#curl-56-failure-writing-output-to-destination)                          |
| Linux 上安装期间 `Killed`                                       | [为低内存服务器添加交换空间](快速开始/01-ClaudeCode概述.md#install-killed-on-low-memory-linux-servers)                          |
| `TLS connect error` 或 `SSL/TLS secure channel`             | [更新 CA 证书](快速开始/01-ClaudeCode概述.md#tls-or-ssl-connection-errors)                                             |
| `Failed to fetch version` 或无法访问下载服务器                       | [检查网络和代理设置](快速开始/01-ClaudeCode概述.md#check-network-connectivity)                                              |
| `irm is not recognized` 或 `&& is not valid`                | [为您的 shell 使用正确的命令](快速开始/01-ClaudeCode概述.md#windows-irm-or--not-recognized)                                  |
| `Claude Code on Windows requires git-bash`                 | [安装或配置 Git Bash](快速开始/01-ClaudeCode概述.md#windows-claude-code-on-windows-requires-git-bash)                   |
| `Error loading shared library`                             | [您的系统安装了错误的二进制变体](快速开始/01-ClaudeCode概述.md#linux-wrong-binary-variant-installed-muslglibc-mismatch)           |
| Linux 上的 `Illegal instruction`                             | [架构不匹配](快速开始/01-ClaudeCode概述.md#illegal-instruction-on-linux)                                                |
| macOS 上的 `dyld: cannot load` 或 `Abort trap`                | [二进制不兼容](快速开始/01-ClaudeCode概述.md#dyld-cannot-load-on-macos)                                                  |
| `Invoke-Expression: Missing argument in parameter list`    | [安装脚本返回 HTML](快速开始/01-ClaudeCode概述.md#install-script-returns-html-instead-of-a-shell-script)                 |
| `App unavailable in region`                                | Claude Code 在您的国家/地区不可用。请参阅[支持的国家/地区](https://www.anthropic.com/supported-countries)。 |
| `unable to get local issuer certificate`                   | [配置企业 CA 证书](快速开始/01-ClaudeCode概述.md#tls-or-ssl-connection-errors)                                           |
| `OAuth error` 或 `403 Forbidden`                            | [修复身份验证](快速开始/01-ClaudeCode概述.md#authentication-issues)                                                      |
...
在 Linux 上，检查缺失的共享库。如果 `ldd` 显示缺失的库，您可能需要安装系统包。在 Alpine Linux 和其他基于 musl 的发行版上，请参阅 [Alpine Linux 设置](44-高级设置.md#alpine-linux-and-musl-based-distributions)。
...
这意味着安装目录不在您的 shell 搜索路径中。请参阅[验证您的 PATH](快速开始/01-ClaudeCode概述.md#verify-your-path) 以获取每个平台上的修复。
...
2. **如果在代理后面**，设置 `HTTPS_PROXY` 以便安装程序可以通过它路由。有关详细信息，请参阅[代理配置](33-企业网络配置.md#proxy-configuration)。
   ```bash  theme={null}
   export HTTPS_PROXY=http://proxy.example.com:8080
   curl -fsSL https://claude.ai/install.sh | bash
   ```
...
**如果已安装 Git** 但 Claude Code 仍然找不到它，在您的 [settings.json 文件](43-ClaudeCode设置.md)中设置路径：
...
[Sandboxing](39-Sandboxing.md) 在 WSL2 上受支持，但需要安装额外的包。如果运行 `/sandbox` 时看到"Sandbox requires socat and bubblewrap"之类的错误，请安装依赖项：
...
如果本机安装程序因权限错误失败，目标目录可能不可写。请参阅[检查目录权限](快速开始/01-ClaudeCode概述.md#check-directory-permissions)。
...
如果您发现自己反复批准相同的命令，您可以使用 `/permissions` 命令允许特定工具无需批准即可运行。请参阅[权限文档](35-配置权限.md#manage-permissions)。
...
* **在代理后面**：企业代理可能干扰 API 请求。有关代理设置，请参阅[网络配置](33-企业网络配置.md)。
...
| `managed-mcp.json`            | [托管 MCP 服务器](29-通过MCP将ClaudeCode连接到工具.md#managed-mcp-configuration)             |
| 托管设置                          | [托管设置](43-ClaudeCode设置.md#settings-files)（服务器管理、MDM/OS 级别策略或基于文件） |

在 Windows 上，`~` 指您的用户主目录，例如 `C:\Users\YourName`。

有关配置这些文件的详细信息，请参阅[设置](43-ClaudeCode设置.md)和 [MCP](29-通过MCP将ClaudeCode连接到工具.md)。
...
然后在您的[环境](17-环境变量.md)中设置 `USE_BUILTIN_RIPGREP=0`。
...
有关其他 JetBrains 配置提示，请参阅 [JetBrains IDE 指南](平台和集成/02-JetBrainsIDEs.md#plugin-settings)。
...
2. **使用后处理 hooks**：设置自动格式化 hooks 以检测和添加缺失的语言标签。有关示例，请参阅[编辑后自动格式化代码](24-使用hooks自动化工作流.md#auto-format-code-after-edits)的 PostToolUse 格式化 hook。
...
3. **指定格式化首选项**：在您的提示或项目[内存](使用 Claude Code/03-Claude如何记住你的项目.md)文件中包含格式化要求。
...
* **使用项目约定**：在 [`CLAUDE.md`](使用 Claude Code/03-Claude如何记住你的项目.md) 中记录您首选的 markdown 风格
