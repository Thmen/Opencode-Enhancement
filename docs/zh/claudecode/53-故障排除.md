...
**Tip:**
如果您想完全跳过终端，[Claude Code 桌面应用](16-开始使用桌面应用.md)让您可以通过图形界面安装和使用 Claude Code。下载 [macOS](https://claude.ai/api/desktop/darwin/universal/dmg/latest/redirect?utm_source=claude_code\&utm_medium=docs) 或 [Windows](https://claude.ai/api/desktop/win32/x64/exe/latest/redirect?utm_source=claude_code\&utm_medium=docs) 版本，无需任何命令行设置即可开始编码。
...
在 Linux 上，检查缺失的共享库。如果 `ldd` 显示缺失的库，您可能需要安装系统包。在 Alpine Linux 和其他基于 musl 的发行版上，请参阅 [Alpine Linux 设置](46-高级设置.md#alpine-linux-and-musl-based-distributions)。
...
2. **如果在代理后面**，设置 `HTTPS_PROXY` 以便安装程序可以通过它路由。有关详细信息，请参阅[代理配置](35-企业网络配置.md#proxy-configuration)。
   ```bash
   export HTTPS_PROXY=http://proxy.example.com:8080
   curl -fsSL https://claude.ai/install.sh | bash
   ```
...
**如果已安装 Git** 但 Claude Code 仍然找不到它，在您的 [settings.json 文件](45-ClaudeCode设置.md)中设置路径：
...
[Sandboxing](41-Sandboxing.md) 在 WSL2 上受支持，但需要安装额外的包。如果运行 `/sandbox` 时看到"Sandbox requires socat and bubblewrap"之类的错误，请安装依赖项：
...
如果您发现自己反复批准相同的命令，您可以使用 `/permissions` 命令允许特定工具无需批准即可运行。请参阅[权限文档](37-配置权限.md#manage-permissions)。
...
* **在代理后面**：企业代理可能干扰 API 请求。有关代理设置，请参阅[网络配置](35-企业网络配置.md)。
...
| `managed-mcp.json`            | [托管 MCP 服务器](31-通过MCP将ClaudeCode连接到工具.md#managed-mcp-configuration)             |
| 托管设置                          | [托管设置](45-ClaudeCode设置.md#settings-files)（服务器管理、MDM/OS 级别策略或基于文件） |

在 Windows 上，`~` 指您的用户主目录，例如 `C:\Users\YourName`。

有关配置这些文件的详细信息，请参阅[设置](45-ClaudeCode设置.md)和 [MCP](31-通过MCP将ClaudeCode连接到工具.md)。
...
然后在您的[环境](19-环境变量.md)中设置 `USE_BUILTIN_RIPGREP=0`。
...
2. **使用后处理 hooks**：设置自动格式化 hooks 以检测和添加缺失的语言标签。有关示例，请参阅[编辑后自动格式化代码](26-使用hooks自动化工作流.md#auto-format-code-after-edits)的 PostToolUse 格式化 hook。
