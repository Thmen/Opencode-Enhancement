...
* 按用户的成本控制
* [分析](03-使用分析跟踪团队使用情况.md)仪表板
* [服务器管理的设置](42-配置服务器管理的设置（公开测试版）.md)
* 审计日志
...
ZDR 不适用于以下内容，即使对于启用了 ZDR 的组织也是如此。这些功能遵循[标准数据保留政策](12-数据使用.md#data-retention)：

| 功能             | 详情                                                                                    |
| -------------- | ------------------------------------------------------------------------------------- |
| claude.ai 上的聊天 | 通过 Claude for Enterprise 网络界面的聊天对话不受 ZDR 保护。                                          |
| Cowork         | Cowork 会话不受 ZDR 保护。                                                                   |
| Claude Code 分析 | 不存储提示或模型响应，但收集生产力元数据，如账户电子邮件和使用统计。对于 ZDR 组织，贡献指标不可用；[分析仪表板](03-使用分析跟踪团队使用情况.md)仅显示使用指标。 |
...
| 功能                                                   | 原因                       |
| ---------------------------------------------------- | ------------------------ |
| [网络上的 Claude Code](07-ClaudeCodeontheweb.md)    | 需要服务器端存储对话历史。            |
| 来自 Desktop 应用的[远程会话](13-使用ClaudeCodeDesktop.md#remote-sessions) | 需要包含提示和完成的持久会话数据。        |
