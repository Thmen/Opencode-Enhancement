...
这很重要，因为当 context 填充时，LLM 性能会下降。当 context window 即将满时，Claude 可能会开始"遗忘"早期的指令或犯更多错误。context window 是最重要的资源。使用 [自定义状态行](../48-自定义你的状态行.md) 持续跟踪 context 使用情况，并查看 [减少 token 使用](../13-有效管理成本.md#reduce-token-usage) 了解减少 token 使用的策略。
...
CLAUDE.md 在每个会话中加载，所以只包括广泛适用的东西。对于仅有时相关的域知识或工作流，改用 [skills](../47-使用skills扩展Claude.md)。Claude 按需加载它们，不会使每次对话都膨胀。
...
阅读更多关于 [权限模式](04-选择权限模式.md)、[权限规则](../37-配置权限.md) 和 [沙箱](../41-Sandboxing.md)。
...
使用 [MCP servers](../31-通过MCP将ClaudeCode连接到工具.md)，你可以要求 Claude 从问题跟踪器实现功能、查询数据库、分析监控数据、集成来自 Figma 的设计并自动化工作流。
...
[Hooks](../26-使用hooks自动化工作流.md) 在 Claude 工作流中的特定点自动运行脚本。与 CLAUDE.md 指令不同，hooks 是确定性的，保证操作发生。
...
[Skills](../47-使用skills扩展Claude.md) 使用特定于你的项目、团队或域的信息扩展 Claude 的知识。Claude 在相关时自动应用它们，或者你可以使用 `/skill-name` 直接调用它们。
...
[Subagents](../49-创建自定义subagents.md) 在自己的 context 中运行，拥有自己的一组允许的工具。它们对于读取许多文件或需要专门关注而不会使你的主对话混乱的任务很有用。
...
[Plugins](../39-创建插件.md) 将 skills、hooks、subagents 和 MCP 服务器捆绑到来自社区和 Anthropic 的单个可安装单元中。如果你使用类型化语言，安装 [代码智能 plugin](../18-通过市场发现和安装预构建插件.md#code-intelligence) 来为 Claude 提供精确的符号导航和编辑后的自动错误检测。
...
* 对于不需要留在 context 中的快速问题，使用 [`/btw`](../27-交互模式.md#side-questions-with-btw)。答案出现在可关闭的覆盖层中，永远不会进入对话历史，所以你可以检查细节而不增加 context。
...
Claude 在更改前自动检查点。双击 `Escape` 或运行 `/rewind` 来打开 rewind 菜单。你可以仅恢复对话、仅恢复代码、恢复两者或从选定的消息进行总结。有关详细信息，请参阅 [Checkpointing](../08-checkpointing.md)。
...
* [Claude Code 桌面应用](../15-使用ClaudeCodeDesktop.md#work-in-parallel-with-sessions)：以视觉方式管理多个本地会话。每个会话获得自己的隔离 worktree。
* [Claude Code 在网络上](../09-ClaudeCodeontheweb.md)：在 Anthropic 的安全云基础设施中的隔离 VM 上运行。
