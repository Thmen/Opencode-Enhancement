[首页](../README.md) / [使用 Claude Code](README.md) / Claude Code 最佳实践
...
本指南涵盖了在 Anthropic 内部团队和在各种代码库、语言和环境中使用 Claude Code 的工程师中已被证明有效的模式。有关代理循环如何在幕后工作的信息，请参阅 [Claude Code 如何工作](../核心概念/02-ClaudeCode如何工作.md)。
...
这很重要，因为当 context 填充时，LLM 性能会下降。当 context window 即将满时，Claude 可能会开始"遗忘"早期的指令或犯更多错误。context window 是最重要的资源。使用 [自定义状态行](../46-自定义你的状态行.md) 持续跟踪 context 使用情况，并查看 [减少 token 使用](../11-有效管理成本.md#reduce-token-usage) 了解减少 token 使用的策略。
...
UI 更改可以使用 [Chrome 中的 Claude 扩展](../平台和集成/01-在Chrome中使用ClaudeCode（测试版）.md) 进行验证。它在浏览器中打开新标签页，测试 UI，并迭代直到代码工作。
...
让 Claude 直接跳到编码可能会产生解决错误问题的代码。使用 [Plan Mode](02-常见工作流程.md#use-plan-mode-for-safe-code-analysis) 将探索与执行分开。
...
一些设置步骤使 Claude Code 在所有会话中显著更有效。有关扩展功能的完整概述和何时使用每个功能，请参阅 [扩展 Claude Code](../核心概念/01-扩展ClaudeCode.md)。
...
CLAUDE.md 在每个会话中加载，所以只包括广泛适用的东西。对于仅有时相关的域知识或工作流，改用 [skills](../45-使用skills扩展Claude.md)。Claude 按需加载它们，不会使每次对话都膨胀。
...
<Tip>
  使用 [auto mode](04-选择权限模式.md#eliminate-prompts-with-auto-mode) 让分类器处理批准，使用 `/permissions` 来允许列表特定命令，或使用 `/sandbox` 进行操作系统级隔离。每种方式都减少中断，同时让你保持控制。
</Tip>
...
阅读更多关于 [权限模式](04-选择权限模式.md)、[权限规则](../35-配置权限.md) 和 [沙箱](../39-Sandboxing.md)。
...
使用 [MCP servers](../29-通过MCP将ClaudeCode连接到工具.md)，你可以要求 Claude 从问题跟踪器实现功能、查询数据库、分析监控数据、集成来自 Figma 的设计并自动化工作流。
...
[Hooks](../24-使用hooks自动化工作流.md) 在 Claude 工作流中的特定点自动运行脚本。与 CLAUDE.md 指令不同，hooks 是确定性的，保证操作发生。
...
[Skills](../45-使用skills扩展Claude.md) 使用特定于你的项目、团队或域的信息扩展 Claude 的知识。Claude 在相关时自动应用它们，或者你可以使用 `/skill-name` 直接调用它们。
...
[Subagents](../47-创建自定义subagents.md) 在自己的 context 中运行，拥有自己的一组允许的工具。它们对于读取许多文件或需要专门关注而不会使你的主对话混乱的任务很有用。
...
[Plugins](../37-创建插件.md) 将 skills、hooks、subagents 和 MCP 服务器捆绑到来自社区和 Anthropic 的单个可安装单元中。如果你使用类型化语言，安装 [代码智能 plugin](../16-通过市场发现和安装预构建插件.md#code-intelligence) 来为 Claude 提供精确的符号导航和编辑后的自动错误检测。

有关在 skills、subagents、hooks 和 MCP 之间选择的指导，请参阅 [扩展 Claude Code](../核心概念/01-扩展ClaudeCode.md#match-features-to-your-goal)。
...
* 对于不需要留在 context 中的快速问题，使用 [`/btw`](../25-交互模式.md#side-questions-with-btw)。答案出现在可关闭的覆盖层中，永远不会进入对话历史，所以你可以检查细节而不增加 context。
...
Claude 在更改前自动检查点。双击 `Escape` 或运行 `/rewind` 来打开 rewind 菜单。你可以仅恢复对话、仅恢复代码、恢复两者或从选定的消息进行总结。有关详细信息，请参阅 [Checkpointing](../06-checkpointing.md)。
...
* [Claude Code 桌面应用](../13-使用ClaudeCodeDesktop.md#work-in-parallel-with-sessions)：以视觉方式管理多个本地会话。每个会话获得自己的隔离 worktree。
* [Claude Code 在网络上](../07-ClaudeCodeontheweb.md)：在 Anthropic 的安全云基础设施中的隔离 VM 上运行。
* [Agent teams](../01-协调ClaudeCode会话团队.md)：具有共享任务、消息和团队主管的多个会话的自动协调。
...
为了不间断的执行和后台安全检查，使用 [auto mode](04-选择权限模式.md#eliminate-prompts-with-auto-mode)。分类器模型在命令运行前审查它们，阻止范围升级、未知基础设施和由敌对内容驱动的操作，同时让常规工作无提示进行。

```bash  theme={null}
claude --permission-mode auto -p "fix all lint errors"
```

对于使用 `-p` 标志的非交互运行，如果分类器重复阻止操作，auto mode 会中止，因为没有用户可以回退到。请参阅 [auto mode 何时回退](04-选择权限模式.md#when-auto-mode-falls-back) 了解阈值。
...
* [Claude Code 如何工作](../核心概念/02-ClaudeCode如何工作.md)：代理循环、工具和 context 管理
* [扩展 Claude Code](../核心概念/01-扩展ClaudeCode.md)：skills、hooks、MCP、subagents 和 plugins
* [常见工作流](02-常见工作流程.md)：调试、测试、PR 等的分步配方
* [CLAUDE.md](03-Claude如何记住你的项目.md)：存储项目约定和持久 context
