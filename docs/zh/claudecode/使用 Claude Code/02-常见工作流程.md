...
<Tip>
  ...
  * 明确说明您要查找的内容
  * 使用项目中的领域语言
  * 为您的语言安装[代码智能插件](../16-通过市场发现和安装预构建插件.md#code-intelligence)，以便 Claude 能够精确地进行"转到定义"和"查找引用"导航
</Tip>
...
<Tip>
  ...
  * 查看[subagents 文档](../47-创建自定义subagents.md)了解详细示例
</Tip>
...
Plan Mode 指示 Claude 通过使用只读操作分析代码库来创建计划，非常适合探索代码库、规划复杂更改或安全地审查代码。在 Plan Mode 中，Claude 使用 [`AskUserQuestion`](../50-工具参考.md) 在提出计划之前收集需求并澄清您的目标。
...
您也可以使用 `-p` 直接在 Plan Mode 中运行查询（即在["无头模式"](../22-以编程方式运行ClaudeCode.md)中）：
...
有关更多配置选项，请参阅[设置文档](../43-ClaudeCode设置.md#available-settings)。
...
<Steps>
  ...
  <Step title="引用 MCP 资源">
    ...
    这使用 @server:resource 格式从连接的 MCP 服务器获取数据。有关详细信息，请参阅 [MCP 资源](../29-通过MCP将ClaudeCode连接到工具.md#use-mcp-resources)。
  </Step>
</Steps>
...
此外，Opus 4.6 和 Sonnet 4.6 支持自适应推理：不是固定的思考令牌预算，而是模型根据您的[努力级别](../31-模型配置.md#adjust-effort-level)设置动态分配思考。扩展思考和自适应推理一起工作，让您能够控制 Claude 在响应前的推理深度。
...
| 范围                   | 如何配置                                                                          | 详细信息                                                                                        |
| -------------------- | ----------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| **努力级别**             | 运行 `/effort`，在 `/model` 中调整，或设置 [`CLAUDE_CODE_EFFORT_LEVEL`](../17-环境变量.md) | 控制 Opus 4.6 和 Sonnet 4.6 的思考深度。请参阅[调整努力级别](../31-模型配置.md#adjust-effort-level)         |
| **`ultrathink` 关键字** | 在提示中的任何地方包含 "ultrathink"                                                      | 在 Opus 4.6 和 Sonnet 4.6 上为该轮设置努力为高。对于需要深度推理的一次性任务很有用，无需永久更改您的努力设置                           |
| **切换快捷键**            | 按 `Option+T`（macOS）或 `Alt+T`（Windows/Linux）                                   | 为当前会话切换思考开/关（所有模型）。可能需要[终端配置](../48-优化您的终端设置.md)来启用 Option 键快捷键                        |
| **全局默认值**            | 使用 `/config` 切换 Thinking Mode                                                 | 在所有项目中设置默认值（所有模型）。<br />保存为 `~/.claude/settings.json` 中的 `alwaysThinkingEnabled`            |
| **限制令牌预算**           | 设置 [`MAX_THINKING_TOKENS`](../17-环境变量.md) 环境变量                              | 将思考预算限制为特定数量的令牌（在 Opus 4.6 和 Sonnet 4.6 上被忽略，除非设置为 0）。示例：`export MAX_THINKING_TOKENS=10000` |
...
**使用 Opus 4.6 和 Sonnet 4.6**，思考使用自适应推理：模型根据您选择的[努力级别](../31-模型配置.md#adjust-effort-level)动态分配思考令牌。这是调整速度和推理深度之间权衡的推荐方式。

**使用较旧的模型**，思考使用固定预算，最多 31,999 个令牌来自您的输出预算。您可以使用 [`MAX_THINKING_TOKENS`](../17-环境变量.md) 环境变量限制此，或通过 `/config` 或 `Option+T`/`Alt+T` 切换完全禁用思考。

`MAX_THINKING_TOKENS` 在 Opus 4.6 和 Sonnet 4.6 上被忽略，因为自适应推理控制思考深度。一个例外：设置 `MAX_THINKING_TOKENS=0` 仍然在任何模型上完全禁用思考。要禁用自适应思考并恢复到固定思考预算，请设置 `CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING=1`。请参阅[环境变量](../17-环境变量.md)。
...
Subagents 也可以使用 worktree 隔离来并行工作而不会冲突。要求 Claude "use worktrees for your agents" 或在[自定义 subagent](../47-创建自定义subagents.md#supported-frontmatter-fields) 中通过在代理的 frontmatter 中添加 `isolation: worktree` 来配置它。每个 subagent 获得自己的 worktree，当 subagent 完成而没有更改时自动清理。
...
Worktree 隔离默认使用 git。对于其他版本控制系统如 SVN、Perforce 或 Mercurial，配置 [WorktreeCreate 和 WorktreeRemove hooks](../23-Hooks参考.md#worktreecreate) 以提供自定义 worktree 创建和清理逻辑。配置后，这些 hooks 在您使用 `--worktree` 时替换默认的 git 行为。
...
当您启动长时间运行的任务并切换到另一个窗口时，您可以设置桌面通知，以便在 Claude 完成或需要您的输入时了解。这使用 `Notification` [hook 事件](../24-使用hooks自动化工作流.md#get-notified-when-claude-needs-input)，每当 Claude 等待权限、空闲并准备好新提示或完成身份验证时触发。
...
有关完整的事件架构和通知类型，请参阅[通知参考](../23-Hooks参考.md#notification)。
