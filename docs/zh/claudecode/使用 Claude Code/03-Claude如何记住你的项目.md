...
* [编写和组织 CLAUDE.md 文件](../快速开始/01-ClaudeCode概述.md#claudemd-files)
...
subagents 也可以维护自己的自动记忆。有关详细信息，请参阅 [subagent 配置](../47-创建自定义subagents.md#enable-persistent-memory)。
...
<Note>
  规则在每个会话或打开匹配文件时加载到上下文中。对于不需要始终在上下文中的特定任务指令，改用 [skills](../45-使用skills扩展Claude.md)，它仅在你调用它们或 Claude 确定它们与你的提示相关时加载。
</Note>
...
<Steps>
  ...
  <Step title="使用你的配置管理系统部署">
    使用 MDM、Group Policy、Ansible 或类似工具在开发者机器上分发文件。有关其他组织范围配置选项，请参阅 [托管设置](../35-配置权限.md#managed-settings)。
  </Step>
</Steps>
...
模式使用 glob 语法与绝对文件路径匹配。你可以在任何 [设置层](../43-ClaudeCode设置.md#settings-files)：用户、项目、本地或托管策略配置 `claudeMdExcludes`。数组跨层合并。
...
对于你想要在系统提示级别的指令，使用 [`--append-system-prompt`](../08-CLI参考.md#system-prompt-flags)。这必须在每次调用时传递，因此它更适合脚本和自动化而不是交互式使用。

<Tip>
  使用 [`InstructionsLoaded` hook](../23-Hooks参考.md#instructionsloaded) 记录确切加载了哪些指令文件、何时加载以及为什么。这对于调试特定路径规则或子目录中的延迟加载文件很有用。
</Tip>
...
* [Skills](../45-使用skills扩展Claude.md)：打包按需加载的可重复工作流
* [Settings](../43-ClaudeCode设置.md)：使用设置文件配置 Claude Code 行为
* [管理会话](/zh-CN/sessions)：管理上下文、恢复对话和运行并行会话
* [Subagent 记忆](../47-创建自定义subagents.md#enable-persistent-memory)：让 subagents 维护自己的自动记忆
