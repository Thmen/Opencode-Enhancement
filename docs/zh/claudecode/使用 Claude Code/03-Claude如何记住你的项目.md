[首页](../README.md) / [使用 Claude Code](README.md) / Claude 如何记住你的项目
...
* [编写和组织 CLAUDE.md 文件](../快速开始/01-ClaudeCode概述.md#claude-md-files)
* [使用 `.claude/rules/` 将规则范围限定到特定文件类型](../快速开始/01-ClaudeCode概述.md#organize-rules-with-clauderules)
* [配置自动记忆](../快速开始/01-ClaudeCode概述.md#auto-memory)，使 Claude 自动记笔记
* [故障排除](../快速开始/01-ClaudeCode概述.md#troubleshoot-memory-issues)，当指令未被遵循时
...
subagents 也可以维护自己的自动记忆。有关详细信息，请参阅 [subagent 配置](../49-创建自定义subagents.md#enable-persistent-memory)。
...
工作目录上方目录层次结构中的 CLAUDE.md 文件在启动时完整加载。子目录中的 CLAUDE.md 文件在 Claude 读取这些目录中的文件时按需加载。有关完整的解析顺序，请参阅 [CLAUDE.md 文件如何加载](../快速开始/01-ClaudeCode概述.md#how-claudemd-files-load)。

对于大型项目，你可以使用 [项目规则](../快速开始/01-ClaudeCode概述.md#organize-rules-with-clauderules) 将指令分解为特定主题的文件。规则让你将指令范围限定到特定文件类型或子目录。
...
**大小**：每个 CLAUDE.md 文件目标在 200 行以下。较长的文件消耗更多上下文并降低遵守度。如果你的指令变得很大，使用 [导入](../快速开始/01-ClaudeCode概述.md#import-additional-files) 或 [`.claude/rules/`](../快速开始/01-ClaudeCode概述.md#organize-rules-with-clauderules) 文件进行分割。
...
**一致性**：如果两条规则相互矛盾，Claude 可能会任意选择一条。定期审查你的 CLAUDE.md 文件、子目录中的嵌套 CLAUDE.md 文件和 [`.claude/rules/`](../快速开始/01-ClaudeCode概述.md#organize-rules-with-clauderules) 以删除过时或冲突的指令。在 monorepos 中，使用 [`claudeMdExcludes`](../快速开始/01-ClaudeCode概述.md#exclude-specific-claudemd-files) 跳过与你的工作无关的其他团队的 CLAUDE.md 文件。
...
有关组织指令的更结构化方法，请参阅 [`.claude/rules/`](../快速开始/01-ClaudeCode概述.md#organize-rules-with-clauderules)。
...
如果你在一个大型 monorepo 中工作，其他团队的 CLAUDE.md 文件被拾取，使用 [`claudeMdExcludes`](../快速开始/01-ClaudeCode概述.md#exclude-specific-claudemd-files) 跳过它们。
...
对于较大的项目，你可以使用 `.claude/rules/` 目录将指令组织到多个文件中。这使指令保持模块化并更容易让团队维护。规则也可以 [范围限定到特定文件路径](../快速开始/01-ClaudeCode概述.md#path-specific-rules)，因此它们仅在 Claude 处理匹配文件时加载到上下文中，减少噪音并节省上下文空间。

**Note:**
规则在每个会话或打开匹配文件时加载到上下文中。对于不需要始终在上下文中的特定任务指令，改用 [skills](../47-使用skills扩展Claude.md)，它仅在你调用它们或 Claude 确定它们与你的提示相关时加载。
...
没有 [`paths` frontmatter](../快速开始/01-ClaudeCode概述.md#path-specific-rules) 的规则在启动时加载，优先级与 `.claude/CLAUDE.md` 相同。
...
#### 使用你的配置管理系统部署
使用 MDM、Group Policy、Ansible 或类似工具在开发者机器上分发文件。有关其他组织范围配置选项，请参阅 [托管设置](../37-配置权限.md#managed-settings)。

托管 CLAUDE.md 和 [托管设置](../45-ClaudeCode设置.md#settings-files) 服务于不同的目的。使用设置进行技术强制，使用 CLAUDE.md 进行行为指导：
...
模式使用 glob 语法与绝对文件路径匹配。你可以在任何 [设置层](../45-ClaudeCode设置.md#settings-files)：用户、项目、本地或托管策略配置 `claudeMdExcludes`。数组跨层合并。
...
自动记忆文件是纯 markdown，你可以随时编辑或删除。运行 [`/memory`](../快速开始/01-ClaudeCode概述.md#view-and-edit-with-memory) 从会话中浏览和打开记忆文件。
...
* 检查相关 CLAUDE.md 是否在为你的会话加载的位置（参见 [选择 CLAUDE.md 文件的位置](../快速开始/01-ClaudeCode概述.md#choose-where-to-put-claudemd-files)）。
* 使指令更具体。"使用 2 空格缩进"比"格式化代码很好"效果更好。
* 查找跨 CLAUDE.md 文件的冲突指令。如果两个文件为相同行为提供不同的指导，Claude 可能会任意选择一个。

对于你想要在系统提示级别的指令，使用 [`--append-system-prompt`](../10-CLI参考.md#system-prompt-flags)。这必须在每次调用时传递，因此它更适合脚本和自动化而不是交互式使用。

**Tip:**
使用 [`InstructionsLoaded` hook](../25-Hooks参考.md#instructionsloaded) 记录确切加载了哪些指令文件、何时加载以及为什么。这对于调试特定路径规则或子目录中的延迟加载文件很有用。
...
超过 200 行的文件消耗更多上下文并可能降低遵守度。将详细内容移到使用 `@path` 导入引用的单独文件中（参见 [导入其他文件](../快速开始/01-ClaudeCode概述.md#import-additional-files)），或将你的指令分割到 `.claude/rules/` 文件中。
...
有关大小、结构和具体性的指导，请参阅 [编写有效的指令](../快速开始/01-ClaudeCode概述.md#write-effective-instructions)。

## 相关资源

* [Skills](../47-使用skills扩展Claude.md)：打包按需加载的可重复工作流
* [Settings](../45-ClaudeCode设置.md)：使用设置文件配置 Claude Code 行为
* [管理会话](/zh-CN/sessions)：管理上下文、恢复对话和运行并行会话
* [Subagent 记忆](../49-创建自定义subagents.md#enable-persistent-memory)：让 subagents 维护自己的自动记忆
