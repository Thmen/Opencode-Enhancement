[首页](../README.md) / [平台和集成](README.md) / 在 Chrome 中使用 Claude Code（测试版）

# 在 Chrome 中使用 Claude Code（测试版）

> 将 Claude Code 连接到 Chrome 浏览器，以测试网络应用、使用控制台日志进行调试、自动填充表单以及从网页中提取数据。

Claude Code 与 Claude in Chrome 浏览器扩展程序集成，为您提供从 CLI 或 [VS Code 扩展程序](06-在VSCode中使用ClaudeCode.md#automate-browser-tasks-with-chrome) 进行浏览器自动化的功能。构建您的代码，然后在浏览器中测试和调试，无需切换上下文。
...
* [Claude Code](../快速开始/02-快速开始.md#step-1-install-claude-code) 版本 2.0.73 或更高版本
...
对于 VS Code，请参阅 [VS Code 中的浏览器自动化](06-在VSCode中使用ClaudeCode.md#automate-browser-tasks-with-chrome)。

### 默认启用 Chrome

为了避免每个会话都传递 `--chrome`，运行 `/chrome` 并选择"默认启用"。

在 [VS Code 扩展程序](06-在VSCode中使用ClaudeCode.md#automate-browser-tasks-with-chrome) 中，只要安装了 Chrome 扩展程序，Chrome 就可用。无需额外标志。
...
* [在 VS Code 中使用 Claude Code](06-在VSCode中使用ClaudeCode.md#automate-browser-tasks-with-chrome)：VS Code 扩展程序中的浏览器自动化
* [CLI 参考](../10-CLI参考.md)：命令行标志，包括 `--chrome`
* [常见工作流](../使用 Claude Code/02-常见工作流程.md)：更多使用 Claude Code 的方式
* [数据和隐私](../14-数据使用.md)：Claude Code 如何处理您的数据
