...
* **远程使用您的完整本地环境**：您的文件系统、[MCP servers](../31-通过MCP将ClaudeCode连接到工具.md)、工具和项目配置都保持可用
* **同时从两个界面工作**：对话在所有连接的设备上保持同步，因此您可以从终端、浏览器和手机交替发送消息
* **在中断后恢复**：如果您的笔记本电脑进入睡眠状态或网络断开，当您的机器重新上线时，会话会自动重新连接

与[网络上的 Claude Code](../09-ClaudeCodeontheweb.md)（在云基础设施上运行）不同，Remote Control 会话直接在您的机器上运行并与您的本地文件系统交互。网络和移动界面只是该本地会话的一个窗口。
...
| `--sandbox` / `--no-sandbox` | 启用或禁用[沙箱](../41-Sandboxing.md)以进行文件系统和网络隔离。默认关闭。                                                                                                                                                                      |
...
Remote Control 和[网络上的 Claude Code](../09-ClaudeCodeontheweb.md)都使用 claude.ai/code 界面。关键区别在于会话运行的位置：Remote Control 在您的机器上执行，因此您的本地 MCP servers、工具和项目配置保持可用。网络上的 Claude Code 在 Anthropic 管理的云基础设施中执行。
...
* **您的 Team 或 Enterprise 管理员尚未启用它**：Remote Control 在这些计划上默认处于关闭状态。管理员可以在 [claude.ai/admin-settings/claude-code](https://claude.ai/admin-settings/claude-code) 通过打开 **Remote Control** 切换来启用它。这是一个服务器端组织设置，不是[仅管理设置](../37-配置权限.md#managed-only-settings)密钥。
...
| :--------------------------------------------- | :--------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------ |
| [Dispatch](../15-使用ClaudeCodeDesktop.md#sessions-from-dispatch) | Message a task from the Claude mobile app                                                      | Your machine (Desktop)                                                                                           | [Pair the mobile app with Desktop](https://support.claude.com/en/articles/13947068)                                                  | Delegating work while you're away, minimal setup              |
...
| [Channels](../06-使用channels将事件推送到运行中的会话.md)                       | Push events from a chat app like Telegram or Discord, or your own server                       | Your machine (CLI)                                                                                               | [Install a channel plugin](../06-使用channels将事件推送到运行中的会话.md#quickstart) or [build your own](../07-Channels参考.md)                                      | Reacting to external events like CI failures or chat messages |
| [Slack](05-Slack中的ClaudeCode.md)                             | Mention `@Claude` in a team channel                                                            | Anthropic cloud                                                                                                  | [Install the Slack app](05-Slack中的ClaudeCode.md#setting-up-claude-code-in-slack) with [Claude Code on the web](../09-ClaudeCodeontheweb.md) enabled | PRs and reviews from team chat                                |
| [Scheduled tasks](../42-按计划运行提示词.md)         | Set a schedule                                                                                 | [CLI](../42-按计划运行提示词.md), [Desktop](../15-使用ClaudeCodeDesktop.md#schedule-recurring-tasks), or [cloud](../55-在网络上安排任务.md) | Pick a frequency                                                                                                                     | Recurring automation like daily reviews                       |

## 相关资源

* [网络上的 Claude Code](../09-ClaudeCodeontheweb.md)：在 Anthropic 管理的云环境中运行会话，而不是在您的机器上
* [Channels](../06-使用channels将事件推送到运行中的会话.md)：将 Telegram 或 Discord 转发到会话中，以便 Claude 在您离开时对消息做出反应
* [Dispatch](../15-使用ClaudeCodeDesktop.md#sessions-from-dispatch)：从您的手机发送任务消息，它可以生成 Desktop 会话来处理它
* [身份验证](../04-身份验证.md)：设置 `/login` 并管理 claude.ai 的凭证
* [CLI 参考](../10-CLI参考.md)：包括 `claude remote-control` 的标志和命令的完整列表
* [安全](../43-安全性.md)：Remote Control 会话如何适应 Claude Code 安全模型
* [数据使用](../14-数据使用.md)：在本地和远程会话期间通过 Anthropic API 流动的数据
