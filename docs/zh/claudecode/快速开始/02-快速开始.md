[首页](../README.md) / [快速开始](README.md) / 快速开始
...
* 一个可以使用的代码项目
* 一个 [Claude 订阅](https://claude.com/pricing?utm_source=claude_code\&utm_medium=docs\&utm_content=quickstart_prereq)（Pro、Max、Teams 或 Enterprise）、[Claude Console](https://console.anthropic.com/) 账户，或通过[支持的云提供商](../51-企业部署概览.md)的访问权限

**Note:**
本指南涵盖终端 CLI。Claude Code 也可在[网页](https://claude.ai/code)、[桌面应用](../15-使用ClaudeCodeDesktop.md)、[VS Code](../平台和集成/06-在VSCode中使用ClaudeCode.md) 和 [JetBrains IDE](../平台和集成/02-JetBrainsIDEs.md)、[Slack](../平台和集成/05-Slack中的ClaudeCode.md) 中使用，以及通过 [GitHub Actions](../21-ClaudeCodeGitHubActions.md) 和 [GitLab](../22-ClaudeCodeGitLabCICD.md) 进行 CI/CD。查看[所有界面](01-ClaudeCode概述.md#use-claude-code-everywhere)。
...
* [Amazon Bedrock、Google Vertex AI 或 Microsoft Foundry](../51-企业部署概览.md)（企业云提供商）
...
**Tip:**
登录后（步骤 2），您的凭证将存储在您的系统上。在[凭证管理](../04-身份验证.md#credential-management)中了解更多信息。
...
有关完整的命令列表，请参阅 [CLI 参考](../10-CLI参考.md)。

## 初学者专业提示

有关更多信息，请参阅[最佳实践](../使用 Claude Code/01-ClaudeCode最佳实践.md)和[常见工作流](../使用 Claude Code/02-常见工作流程.md)。
...
- [Claude Code 如何工作](../核心概念/02-ClaudeCode如何工作.md): 了解代理循环、内置工具以及 Claude Code 如何与您的项目交互

- [最佳实践](../使用 Claude Code/01-ClaudeCode最佳实践.md): 通过有效的提示和项目设置获得更好的结果

- [常见工作流](../使用 Claude Code/02-常见工作流程.md): 常见任务的分步指南

- [扩展 Claude Code](../核心概念/01-扩展ClaudeCode.md): 使用 CLAUDE.md、skills、hooks、MCP 等进行自定义
