[首页](../README.md) / [核心概念](README.md) / 扩展 Claude Code

# 扩展 Claude Code

> 了解何时使用 CLAUDE.md、Skills、subagents、hooks、MCP 和 plugins。

Claude Code 结合了一个能够推理代码的模型和[内置工具](02-ClaudeCode如何工作.md#tools)，用于文件操作、搜索、执行和网络访问。内置工具涵盖了大多数编码任务。本指南涵盖扩展层：您添加的功能，用于自定义 Claude 的知识、将其连接到外部服务以及自动化工作流。

**Note:**
有关核心代理循环如何工作的信息，请参阅 [Claude Code 如何工作](02-ClaudeCode如何工作.md)。

**初次使用 Claude Code？** 从 [CLAUDE.md](../使用 Claude Code/03-Claude如何记住你的项目.md) 开始了解项目约定。根据需要添加其他扩展。

## 概述

扩展插入代理循环的不同部分：

* **[CLAUDE.md](../使用 Claude Code/03-Claude如何记住你的项目.md)** 添加 Claude 每个会话都能看到的持久上下文
* **[Skills](../47-使用skills扩展Claude.md)** 添加可重用的知识和可调用的工作流
* **[MCP](../31-通过MCP将ClaudeCode连接到工具.md)** 将 Claude 连接到外部服务和工具
* **[Subagents](../49-创建自定义subagents.md)** 在隔离的上下文中运行自己的循环，返回摘要
* **[Agent teams](../01-协调ClaudeCode会话团队.md)** 协调多个独立会话，具有共享任务和点对点消息传递
* **[Hooks](../25-Hooks参考.md)** 完全在循环外作为确定性脚本运行
* **[Plugins](../39-创建插件.md)** 和 **[marketplaces](../38-创建和分发pluginmarketplace.md)** 打包和分发这些功能

[Skills](../47-使用skills扩展Claude.md) 是最灵活的扩展。Skill 是一个包含知识、工作流或说明的 markdown 文件。您可以使用 `/deploy` 之类的命令调用 skills，或者 Claude 可以在相关时自动加载它们。Skills 可以在您当前的对话中运行，也可以通过 subagents 在隔离的上下文中运行。
...
| **[Agent teams](../01-协调ClaudeCode会话团队.md)** | 协调多个独立的 Claude Code 会话 | 并行研究、新功能开发、使用竞争假设进行调试 | 生成审查者同时检查安全性、性能和测试                      |
| **MCP**                               | 连接到外部服务                | 外部数据或操作               | 查询您的数据库、发布到 Slack、控制浏览器                 |
| **Hook**                              | 在事件上运行的确定性脚本           | 可预测的自动化，不涉及 LLM       | 每次文件编辑后运行 ESLint                        |

**[Plugins](../39-创建插件.md)** 是打包层。Plugin 将 skills、hooks、subagents 和 MCP servers 捆绑到单个可安装单元中。Plugin skills 是命名空间的（如 `/my-plugin:review`），因此多个 plugins 可以共存。当您想在多个存储库中重用相同的设置或通过 **[marketplace](../38-创建和分发pluginmarketplace.md)** 分发给他人时，使用 plugins。
...
**它们可以结合。** Subagent 可以预加载特定的 skills（`skills:` 字段）。Skill 可以使用 `context: fork` 在隔离的上下文中运行。有关详细信息，请参阅 [Skills](../47-使用skills扩展Claude.md)。
...
**经验法则：** 保持 CLAUDE.md 在 200 行以下。如果它在增长，将参考内容移到 skills 或拆分为 [`.claude/rules/`](../使用 Claude Code/03-Claude如何记住你的项目.md#organize-rules-with-clauderules) 文件。
...
**使用 rules 来保持 CLAUDE.md 专注。** 带有 [`paths` frontmatter](../使用 Claude Code/03-Claude如何记住你的项目.md#path-specific-rules) 的 rules 仅在 Claude 处理匹配文件时加载，节省上下文。
...
**Note:**
  Agent teams 是实验性的，默认禁用。有关设置和当前限制，请参阅 [agent teams](../01-协调ClaudeCode会话团队.md)。
...
* **CLAUDE.md 文件** 是累加的：所有级别同时向 Claude 的上下文贡献内容。来自您的工作目录及以上的文件在启动时加载；子目录在您在其中工作时加载。当说明冲突时，Claude 使用判断来协调它们，更具体的说明通常优先。有关详细信息，请参阅 [CLAUDE.md 文件如何加载](../使用 Claude Code/03-Claude如何记住你的项目.md#how-claudemd-files-load)。
* **Skills 和 subagents** 按名称覆盖：当相同的名称存在于多个级别时，一个定义根据优先级获胜（对于 skills 为托管 > 用户 > 项目；对于 subagents 为托管 > CLI 标志 > 项目 > 用户 > plugin）。Plugin skills 是 [命名空间的](../39-创建插件.md#add-skills-to-your-plugin) 以避免冲突。有关详细信息，请参阅 [skill 发现](../47-使用skills扩展Claude.md#where-skills-live) 和 [subagent 范围](../49-创建自定义subagents.md#choose-the-subagent-scope)。
* **MCP 服务器** 按名称覆盖：本地 > 项目 > 用户。有关详细信息，请参阅 [MCP 范围](../31-通过MCP将ClaudeCode连接到工具.md#scope-hierarchy-and-precedence)。
* **Hooks** 合并：所有注册的 hooks 为其匹配的事件触发，无论来源如何。有关详细信息，请参阅 [hooks](../25-Hooks参考.md)。
...
**继承：** Claude 从您的工作目录读取 CLAUDE.md 文件直到根目录，并在访问这些文件时发现子目录中的嵌套文件。有关详细信息，请参阅 [CLAUDE.md 文件如何加载](../使用 Claude Code/03-Claude如何记住你的项目.md#how-claudemd-files-load)。

**Tip:** 保持 CLAUDE.md 在约 500 行以下。将参考材料移到 skills，这些 skills 按需加载。

#### Skills
Skills 是 Claude 工具包中的额外功能。它们可以是参考材料（如 API 风格指南）或可调用的工作流，您可以使用 `/<name>` 触发（如 `/deploy`）。Claude Code 附带 [捆绑的 skills](../47-使用skills扩展Claude.md#bundled-skills)，如 `/simplify`、`/batch` 和 `/debug`，可以开箱即用。您也可以创建自己的。Claude 在适当时使用 skills，或者您可以直接调用一个。
...
**Tip:** 对于有副作用的 skills，使用 `disable-model-invocation: true`。这节省上下文并确保只有您触发它们。
...
**上下文成本：** [工具搜索](../31-通过MCP将ClaudeCode连接到工具.md#scale-with-mcp-tool-search)（默认启用）将 MCP 工具加载到上下文的 10%，并延迟其余部分直到需要。

**可靠性说明：** MCP 连接可能在会话中途无声地失败。如果服务器断开连接，其工具会无警告地消失。Claude 可能尝试使用不再存在的工具。如果您注意到 Claude 无法使用它之前可以访问的 MCP 工具，请使用 `/mcp` 检查连接。

**Tip:** 运行 `/mcp` 查看每个服务器的令牌成本。断开您未主动使用的服务器。
...
**Tip:** 对于不需要您完整对话上下文的工作，使用 subagents。它们的隔离防止膨胀您的主会话。

#### Hooks
**何时：** 触发时。Hooks 在特定的生命周期事件上触发，如工具执行、会话边界、提示提交、权限请求和压缩。有关完整列表，请参阅 [Hooks](../25-Hooks参考.md)。

**加载内容：** 默认情况下无。Hooks 作为外部脚本运行。

**上下文成本：** 零，除非 hook 返回作为消息添加到您的对话中的输出。

**Tip:** Hooks 非常适合不需要影响 Claude 上下文的副作用（linting、logging）。

## 了解更多

每个功能都有自己的指南，包含设置说明、示例和配置选项。

- [CLAUDE.md](../使用 Claude Code/03-Claude如何记住你的项目.md): 存储项目上下文、约定和说明

- [Skills](../47-使用skills扩展Claude.md): 给予 Claude 领域专业知识和可重用的工作流

- [Subagents](../49-创建自定义subagents.md): 将工作卸载到隔离的上下文

- [Agent teams](../01-协调ClaudeCode会话团队.md): 协调多个并行工作的会话

- [MCP](../31-通过MCP将ClaudeCode连接到工具.md): 将 Claude 连接到外部服务

- [Hooks](../26-使用hooks自动化工作流.md): 使用 hooks 自动化工作流

- [Plugins](../39-创建插件.md): 捆绑和共享功能集

- [Marketplaces](../38-创建和分发pluginmarketplace.md): 托管和分发 plugin 集合
