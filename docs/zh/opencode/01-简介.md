...
为了在 Windows 上获得最佳体验，我们推荐使用 [Windows Subsystem for Linux (WSL)](07-Windows.md)。它提供更好的性能，并完全兼容 OpenCode 的所有功能。
...
如果你刚开始接触 LLM 提供商，我们推荐使用 [OpenCode Zen](使用/06-Zen.md)。这是一组经过 OpenCode 团队测试和验证的精选模型。
...
你与 OpenCode 的对话可以[与团队分享](使用/07-分享.md)。
...
要让它更符合你的习惯，我们推荐[选择一个主题](配置/05-主题.md)、[自定义快捷键](配置/06-快捷键.md)、[配置代码格式化工具](配置/08-格式化工具.md)、[创建自定义命令](配置/07-命令.md)，或者探索 [OpenCode 配置](02-配置.md)。
