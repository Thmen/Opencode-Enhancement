...
您还可以配置[本地模型](配置/04-模型.md#local)。[了解更多](配置/04-模型.md)。
...
Bearer Token（`AWS_BEARER_TOKEN_BEDROCK` 或 `/connect`）优先于基于配置文件的身份验证。详情请参见[身份验证优先级](03-提供商.md#authentication-precedence)。

[了解更多关于 Amazon Bedrock 配置的信息](03-提供商.md#amazon-bedrock)。
...
您还可以使用 `~/.config/opencode/agents/` 或 `.opencode/agents/` 中的 Markdown 文件定义代理。[在此了解更多](配置/03-代理.md)。
...
默认代理必须是主代理（不能是子代理）。可以是内置代理（如 `"build"` 或 `"plan"`），也可以是您定义的[自定义代理](配置/03-代理.md)。如果指定的代理不存在或是子代理，OpenCode 将回退到 `"build"` 并发出警告。
...
您可以通过 `share` 选项配置[分享](使用/07-分享.md)功能。
...
您还可以使用 `~/.config/opencode/commands/` 或 `.opencode/commands/` 中的 Markdown 文件定义命令。[在此了解更多](配置/07-命令.md)。
...
[在此了解更多关于格式化程序的信息](配置/08-格式化工具.md)。
...
[在此了解更多](配置/11-MCP服务器.md)。

---

### 插件

[插件](开发/03-插件.md)通过自定义工具、钩子和集成来扩展 OpenCode。
...
该选项接受指令文件路径和 glob 模式的数组。[在此了解更多关于规则的信息](配置/02-规则.md)。
