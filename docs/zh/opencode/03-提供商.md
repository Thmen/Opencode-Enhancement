...
OpenCode Zen 是由 OpenCode 团队提供的模型列表，这些模型已经过测试和验证，能够与 OpenCode 良好配合使用。[了解更多](使用/06-Zen.md)。
...
3. 输入你的 Hugging Face 令牌。
   ...
   └ enter
   ```
4. 执行 `/models` 命令选择模型，例如 *Kimi-K2-Instruct* 或 *GLM-4.6*。
...
OpenCode Zen 是由 OpenCode 团队提供的经过测试和验证的模型列表。[了解更多](使用/06-Zen.md)。
...
3. 输入你的 STACKIT AI Model Serving 认证令牌。
   ...
   └ enter
   ```
4. 执行 `/models` 命令选择模型，例如 *Qwen3-VL 235B* 或 *Llama 3.3 70B*。
...
3. 输入你的 Scaleway API 密钥。
   ...
   └ enter
   ```
4. 执行 `/models` 命令选择模型，例如 *devstral-2-123b-instruct-2512* 或 *gpt-oss-120b*。
