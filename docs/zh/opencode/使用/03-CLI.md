...
OpenCode CLI 在不带任何参数运行时，默认启动 [TUI](02-TUI.md)。
...
此命令会设置必要的 GitHub Actions 工作流并引导您完成配置过程。[了解更多](08-GitHub.md)。
...
启动无界面的 OpenCode 服务器以提供 API 访问。查看[服务器文档](../开发/02-服务器.md)了解完整的 HTTP 接口。
