...
如需了解示例，请查看社区创建的[插件](04-生态系统.md#plugins)。
...
浏览[生态系统](04-生态系统.md#plugins)中的可用插件。
...
**本地插件**直接从插件目录加载。如果需要使用外部包，你必须在配置目录中创建 `package.json`（参见[依赖项](#dependencies)），或者将插件发布到 npm 并[将其添加到配置中](../02-配置.md#plugins)。
...
日志级别：`debug`、`info`、`warn`、`error`。详情请参阅 [SDK 文档](01-SDK.md)。
