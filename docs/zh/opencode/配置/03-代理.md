...
如果您不指定模型，主代理将使用[全局配置的模型](../02-配置.md#models)，而子代理将使用调用它的主代理所使用的模型。
...
OpenCode 配置中的模型 ID 使用 `provider/model-id` 格式。例如，如果您使用 [OpenCode Zen](../使用/06-Zen.md)，则可以使用 `opencode/gpt-5.1-codex` 来表示 GPT 5.1 Codex。
