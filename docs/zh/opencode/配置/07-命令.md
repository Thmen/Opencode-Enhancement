...
自定义命令是 `/init`、`/undo`、`/redo`、`/share`、`/help` 等内置命令之外的补充。[了解更多](../使用/02-TUI.md#commands)。
...
使用 *!`command`* 将 [bash 命令](../使用/02-TUI.md#bash-commands)输出注入到提示词中。
...
使用 `agent` 配置可选地指定由哪个[代理](03-代理.md)执行此命令。
如果这是一个[子代理](03-代理.md#subagents)，该命令默认会触发子代理调用。
...
使用 `subtask` 布尔值强制命令触发[子代理](03-代理.md#subagents)调用。
如果你希望命令不污染主要上下文，这会很有用，它会**强制**代理作为子代理运行，
即使[代理](03-代理.md)配置中的 `mode` 设置为 `primary`。
...
opencode 包含多个内置命令，如 `/init`、`/undo`、`/redo`、`/share`、`/help`；[了解更多](../使用/02-TUI.md#commands)。
