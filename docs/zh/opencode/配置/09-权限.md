...
你可以在模式开头使用 `~` 或 `$HOME` 来引用你的主目录。这对于 [`external_directory`](#%E5%A4%96%E9%83%A8%E7%9B%AE%E5%BD%95) 规则特别有用。
...
此处允许的任何目录都会继承与当前工作空间相同的默认值。由于 [`read` 默认为 `allow`](#%E9%BB%98%E8%AE%A4%E5%80%BC)，`external_directory` 下的条目也允许读取，除非另行覆盖。当需要在这些路径中限制某个工具时，请添加显式规则，例如在保留读取的同时阻止编辑：
...
你可以为每个代理单独覆盖权限。代理权限会与全局配置合并，且代理规则优先。[了解更多](03-代理.md#permissions)关于代理权限的内容。

注意

有关更详细的模式匹配示例，请参阅上方的[细粒度规则（对象语法）](#%E7%BB%86%E7%B2%92%E5%BA%A6%E8%A7%84%E5%88%99%E5%AF%B9%E8%B1%A1%E8%AF%AD%E6%B3%95)部分。
