[首页](../README.md) / [配置](README.md) / MCP 服务器

# MCP 服务器

添加本地和远程 MCP 工具。

你可以通过 *Model Context Protocol*（MCP）为 OpenCode 添加外部工具。OpenCode 同时支持本地和远程服务器。
...
本地配置值会覆盖远程默认值。详情请参阅[配置优先级](../02-配置.md#precedence-order)。
...
1. 全局禁用该工具。
2. 在[代理配置](03-代理.md#tools)中，将 MCP 服务器作为工具启用。
...
你也可以在 [AGENTS.md](02-规则.md) 中添加类似的规则。
